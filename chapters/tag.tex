...
树邻接语法（TAG）
\isc{树邻接语法|(}\is{Tree Adjoining Grammar (TAG)|(}
...
\isc{上下文相关文法}\is{context"=sensitive grammar} （1-型） 语言之间。这个类被称为“弱上下文相关”（mildly context sensitive）
\isc{弱上下文相关}\is{mildly context"=sensitive grammar}。
...
处理量词作用域
\isc{辖域}\is{scope}的问题，他们也讨论了嵌入动词的复杂情况。感兴趣的读者可以阅读他们的原始论文。
...
\glt `因为有人向他保证读这本书'
\z
在（\mex{0}）中, das Buch（这本书）是zu lesen（读）的宾语，ihm（他）和jemand（有个人）分别是versprochen（保证）和hat（AUX）的依赖词。这些例子可以用\citet{Joshi87b}提出来的LD/LP"=TAG进行分析，也可以用自由序TAG（Free Order TAG，简称FO"=TAG
\isc{自由语序树邻接语法}\is{TAG!Free Order (FO"=TAG)})\citep*[\page 21]{BJR91a}进行分析，二者都允许表示依存关系的边交叉。
...
\isc{多成分树邻接语法}\is{Tree Adjoining Grammar (TAG)!Multi"=Component (MC-TAG)|(}。 
...
{}  CONJ  DET.\nom{} 探员 DET.\dat{} 委托人   \spacebr{}DET.\acc{} 嫌疑人 DET.\gen{} 犯罪 INF 起诉 保证\\
...
{}     CONJ  DET.\gen{} 犯罪           DET.\nom{} 探员 DET.\acc{} 嫌疑人   DET.\dat{}  委托人   {}      {}     INF 起诉  保证\\
\zl
		...
		[S
			...
			[VP
				...
				[NP$_2^2$
					[e]]
				[V$_2$
					[zu überführen;INF 起诉]]]]]]
%					[zu überführen;INF 起诉to indict]]]]]]
\end{forest}
\begin{forest}
tag, baseline
[S
...
\isc{多成分树邻接语法}\is{Multi-Component TAG}
\isc{树邻接语法!多成分树邻接语法}\is{Tree Adjoining Grammar (TAG)!Multi"=Component (MC-TAG)|)} 
%Andere Varianten, die andere Konstituentenanordnungen zulassen sind V-TAG
其它允准上述结构成分语序的TAG变体包括V-TAG
\isc{树邻接语法!矢量树邻接语法}\is{Tree Adjoining Grammar (TAG)!Vector (V-TAG)}\citep{Rambow94a} 
和TT-MC-TAG
\isc{树邻接语法!树元组多成分树邻接语法}\is{Tree Adjoining Grammar (TAG)!Tree Tuple MC-TAG (TT-MC-TAG)}\citep{Lichte2007a}。
...
\isc{被动／被动式|(}\is{passive|(}：通过词汇规则来根据一个针对主动形式的词汇项，
...
谁$_i$ AUX John 告诉 Sam CONJ Bill 喜欢 \_$_i$\\
...
谁$_i$ AUX John 告诉 Sam CONJ Mary 说 CONJ Bill 喜欢 \_$_i$\\
...
\isc{附接的必要性}\is{adjunction!obligatory}。
\il{English|)}
...
在FTAG
{基于特征结构的树邻接语法}\is{Tree Adjoining Grammar (TAG)!Feature Structure"=Based (FTAG)|(}中，结点并不是原子的（N、NP、VP或S），而是包含有特征描写。
...
\isc{附接的必要性}\is{adjunction!obligatory|)}机制可以通过令顶结构和底结构特征取值不一致来实现。
如果一棵树中存在不兼容的顶结构与底结构，这棵树就不能是最终的导出树。%又一个错误
这意味着至少需要再进行一次附接操作才能得到良形式的树。
\isc{基于特征结构的树邻接语法}\is{Tree Adjoining Grammar (TAG)!Feature Structure"=Based (FTAG)|)}

\subsection{V"=TAG}
\label{sec-vtag}

V"=TAG
\isc{矢量树邻接语法}\is{Tree Adjoining Grammar (TAG)!Vector (V-TAG)|(}是Owen\citet{Rambow94a}提出来的一种TAG变体，它同样包含了特征结构。
...
\isc{矢量树邻接语法}\is{Tree Adjoining Grammar (TAG)!Vector (V-TAG)|)}
...
%\glt `that the dog that chases the cat that caught the mouse barks'
     CONJ DET 狗  叫  CONJ DET 猫 追 CONJ DET 老鼠 捉 AUX   \\
\glt `追着那只捉老鼠的猫的那只狗在叫'
\ex 
\gll dass der Hund, [$_1$ der  die Katze, [$_2$ die  die Maus  gefangen hat,~$_2$] jagt~$_1$] bellt\\
     ...
     CONJ DET 狗  {}  CONJ DET 猫 {} CONJ DET 老鼠 捉 AUX   捉 叫 \\
\glt `追着那只捉老鼠的猫的那只狗在叫'
\zl
...
DET 长椅 {} PREP 其 当时 DET 老.人 PREP.DET 村庄 {} CONJ 所有 儿童 {} CONJ 来.PREP {} 友好地 目光 给 {}
长 小时 安静 彼此相邻 坐 {} 必须 PREP.DET 上一个 年 ART 停车场 让道\\
...
Hoberg使用的是代词的所有格——ihr（她），而不是das（DET）。
...
%\glt `Hanna has always known that the child wants to leave her.'
	 Hanna AUX 总是 已经 知道 CONJ DET 孩子 她 离开 希望 \\ 
...
%\glt Preferred reading: `Hanna has always known that she wants to leave the child.'
      Hanna AUX 总是 已经 知道 CONJ 她 DET 孩子 离开 希望 \\ 
...
%\glt `because he wants to let the man help the boy remember his friend'
     因为 他.\nom{} DET.\acc{} 男人  DET.\dat{} 男孩    PREP.DET.\gen{} 朋友 记得 帮助 让 希望 \\
...
%     because nobody.\nom{} \spacebr{}the.\acc{} man \spacebr{}the.\dat{} woman \spacebr{}this.\acc{} pond empty fish help saw\\
     因为 没有人.\nom{} \spacebr{}DET.\acc{} 男人 \spacebr{}DET.\dat{} 女人 \spacebr{}这个.\acc{} 池塘 空 钓鱼 帮助 看见 \\
...
% because \spacebr{}the.\dat{} woman \spacebr{}this.\acc{} pond \spacebr{}the.\acc{} man nobody.\nom{} empty fish help saw\\
  因为 \spacebr{}DET.\dat{} 女人 \spacebr{}这个.\acc{} 池塘 \spacebr{}DET.\acc{} 男人 没有人.\nom{} 空 钓鱼 帮助 看见 \\
\glt `因为没有人看见那个男人帮助那个女人把池塘里的鱼钓光了'
\z
...
\isc{矢量树邻接语法}\is{Tree Adjoining Grammar (TAG)!Vector (V-TAG)}或TT-MC-TAG
\isc{树元组多成分树邻接语法}\is{Tree Adjoining Grammar (TAG)!Tree Tuple MC-TAG (TT-MC-TAG)}这样的TAG变体\citep{Lichte2007a}，这些变体能够解释多种语序，然后可以使用语言运用模型来解释为什么句子可接受程度不同。
...
\isc{矢量树邻接语法}\is{Tree Adjoining Grammar (TAG)!Vector (V-TAG)}）和Karttunen的激进词汇主义（Radical Lexicalism）方法、Uszkoreit的GPSG\indexgpsgc、组合范畴语法\indexcgc, HPSG\indexhpsgc 以及依存语法\indexdgc。 
