...
从表示力来看，TAG和它的一些扩展性变体相对精确地展现了人类在理解和处理语言的时候做了什么，这也是这种语法理论能够吸引大量研究人员注意的主要原因。为了便于和上下文无关文法\isce{上下文无关文法}{context-free grammar}（2-型语言）形成对应，广义短语结构语法\indexgpsgc 在设计之初就对表示能力施加了很多限制，即便如此，事实上，GPSG在这方面仍然有很多问题\citep{Shieber85a,Culy85a}\footnote{%
...
目前通行的观点认为自然语言的复杂度位于上下文无关\isce{复杂类型}{complexity class}\isce{上下文无关文法}{context-free grammar}和上下文相关\isce{上下文相关文法}{context-sensitive grammar}（1-型）语言之间。这一类型被称为“弱上下文相关”（mildly context sensitive）。\isce{弱上下文相关文法}{mildly context-sensitive grammar}有些TAG变体属于这一语言类型，有种观点认为它们恰好产生出自然语言中的结构，不多也不少。
...
\isce[|(]{附加}{adjunction}
\isce[|(]{附加语}{adjunct}展示了如何使用always的附加树（adjunction tree）。
...
\isce{多成分树邻接语法}{Multi-Component TAG}是TAG的一个扩展， \citet{KJ2003a}展示了如何用LTAG
...
行分析，也可以用自由序TAG（Free Order TAG，简称FO-TAG\isc{树邻接语法（TAG）!自由语序树邻接语法（FO-TAG）}\is{TAG!Free Order (FO-TAG)})\citep*[\page 21]{BJR91a}进行分析，二者都允许表示依存关系的边交叉。\isce{可投射性}{projectivity}

因为某些限制在FO-TAG中不能很好地表示\citep[\page 48--50]{Rambow94a}，研究者又提出了多元成分TAG（Multi-Component TAG，简称MC-TAG）。%
\isce{多成分树邻接语法}{Multi-Component TAG}\iscesub[|(]{树邻接语法（TAG）}{Tree Adjoining Grammar (TAG)}{多成分树邻接语法（MC-TAG）}{Multi-Component (MC-TAG)}
...
对于不定式动词，假定主语由一个PRO
\isce{PRO}{PRO}来实现。PRO像\emph{e}一样，是一个语音形式为空的代词性范畴\is{empty element!PRO|see{PRO}}，它同样源自GB理论。
...
\isce{多成分树邻接语法}{Multi-Component TAG}
...
其他允准上述结构成分语序的TAG变体包括V-TAG\isc{树邻接语法（TAG）!矢量树邻接语法（V-TAG）}\is{Tree Adjoining Grammar (TAG)!Vector (V-TAG)}\citep{Rambow94a}和TT-MC-TAG\citep{Lichte2007a}。\isc{树邻接语法（TAG）!树元组多成分树邻接语法（TT-MC-TAG）}\is{Tree Adjoining Grammar (TAG)!Tree Tuple MC-TAG (TT-MC-TAG)}\isce[|)]{成分序列}{constituent order}
...
这个例子展示了，我们在处理长距离依存时曾经采用过的强制附加\iscesub{附加}{adjunction}{附加的必要性}{obligatory}机制可以通过令顶结构和底结构特征取值不一致来实现。
...
V-TAG
\iscesub[|(]{树邻接语法（TAG）}{Tree Adjoining Grammar (TAG)}{矢量树邻接语法（V-TAG）}{Vector (V-TAG)}是Owen \citet{Rambow94a}提出来的一种TAG变体，它同样包含了特征结构。
...
\iscesub[|)]{树邻接语法（TAG）}{Tree Adjoining Grammar (TAG)}{矢量树邻接语法（V-TAG）}{Vector (V-TAG)}
...
本书中\isce[|(]{语言能力}{competence}\isce[|(]{语言运用}{performance}讨论的很多理论都区分语言能力（competence）和语言运用（performance）\citep[\S~I.1]{Chomsky65a}。我们假定语言能力理论描写语言知识，而语言运用理论应该解释语言知识如何被使用以及为什么我们在使用和理解语言的过程中会产生错误等问题。参见第\ref{chap-competence-performance}章以了解更多的内容。 
...
在构造例子的过程中，很重要的是确保所有涉及到的名词尽量采用不同的格\isce{格}{case}和选择限制（selectional restrictions）\isce{选择限制}{selection!restriction}（如有生命/无生命），因为一个听者/读者可以用这些特征来将语序重列的论元匹配到它们的中心词上。
...
\isce[|)]{语言能力}{competence}
\isce[|)]{语言运用}{performance}
...
因为递归从基本树中被排除掉了，因此在导出树（局域的扩展域\isce{局部性}{locality}）中距离特别远的元素可以包含在同一个基本树中。
 ...
 \citet*{KKNV95a}论证了满足特定要求的HPSG语法可以转写为TAG语法。通过这种方式，我们可以得到一个计算复杂度\isce{生成能力}{capacity!generative}更明确的语法。HPSG语法一般都是0-型文法，而不同的TAG变体可以用来刻画从2-型语言到弱上下文相关文法\isce{弱上下文相关文法}{mildly context-sensitive grammar}之间的不同语言\citep{Joshi85a-u}。 \citet*{YMTT2001a}提出了一个算法可以将FB-LTAG语法翻译成弱上下文相关文法的HPSG文法。
...
重要的论文有 \citew*{JLT75a-u}、 \citew{Joshi87a-u}和 \citew{JS97a}。很多更关心语言本体的读者并不会过多涉猎讨论TAG形式性质的论文。
 ...
 \citet{SJ93a}讨论了从心理语言学来说可行的加工模型，并且论证了基于TAG进行增量式句法分析的可行性。他们进一步提出了TAG的一种变体：同步TAG（synchronous TAG\indexstagc）。在这种TAG变体中，一棵句法树有一棵语义树与之相对应。当构建句法结构的时候，语义结构也同步构建起来。这个同步构建的结构对应于GB理论中通过转换\isce{转换}{transformation}从表层结构中推导得到的逻辑形式（Logical Form\isc{逻辑形式（LF）}\is{Logical Form (LF)}）。
