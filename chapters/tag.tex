...
目前通行的观点认为自然语言的复杂度位于上下文无关\isc{复杂类型}\is{complexity class}\isc{上下文无关文法}\is{context"=free grammar}和上下文相关\isc{上下文相关文法}\is{context"=sensitive grammar}（1-型）语言之间。这一类型被称为“弱上下文相关”（mildly context sensitive）。\isc{弱上下文相关文法}\is{mildly context"=sensitive grammar}有些TAG变体属于这一语言类型，有种观点认为它们恰好产生出自然语言中的结构，不多也不少。
...
行分析，也可以用自由序TAG（Free Order TAG，简称FO"=TAG\isc{树邻接语法（TAG）!自由语序树邻接语法（FO-TAG）}\is{TAG!Free Order (FO"=TAG)})\citep*[\page 21]{BJR91a}进行分析，二者都允许表示依存关系的边交叉。\isc{可投射性}\is{projectivity}
...
因为递归从基本树中被排除掉了，因此在导出树（局域的扩展域\isc{局部性}\is{locality}）中距离特别远的元素可以包含在同一个基本树中。

 \citet*{KKNV95a}论证了满足特定要求的HPSG语法可以转写为TAG语法。通过这种方式，我们可以得到一个计算复杂度\isc{生成能力}\is{capacity!generative}更明确的语法。HPSG语法一般都是0-型文法，而不同的TAG变体可以用来刻画从2-型语言到弱上下文相关文法\isc{弱上下文相关文法}\is{mildly context"=sensitive grammar}之间的不同语言\citep{Joshi85a-u}。 \citet*{YMTT2001a}提出了一个算法可以将FB"=LTAG语法翻译成弱上下文相关文法的HPSG文法。
