...
\isc{树邻接语法（TAG）!矢量树邻接语法（V-TAG）}\is{Tree Adjoining Grammar (TAG)!Vector (V-TAG)}\citep{Rambow94a} 
和TT-MC-TAG
\isc{树邻接语法（TAG）!树元组多成分树邻接语法（TT-MC-TAG）}\is{Tree Adjoining Grammar (TAG)!Tree Tuple MC-TAG (TT-MC-TAG)}\citep{Lichte2007a}。
...
在FTAG
{树邻接语法（TAG）!基于特征结构的树邻接语法（FTAG）|(}\is{Tree Adjoining Grammar (TAG)!Feature Structure"=Based (FTAG)|(}中，结点并不是原子的（N、NP、VP或S），而是包含有特征描写。
...
\isc{树邻接语法（TAG）基于特征结构的树邻接语法}\is{Tree Adjoining Grammar (TAG)!Feature Structure"=Based (FTAG)|)}
