...
现在，我们面对的问题是，从中枢图式向一个带有一个论元结构的词项的跳跃是怎样发生的。在Tomasello的方法中，两者之间没有停顿。中枢图式是短语模式而[Sbj TrVerb Obj]也是一个短语模式。两种模式都有 空位允许一些成分插入。在基于选择的方法中，情形是类似的：在基于选择的方法中，中枢图式的固定部分是功能符\isc{功能符}\is{functor}。 \citet{Green-Grammar-Growth}在HPSG框架中提出了一种习得理论，这种理论可以在不使用UG的情况下起作用。对于双词短语，她假设where’s是例（\mex{1}） 中一个表达的中心语并且where’s选择Robin作为其论元。
