...
}。Gibson \& Wexler又提出要为特定参数设置缺省值，缺省值可以让学习者避免有问题的情况。对于V2参数\isce{参数!V2}{parameter!V2}，他们假设缺省值\iscesub{参数}{parameter}{缺省值}{default value}是“$-$”。
...
\z
Yang讨论了pro-脱落\isce{参数!pro"-脱落|(}{parameter!pro-drop|(}和话题省略参数\iscesub{参数}{parameter}{话题省略}{topic drop}。在pro-脱落语言中（如意大利语\ilce[|(]{意大利语}{Italian}），可以省略主语；在话题省略语言（\egc 现代汉语\ilce{现代汉语}{Mandarin Chinese}）中，充当话题的主语和宾语都可以省略。Yang对比了说英语\ilce[|(]{英语}{English}和说汉语\ilce{现代汉语}{Mandarin Chinese}的儿童，发现说英语的儿童在很早的语言阶段就省略了主语和宾语。他认为造成这一现象的原因是说英语的儿童从使用汉语语法开始。
...
\zl
下面引述的涅槃乐队\isce{Nirvana}{Nirvana}的歌曲题目也来自于Valian的文章：
...
我们会针对该观点提出以下问题：儿童怎样区分他所听到的句子对应于两种语法中的哪一种呢？\isce{参数!pro-脱落|)}{parameter!pro-drop|)}
...
%\mytrans{I want it.}
\z
例（\mex{0}）中的短代词lo在定式动词之前出现，所以动词出现在句末。但是，在英语中宾语在动词之后。另外，英语输入中复合动词形式的出现频率（70\,\%）远远高于西班牙语（25\,\%）。这是因为英语中的进行体\isce{进行体}{progressive}出现频率高而且问句形式中会出现do支撑\isce{do-支撑@\emph{do}-支撑}{do-Support@\emph{do}-Support}。\ilce[|)]{西班牙语}{Spanish}
