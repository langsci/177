...
\end{sidewaysfigure}%
空的中心语需要一个不及物动词、一个形容词、一个受格宾语和一个主语。Fischt（钓鱼）的主语当然要与空中心语的主语一致。这一点没有呈现在图中。但是，可以建立这一等同性（参见\citet{HN94a}）。在这个分析中，致使语义是由空的中心语贡献的。这里使用的技巧正是在\ref{Abschnitt-Eleminierung-leerer-Elemente}中所使用的，只是方向相反：在前面的章节中，含有空的子结点的二叉结构被单分支结构所替代。在这一节中，我们用含有空的子结点的二叉结构代替单分支结构。\dotfootnote{%
