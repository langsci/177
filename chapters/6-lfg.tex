...
\pred 的规约对应于GB理论中的题元格\isce{$\theta$-栅}{theta-grid@$\theta$-grid}。
...
这两条限制合在一起对应于GB理论中的题元准则\isce{$\theta$-准则@$\theta$-准则}{theta-criterion@Theta-Criterion}（详见第\pageref{theta-Kriterium}页）\footnote{%
...
\isce{范畴!功能!Neg}{category!functional!Neg}
\isce{范畴!功能!T}{category!functional!T}
\isce{范畴!功能!Agr}{category!functional!Agr}
...
\ex
\begin{sloppypar}
   主语-映射-原则：\isce{原则!主语-映射}{principle!Subject-Mapping} 标记为[$-$o]的最优先的角色如果是a-结构中的初始成分，则它被映射为\lfgsubj，
...
他的研究表明很多分析本身是可以互相转化的：LFG中的f-结构的功能可以通过GB中的题元准则（Theta-Criterion\isce{$\theta$-理论@$\theta$-理论}{theta-theory@$\theta$-Theory}\isce{$\theta$-准则@$\theta$-准则}{theta-criterion@Theta-Criterion}）和格理论（Case Theory\isce{格}{case}\isce{格语法}{Case Theory}）进行分析处理。
LFG可以显性地区分主语和非主语。
