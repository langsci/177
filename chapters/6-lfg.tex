...
另有一些基于Kamp的语篇章表示结构 \citep{KR93a}的分析方法\citep{FR83b,FR83a}。
...
\citet{Dalrymple99a-ed}讨论了针对量化\isc{量化}\is{quantification}、修饰和其它现象的粘着语义分析。 
...
也就是说，结合语法中其它的可与之交互的规则，相应的VP变成可选的。\isc{成分序列|)}\is{constituent order|)}  
...
只有语法化的语篇功能才能在f-结构中进行表示，也就是说，那些被特定的句法机制创造出来的并且和句法其它部分相交互的部分。
