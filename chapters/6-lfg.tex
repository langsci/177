...
\pred 的规约对应于GB理论中的题元格\isc{$\theta$-栅}\is{theta-grid@$\theta$-grid}。
...
\isc{c-结构}\is{c"=structure|)}\ilce[|)]{英语}{English}
...
附加语\isc{附加语}\is{adjuncts}\ilce[|(]{英语}{English}并不被中心词选择。\textsc{adj}
...
\isc{范畴!功能!Neg}\is{category!functional!Neg}
\isc{范畴!功能!T}\is{category!functional!T}
\isc{范畴!功能!Agr}\is{category!functional!Agr}
...
\ex
\begin{sloppypar}
   主语"=映射"=原则：\isc{原则!主语-映射}\is{principle!Subject"=Mapping} 标记为[$-$o]的最优先的角色如果是a-结构中的初始成分，则它被映射为\lfgsubj，
...
\isc{中心语域!扩展|)}\is{head domain!extended|)}
...
LFG可以显性地区分主语和非主语。
在GB中则是区分外部\isc{论元!外部论元}\is{argument!external}和内部\isc{论元!内部论元}\is{argument!internal}论元（参见\citealp[\S~1.2]{Williams84a}）。
