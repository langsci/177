...
这些功能被称为可被管辖的语法功能（\emph{governable grammatical functions}）\isce{语法功能}{grammatical function!governable}。
...
在后来的研究中，
词汇映射理论（Lexical Mapping Theory）\isc{词汇映射理论（LMT）|(}\is{Lexical Mapping Theory (LMT)|(}\isce[|(]{联接}{linking}\label{page-LMT}成为语法功能的分配的主流理论
...
因为该格（与事）与特定的语义角色相关联，因而宾语的词汇格\isce{词汇格}{case!lexical}在a-结构中给出\citep*[\page 465]{ZMT85a}。
...
这些映射原则乍看很复杂，但它们在分析相关的语言现象时发挥了很大的作用，如非宾格结构\isce{非宾格动词}{verb!unaccusative}\citep{BZ90a}。
...
\rulenode{VP\\* \up~=~\down}}
\z
这里的|\isc{$\vert$}\is{$\vert$}表示析取（disjunction\isce{析取}{disjunction}），也就是说，NP既可以是相应f-结构的主语也可以是宾语。
...
他的研究表明很多分析本身是可以互相转化的：LFG中的f-结构的功能可以通过GB中的题元准则（Theta"=Criterion\isc{$\theta$-理论@$\theta$-理论}\is{theta-theory@$\theta$-Theory}\isc{$\theta$-准则@$\theta$-准则}\is{theta-criterion@Theta"=Criterion}）和格理论（Case Theory\isce{格}{case}\isce{格语法}{Case Theory}）进行分析处理。
LFG可以显性地区分主语和非主语。
...
这个特殊的论元被称为指定的论元（designated argument\isce{指派论元}{argument!designated}）。
