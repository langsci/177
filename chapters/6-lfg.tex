...
这两条限制合在一起对应于GB理论中的题元准则\isceat{theta-准则}{$\theta$-准则}{theta-criterion}{Theta-Criterion}（详见第\pageref{theta-Kriterium}页）\bracketfootnote{%
...
\begin{sloppypar}
\begin{itemize}
\item 在动词末位位置存在一个语迹（参见GB\indexgb）（详见\citealp{Choi99a-u}；\citealp[2.1.4]{Berman96a-u}） 
...
在中心词的扩展域的分析中，动词只不过是从动词短语中省略掉了。下面的VP规则的基本变种很常见：\dotfootnote{%
...
\z
\addlines
动词短语的所有组成部分都是可选的，注意括号表示可以出现也可以不出现。Kleene星号\isce{Kleene星号}{Kleene star}\isce{*}{*}表示符号可以出现任意次。这也包括出现零次的情况。和GB的分析一致，动词在动词前置的小句中属于C范畴，而并不假设范畴I存在投射（\citealp{Haider93a,Haider95b-u,Haider97a}；\citealp[IV.3]{Sternefeld2006a-u}），
这是出于对德语语言事实的考量\citep[3.2.2]{Berman2003a}。动词会从C范畴位置提供f-结构信息。
...
\item 直接在短语规则中允准（详见Berman\citeyear[2.1.3.1]{Berman96a-u}；\citeyear{Berman2003a}）
\end{itemize}
...
接下来，我将讨论 \citet[2.1.3]{Berman96a-u}提出的分析，并在一定程度上作出简化。
动词论元的格和语法功能由词汇所决定\citep[\page 22]{Berman96a-u}。
（\mex{1}）展示了德语动词verschlingen（吞食）的词汇项信息：\dotfootnote{%
...
\noindent
   \citet[2.1]{Karttunen89a-u}在分析芬兰语的时候，基于范畴语法\indexcxg 的框架提出了类似的分析。
...
\noindent
用于构建英语中前置的c-结构规则最终可以表示如（\mex{1}）所示：\dotfootnote{%
...
而相应的c-结构规则如（\mex{1}）所示：\dotfootnote{\label{fn-zp}%
...
LFG可以显性地区分主语和非主语。
在GB中则是区分外部\iscesub{论元}{argument}{外部论元}{external}和内部\iscesub{论元}{argument}{内部论元}{internal}论元（参见\citealp[1.2]{Williams84a}）。
