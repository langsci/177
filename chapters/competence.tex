...
另外， \citet[\page 53--54]{SW2015a}指出，在从话语或语境里获得充足信息前，听话者经常不会明确语义的解读。在处理话语自身或环境时经常在获得充足信息之前会让语义解释不完全赋值。他们不会过早认定某一特定意义而进入花园路径或回溯到其他语义。运用不完全赋值语义学变体的理论会恰当描述这一点。一个不完全赋值语义的例子可以见\ref{sec-MRS-wieder}。
