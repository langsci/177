...
上述那些例子都为以下方法提供了证据，该方法假设在处理语言时，来自所有现有渠道的信息都会使用，而且一旦可用就立即使用，而不是一定等到整个话语或整个短语被构建之后。实验研究因此证明，语言知识的严格模块化\isc{模块性}\is{modularity}组织一定会遭到反对。这一假设的支持者假设一个模块的输出可以充当另一模块的输入，而既有模块不用接触那一模块的内部状态或过程。例如，形态模块可以为句法提供输入然后再被语义模块处理。语言知识这一组织方式的经常被引用的证据就是所谓的花园幽径句，例如 （\mex{1}）：
