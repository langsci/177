...
在这一点上，我们可以看到上述的短语结构规则有几条不同的语义规则。按照这一方式来分析语言的理论假说叫做“规则对规则假说”（rule"=to"=rule hypothesis）\isc{规则对规则假说}\is{rule"=to"=rule hypothesis}\citep[\page 184]{Bach76a}。针对语言表达式的意义的推导过程将在\ref{Sec-GPSG-Sem}中详细展开。
...
我们不仅可以在名词短语中省略名词，在某些语境中，限定词也可以不出现。例（\mex{1}）展示了复数\isc{复数}\is{plural}的名词短语：
...
词汇项可以跟它的补足语\isc{补足语}\is{complement}组合。最后一条规则中的“*”\isc{*}\is{*}表示它后面的符号可以不受限制地重复。一个特殊的情况是补足语的缺失。在das Bild（这张图片）中， Bild（图片）没有PP担当补足语，由此变成了\nbarc。词汇项及其补足语结合的结果是X的一个新的投射层：投射层1，并由一个横杠来表示。之后，\xbarc 可以跟附加语组合。这一过程发生在\xbarc 的左边或右边。这一组合过程的结果仍然是一个\xbarc，也就是说，将一个附加语\isc{附加语}\is{adjunct}组合进来并没有改变投射层。最大投射由两个横杠表示。我们也可以将XP写成带有两个横杠的X。一个XP包括一个限定语\isc{限定语}\is{specifier}和\xbarc。在有些理论中，句子中的主语（\citealp{Haider95b-u,Haider97a}；\citealp[\S~3.2.2]{Berman2003a}）和NP中的限定词\citep[\page 210]{Chomsky70a}都是限定语。进而，形容词短语中的程度修饰词\citep[\page 210]{Chomsky70a}和介词短语中的方式指示词也被算作是限定语。
...
}详细的使用说明参考定子句文法（Definite Clause Grammar，简称DCG）\isc{定子句文法}\is{Definite Clause Grammar (DCG)}的英文维基百科条目\footnote{%
