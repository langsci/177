...
\z
%CAUTION:
这一过程叫做$\beta$-约归（$\beta$-reduction）\isceat{beta-约归}{$\beta$-约归}{beta-reduction}{$\beta$-reduction}或$\lambda$-变换（$\lambda$-conversion）\isceat{lambda-变换}{$\lambda$-变换}{lambda-conversion}{$\lambda$-conversion}。为了深入说明这一概念，我们用及物动词的例子来分析。例（\mex{1}a）中的句子具有（\mex{1}b）所表示的含义：
