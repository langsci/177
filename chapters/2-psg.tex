...
根据词的屈折形态和句法分布，我们可以判断出它们的词性。由此，例（\mex{1}）中的weil（因为）是连词\isc{连词}\is{conjunction}，而das（这）和dem（这）是冠词\isc{冠词}\is{article}，后两者也叫做限定词\isc{限定词}\is{determiner}。Buch（书）和Mann（人）是名词\isc{名词}\is{noun} ，而gibt（给）是动词\isc{动词}\is{verb}。
