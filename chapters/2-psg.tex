...
\begin{tabular}[t]{@{}r|llllll@{\hspace{1.7cm}}l@{}}
 & \multicolumn{6}{l@{}}{词和符号} & 应用的规则\\\hline
...
\end{tabular}
\z
该文法允准了如图\ref{er-das-buch-dem-mann-gibt-bin}所示的二叉树结构。
...
\addlines
（\ref{NP-Regeln}c）说明了 \nbar 有可能只带一个名词。但我们还没有讨论更为重要的规则：我们需要另一个规则来整合名词与它们所带的论元成分。如例（\mex{1}a--b）所示，这些名词有Vater（爸爸）、Sohn（儿子）和Bild（照片）这类“关系名词”（relational nouns）\isc{名词!关系名词}\is{noun!relational}。例（\mex{1}c）说明了动词与其所带论元的名词化过程：
...
\zl
图\ref{Abbildung-NP-ohne-Det}展示了相应的树形图。
...
，而不是N，我们就会遇到伪歧义（spurious ambiguities）\isc{歧义!伪歧义}\is{ambiguity!spurious}问题。例（\mex{1}）中的结构是唯一可能的分析结果。
...
\gll {}[[Kurz] nach dem Start] fiel die Klimaanlage aus.\\
	 {}\spacebr{}\spacebr{}短暂 PREP DET  起飞 坏 DET 空调 出来\\
\glt `起飞后不久，空调就停止工作了。'
\ex 
...
\ex \pbar $\to$ P NP
\zl
规则（\ref{Regel-PP-einfach}）变成了（\mex{0}d）。（\mex{0}c）中的规则证明了PP可以包括\pbar 。图\ref{Abbildung-PP}显示了应用（\mex{0}c）和（\mex{0}d）对（\mex{1}）的分析，以及在规则（\mex{0}b）和（\mex{0}d）的约束下形容词位于首位的例子：
...
与没有限定词的NP不同的是，不带有关程度或方式的介词短语并不缺少任何意义的组成部分。所以说，我们没有必要假设有一个表示量度的空成分，并对整个PP有语义贡献。由此，（\ref{Regel-PP-P}）中的规则说明了介词短语包括\pbar ，即由P和NP构成。\isc{介词|)}\is{preposition|)}
...
这种抽象的方式就叫做 \xbar 理论 （或者X"=杠理论）。该理论由\citet{Chomsky70a}提出，并由\citet{Jackendoff77}完善。这种形式的抽象规则在很多不同的理论中发挥了重要的作用。比如说：管辖与约束理论（第\ref{Kapitel-GB}章）、扩展的短语结构语法（第\ref{Kapitel-GPSG}章）和词汇功能语法（第\ref{Kapitel-LFG}章）。在中心语驱动的短语结构语法（第\ref{Kapitel-HPSG}章）中，\xbar 理论也起到了重要的作用，但是并没有采纳\xbar 范式的所有限制条件。
...
%
\hfill\mbox{}
\caption{\label{Abb-das-schoene-Bild-von-Paris}das Bild（这张图片）和das schöne Bild von Paris（这张巴黎的美丽图片）的\xbar 分析}
...
%\addlines[-1]
再者，其他的\xbar 理论与本书所讨论的理论也有不同的看法。特别是，我们将不予理会非中心语永远作为最大投射\isc{投射!最大投射}\is{projection!maximal}这一条假设。\citet{Pullum85a}和\citet{KP90a}认为，有些理论并不一定没有严格的\xbar 理论的限制多。也可以参考\ref{sec-Diskussion-X-Bar}中的讨论。
...
\item 为什么\xbar 理论不能说明没有额外假设的德语形容词短语呢？（这一问题只适合于德语母语者。）
