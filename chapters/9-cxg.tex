...
\z
构式实现为非连续形式并且附接语插入到空位中。在这种分析方式中，仍然需要解释量词以及附接语的辖域\isc{辖域}\is{scope}是如何确定的。虽然这是可能的，但是这种方式并不明显而且至今没有在任何CxG方法中实现出来。对于允许不连续成分方法的进一步论述可以参看第~\ref{sec-discontinuous-constituents-HPSG}节。\isc{成分!非连续|)}\is{constituent!discontinuous|)}\isc{动词位置|)}\is{verb position|)}
