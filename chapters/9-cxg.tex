...
} 特征框架中的其他变化（取消\localfc 和使用单个价特征）也是有问题的。但是，如果我们真的拒绝调整后的特征框架并且返回到以前使用的特征框架，那么基于符号的\sbcgc 与构式化HPSG\citep{Sag97a}就几乎是无法区分的了。\isc{局域性}\is{locality}\isc{习语|)}\is{idiom|)}
