...
{}[+\textsc{top}] $<$ X
\z
\textsc{top}表示话题化（topicalized）。正如我们在第\pageref{Seite-Topikalisierung}页提到的，前场并没有限制为话题。焦点元素与虚位成分可以在前场出现，使得特征名称并不理想。但是，有可能将它替换为其他名称，如前场（prefield）。这不会影响我们的分析。（\mex{0}）中的X表示一个任意范畴。这是一个新的X，与（\ref{gpsg-vs-regel}）中的X并无干系。
