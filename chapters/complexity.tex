...
不应该是描述语言限制理论而应该是理论包含必须适用于研究对象的限制。这是 \citet[\page 277, 280]{Chomsky81b} 所持的观点。也可以参看\citew[Section~4]{Berwick82a-u},\citew[Section~8]{KB82a-u} 对于LFG的论述以及\citew[Section~3.5]{Johnson88}对于LFG中离线句法分析能力限制（Off-Line Parsability Constraint）的论述和对于属性-值语法的总体论述。
...
%informative.
    下面我将讨论另外一个例子来说明特定语言的限制会限制语法的复杂性：\citet[Section~3.2]{GM2007a}认为Stabler提出的增加了后附接（late adjunction）和外置的最简语法\indexmg (参看第~\ref{Abschnitt-MG}节)比弱上下文有关语法的生成力更强。如果一种语法禁止从附接语位置\is{extraction!from adjuncts}提取成分(\citealp[\page46]{FG2002a})并且认同最短移位限制\isc{最短移位限制}\is{Shortest Move Constraint (SMC)}（参看第~\pageref{Fn-SMC}页脚注~\ref{Fn-SMC}），那么该语法就是弱上下文有关语法\citep[\page 178]{GM2007a}。包含最短移位限制并限制从指定语位置提取成分的语法也是弱上下文有关语法。
...
%This means that a ban on extraction from specifiers cannot hold for German. As such, it cannot be true for all languages.
    我们的语法与允许不连续成分的语法相似：因为不能将禁止提取整合到语法形式描述中，所以语法还是比描述自然语言所需语法的生成能力强。但是，真实语法中的限制——在当前研究中，是相关语言中对从指定语位置提取成分的限制——能确保相应具体语言的语法的生成能力与弱上下文有关语法相同。\is{capacity!generative|)}\is{complexity class|)}
