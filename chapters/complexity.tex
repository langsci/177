...
%informative.
    下面我将讨论另外一个例子来说明特定语言的限制会限制语法的复杂性： \citet[\S~3.2]{GM2007a}认为Stabler提出的增加了后附加（late adjunction）和外置的最简语法\indexmgc（参看\ref{Abschnitt-MG}）比弱上下文相关文法的生成力更强。如果一种语法禁止从附加语位置提取成分\isc{提取!从附加语位置}\is{extraction!from adjuncts}（\citealp[\page46]{FG2002a}）并且认同最短移位限制\isc{最短移位限制}\is{Shortest Move Constraint (SMC)}（参看第~\pageref{Fn-SMC}页脚注~\ref{Fn-SMC}），那么该语法就是弱上下文相关文法\citep[\page 178]{GM2007a}。包含最短移位限制并限制从指定语位置提取成分的语法也是弱上下文相关文法。
...
\gll {}[Von der HVA]$_i$ hielten sich [etwa 120 Leute \_$_i$] dort in ihren Gebäuden auf.\footnotemark\\
	 {}\spacebr{}来自……的 \textsc{det} HVA 容纳 \refl{} \spacebr{}大约 120 人 {} 那里 \textsc{prep} 他们的 建筑 \prt{}\\
\footnotetext{%
       《镜报》（Spiegel），1999年第3卷，第42页。
     }
...
\gll {}[Aus dem "`Englischen Theater"']$_i$ stehen [zwei Modelle \_$_i$] in den Vitrinen.\footnotemark\hspace{-3pt}\\
	 {}\spacebr{}来自 \textsc{det} \hspaceThis{"`}英语 剧场 站 \spacebr{}两个 模特 {} 在……里 \textsc{det} 包厢\\
\footnotetext{%
        《法兰克福评论》（Frankfurter Rundschau），摘自 \citew[\page 52]{deKuthy2001a}。
      }
...
\gll {}[Aus der Fraktion]$_i$ stimmten ihm [viele \_$_i$] zu darin, dass die Kaufkraft der Bürger gepäppelt werden müsse, nicht die gute Laune der Wirtschaft.\footnotemark\\
	 {}\spacebr{}来自 \textsc{det} 派系 同意 他 \spacebr{}很多 {} \prt{} 在这点上} \textsc{comp} \textsc{det} 购买.能力 \textsc{det} 市民 增加 变得 应该 \textsc{neg} \textsc{det} 好的 氛围 \textsc{det} 经济\\
\footnotetext{%
        《日报》（taz），\zhdate{2003/10/16}，第5页。%  taz Themen des Tages 282 Zeilen, ULRIKE WINKELMANN S. 5
}
...
\gll {}[Vom Erzbischof Carl Theodor Freiherr von Dalberg]$_i$ gibt es beispielsweise [ein Bild \_$_i$]
	...
	{}\spacebr{}从.\textsc{det} 大主教 Carl Theodor Freiherr 来自 Dalberg 有 \expl{} 例如 \spacebr{}一 图画 {} 在.\textsc{det} 城市.档案\\
\footnotetext{%
        《法兰克福评论》（Frankfurter Rundschau），摘自 \citew[\page 7]{deKuthy2002a}。
}
...
\gll {}[Gegen die wegen Ehebruchs zum Tod durch Steinigen verurteilte Amina Lawal]$_i$ hat gestern in Nigeria
	...
	{}\spacebr{}对 \textsc{det} 因为.\textsc{prep} 私通 为了.\textsc{det} 死亡 通过 石刑 判处 Amina Lawal \textsc{aux} 昨天 在 Nigeria \spacebr{}\textsc{det} 第二 上诉.过程 {} 开始\\
\footnotetext{%
        《日报》（taz），\zhdate{2003/08/28}，第2页。
    }
...
\gll {}[Gegen                  diese        Kahlschlagspolitik]$_i$ finden derzeit bundesweit                     [Proteste und Streiks \_$_i$ ] statt.\footnotemark\\
     {}\spacebr{}对 \textsc{det} 秃的.打.政治    发生   现在 \spacebr{}全国 \spacebr{}抗议 和 罢工 {} {} \prt{}\\
\footnotetext{%
        《罢工号召》（Streikaufruf），不来梅大学（Universität Bremen），\zhdate{2003/12/03}，第1页。
    }
...
%This means that a ban on extraction from specifiers cannot hold for German. As such, it cannot be true for all languages.
    我们的这个情况与允许不连续成分的语法相似：因为不能将禁止提取整合到语法形式描述中，所以语法形式还是比描述自然语言所需语法的生成能力强。但是，真实语法中的限制——在当前研究中，是相关语言中对从指定语位置提取成分的限制——能确保相应具体语言语法的生成能力与弱上下文相关文法相同。\isc{能力!生成能力|)}\is{capacity!generative|)}\isc{复杂类型|)}\is{complexity class|)}
