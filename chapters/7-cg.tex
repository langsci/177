...
在本书所讨论的所有方法中，范畴语法
\isc{范畴语法|(}\is{Categorial Grammar (CG)|(}是第二古老的方法。
...
一些处理范畴语法的系统增加了概率
\isc{统计学}\is{statistics}模块，所以处理结果的鲁棒性很高\citep*{OB97a,CHS2002a-u}。
一些系统采用了从（标注）语料中抽取词汇项的方法，\citet{Briscoe2000a}和\citet{Villavicencio2002a}使用了一些统计信息，
这些信息来自于他们的基于普遍语法
\isc{普遍语法}\is{Universal Grammar (UG)}的语言获取模型
\isc{语言习得}\is{language acquisition}。
...
在
\isc{价|(}\is{valence|(}范畴语法中，复杂范畴替换了GPSG\indexgpsgc 中的\textsc{subcat}特征来确保一个中心词只能使用适合的句法规则。
一些短语结构语法规则
...
称X/Y为\emph{函子}
\isc{函子}\is{functor}，而Y为函子的\emph{变元}
\isc{论元/变元（CG）}\is{argument}。
...
在范畴语法中，被动
\isc{被动／被动式|(}\is{passive|(}的分析采用了词汇规则（\citealp[\page412]{Dowty78a}；\citealp[\S~3.4]{Dowty2003a}）。
...
或假设新的规则，如（\mex{0}b）。
\isc{被动／被动式|)}\is{passive|)}\todostefan{NW:
...
\isc{前向组构}\is{composition!forward}/后向组构（backward composition）
\isc{后向组构}\is{composition!backward}。
接下来我们介绍这几个新增加的运算操作。
...
话题化
\isc{话题化}\is{topicalization} ($\uparrow$
\isc{$\uparrow$}
\is{$\uparrow$}):\\
X $\Rightarrow$ st/(s/X)\\
...
针对n=2的话题化规则
\isc{话题化}\is{topicalization} ($\uparrow\uparrow$
\isc{$\uparrow\uparrow$}
\is{$\uparrow\uparrow$}):\\
...
\gll Here's the minister [[in [the middle [of [whose sermon]]]] the dog barked]. \\
     这里是 DET 牧师 PREP DET 中间 PREP CONJ 布道 DET 狗 叫 \\
...
\gll Reports [the height of the lettering on the covers of which] the government prescribes should be abolished.\\
     报告 DET 高度 PREP DET 印字 PREP DET 封面 CONJ DET 政府 规定 应该 被 废除 \\
...
ART 报告 DET 封面 PREP CONJ Keats 期待 CONJ Chapman AUX 设计\\
\glt `一个Keats（以为Chapman）会设计的报告的封面'
\ex 
\gll a subject on which Keats (expects that Chapman) will speak\\
ART 主题 PREP CONJ Keats 期待 CONJ Chapman AUX 发言\\
...
%TODO: 书中有的错误，少了一个[
\ea
\gll ~[reports [the height of the lettering on the covers of]\sub{np/np}
which]\sub{(n$\backslash$n)/(s/np)} the government prescribes\\
报告 DET 高度 PREP DET 字体 PREP DET 封面 PREP CONJ DET 政府 规定\\
...
\gll the contract [the loss of which after so much wrangling] John would finally have to pay for \\
     DET 合同 DET 损失 PREP CONJ PREP 如此 多 争论 John 会 终于 AUX INF 赔偿 PREP\\
