...
一些短语结构语法规则\isce{短语结构语法}{phrase structure grammar}可以用复杂范畴来替换：\isc{/|(}\is{/|(} 
...
话题化
\isce{话题化}{topicalization} ($\uparrow$\isc{$\uparrow$}\is{$\uparrow$}):\\
X $\Rightarrow$ st/(s/X)\\
...
针对n=2的话题化规则
\isce{话题化}{topicalization} ($\uparrow\uparrow$\isc{$\uparrow\uparrow$}\is{$\uparrow\uparrow$}):\\
