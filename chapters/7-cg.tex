...
\end{itemize}
另外，\citet*[\page 15]{BCPW2007a}提到过一个针对古典阿拉伯语\il{Arabic}的实现。
...
图\vref{abb-cg-transitives-Verb}展示了一个及物动词的词条是如何和它的宾语组合的。
\begin{figure}
\centerline{%
\deriv{2}{
%\begin{tabular}{@{}cc@{}}
...
John & \passivepst{}            & \textrm{触碰}      & \textsc{prep}         & Mary.\\
...
如果我们假定第一个出现的动词是函子，那就有可能捕捉到标句词（complementizers）
\isc{标句词}\is{complementizer}和前置位置上的动词之间的这种平行性\citep{Hoehle97a}：出现在前置位置的定式动词
和标补语之间的差别仅仅在于其需要动词语迹的一个投射，而补语化标记成分需要显式动词的投射。
\eal
\ex 
...
通过类型提升
\isc{类型提升|(}\is{type raising|(}规则，范畴np可以变形为范畴(s/(s\bs np))。
...
这对于分析并列结构是有好处的（参见\ref{Abschnitt-Koordination}）。此外， \citet{Steedman91a}讨论认为韵律数据
...
\mytrans{这有一个那条狗在他布道过程中吠叫的牧师。}\footnote{\citew[\page 212]{ps2}。}
\ex 
...
\mytrans{那些封面上的印字高度被政府规定的报告应该被废弃。}\label{Ross-reports}\footnote{\citew[\page 109]{Ross67}。\nocite{Ross86a-u}}
\zl
...
一 报告 \textsc{det} 封面 \textsc{prep} \textsc{rel} Keats \hspaceThis{(}期待 \textsc{comp} Chapman 将 设计\\
