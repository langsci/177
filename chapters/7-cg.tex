...
一 报告 \textsc{det} 封面 \textsc{prep} \textsc{rel} Keats \hspaceThis{(}期待 \textsc{conj} Chapman \textsc{aux} 设计\\
\glt `一个Keats（以为Chapman）会设计的报告的封面'
\ex 
\gll a subject on which Keats (expects that Chapman) will speak\\
一 主题 \textsc{prep} \textsc{rel} Keats \hspaceThis{(}期待 \textsc{conj} Chapman \textsc{aux} 发言\\
