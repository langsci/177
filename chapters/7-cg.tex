...
除了组合范畴语法之外，也有一些范畴语法的其它变体，如类型逻辑范畴语法\citep*{Morrill94a-u,Dowty97a-u,Moortgat2011a-u}。
...
我们需要扩展（\ref{Lexikonregel-Passiv-CG}）的规则，并引入新的技术分析手段\footnote{%
  Baldridge\aimention{Jason Baldridge}（p.\,M.\ 2010）建议在被动的词汇规则里使用正则表达式。
 }
或假设新的规则，如（\mex{0}b）。
...
这对于分析并列结构是有好处的（参见\ref{Abschnitt-Koordination}）。此外， \citet{Steedman91a}讨论认为韵律数据\isc{韵律}\is{prosody}也支持把这些字符串处理成组成成分。
