...
在德语中，VP和IP的中心语位置位于它们的补足语右边，\vnullc 和\inullc 属于句子的右边界。主语和其他成分（补足语和附加语）都出现在\vnullc 和\inullc 的左边，并且构成中场。一般认为，德语——至少从D"=结构来看——是一个SOV类型的语言（=~这种语言的基本语序是主语--宾语--动词）。有关德语作为一种SOV型语言的研究几乎跟转换语法一样古老。最早是由 \citet*[\page34]{Bierwisch63}提出的。\footnote{%
