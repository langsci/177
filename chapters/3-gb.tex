...
%} 
针对转换语法的早期版本的另一个批评是，由于缺乏限制，转换规则之间的相互关系是不够清楚的。而且，有些转换规则会删除语言材料这点也是有问题的（参看\citealp[\S~3.1.4]{Klenk2003a}）。基于上述原因，新的理论被提出来，如管辖与约束理论\citep{Chomsky81a}。在这个模型下，语法规则的形式受到严格的限制（请看\ref{Abschnitt-X-Bar}）。通过转换而移动的元素仍然在他们原始的位置上，这样就使得原始位置是可获得的，也就为语义解读提供了可能。也有更具有普遍性的原则来限制转换的\isc{转换|)}\is{transformation|)}过程。
