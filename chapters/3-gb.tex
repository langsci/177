...
GB理论认为，所有的规则都应该采用\ref{sec-xbar}中讨论的\xbarc 这种形式。在其他理论中，采用\xbarc 形式的规则跟不采用这种形式的规则一起使用。如果按照严格的\xbartc，就会得到短语的向心性（endocentricity）\isc{向心构式}\is{endocentricity}假说：每个短语都有一个中心语，而且每个中心语都是短语的一部分（用更为技术化的语言来说：每个中心语都投射\isc{投射}\is{projection}到一个短语上）。
