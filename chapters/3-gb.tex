...
另外，还有一致格（agreeing case）\isc{格!一致格}\is{case!agreement} （请看第\pageref{page-Kasuskongruenz}页）和语义格（semantic case）。一致格一般出现在谓词中。这个格属性也随着相应的结构发生变化，但是这个变化是由于先行语发生格变而造成的。语义格与某些短语的功能具有相关性（\egc 第四格时间状语）。进而，与宾语的词汇格类似的是，语义格并不受到句法环境的影响而发生变化。对于这一小节将讨论到的被动的分析而言，只有结构格和词汇格是相关的。
}
