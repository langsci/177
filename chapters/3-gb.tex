...
\end{figure}%
%
被动在形态上要求主语不出现，并且有结构化的宾格。主动句中的宾格宾语在被动式中仅会在它的原始生成位置获得一个语义角色，但是它不能得到格。所以，它就必须移动到可以指派格的位置上\citep[\page 124]{Chomsky81a}。图\vref{Abb-GB-Passiv} 说明了对于例（\mex{0}b）而言，这一过程是如何进行的。
