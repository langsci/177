...
这一节说明本章的主要内容。在\ref{Abschnitt-Transformationen}，我将介绍转换的概念。\ref{Abschnitt-GB-Paramater}介绍有关语言习得假设的背景信息，这些信息在一定程度上构成了转换理论。
...
\end{figure}%
S"=结构包括语音形式（Phonetic Form，PF）\isc{语音形式（PF）}\is{Phonetic Form (PF)} 和逻辑形式（Logical Form，LF）\isc{逻辑形式（LF）}\is{Logical Form (LF)}。这一模型被称为T模型（T-model），或Y模型（or Y-model），这是因为D"=结构、S"=结构、PF和LF从上到下构成了一个T（或Y）型。我们将详细讲解其中每一个成分。
