...
\end{figure}%
S"=结构演化出语音形式（Phonetic Form，PF）\isc{语音形式（PF）}\is{Phonetic Form (PF)} 和逻辑形式（Logical Form，LF）\isc{逻辑形式（LF）}\is{Logical Form (LF)}。这一模型被称为T模型（T-model），或Y模型（or Y-model），这是因为D"=结构、S"=结构、PF和LF从上到下构成了一个倒T（或Y）型。我们将详细讲解其中每一个成分。
