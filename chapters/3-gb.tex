...
\end{figure}%
决定短语移动的关键因素是句子的信息结构（information structure）\isc{信息结构}\is{information structure}。具体来说，与前述和其他已知信息相联系的内容放在最左端（最好在前场），而新信息一般出现在右边。在陈述句中前置到前场的过程一般叫做话题化（topicalization）\label{Seite-Topikalisierung}\isc{话题化}\is{topicalization}。但是，这一术语不够准确，因为焦点\isc{焦点}\is{focus}（非正式定义：所要表达的成分）也可以出现在前场。进一步说，虚指代词\isc{代词!虚指代词}\is{pronoun!expletive}可以出现在那里，而这些成分是无指的，所以它们并不能联系到前述的和已知的信息。据此，虚指成分永远不能作话题。
