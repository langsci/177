...
\noindent
有些观点认为语迹会带来诸如对于句子有不同解读的问题，事实上，这些问题是不存在的（请看\citealp[\page 146]{Kiss2001a}和\citealp[\S~2.6]{Fanselow2001a}）。以（\mex{1}）的例子来说，可以将mindestens einem Verleger（至少一个出版商）在\_$_i$位置上进行解释，这样就可以得到后面的解读，即fast jedes Gedicht（几乎每一首诗）辖域涵盖了mindestens einem Verleger（至少一个出版商）。但是，这样的解释是不存在的。
...
Yatsushiro\aimention{Kazuko Yatsushiro}。他们提出了这样一个分析，其中第一步是将宾格宾语提到主语的前面。然后，与格宾语放在它的前面，接下来，第三步移位是，宾语再次移动。最后一步既可以构成S"=结构\footnote{%
