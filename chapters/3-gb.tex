...
\zl
值得一提的是，辖域关系并不受到动词位置的影响。如果我们假设动词二位的句子具有如（\mex{0}）中的底层结构，那么这一事实就不需要再深入解释了。（\mex{1}）显示了（\mex{0}）推导出的S"=结构：
...
\noindent
在简单介绍了动词末位语序的分析之后，我将详细说明德语的CP/IP分析。\cnullc 对应句子的左边界，并且可以按照两个不同的方式来填充：英语中，在由连词引导的从句中，从句连词（标补语\isc{标补语}\is{complementizer}）占据了\cnullc 的位置。动词还是位于句子的右边界，如例（\mex{1}）所示。
