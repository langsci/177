...
谓词及其论元之间的关系是由词项决定的，这点在语义解读的所有表征层面上都是可及的。基于这一原因，移动成分的原本位置被标记为语迹（trace）。这就意味着，前置的gibt（给）的原始位置在例（\mex{1}b）中有所标示。相应的标记叫做语迹（trace）\isc{语迹}\is{trace}或空位（gap）\isc{空位}\is{gap}。这类空范畴乍看起来难以理解，但是我们早在\ref{sec-psg-np} （第\pageref{np-epsilon}页）就介绍了名词结构中的空语类。
