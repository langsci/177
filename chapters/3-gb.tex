...
针对乔姆斯基式理论的不同应用通常被归入到生成语法（Generative Grammar）\index{Generative Grammar}的名头下面。这一术语来源于这样的事实，乔姆斯基提出的短语结构语法和其扩充的理论框架可以生成合乎语法的表达式（请看第\pageref{Seite-generiert}页）。正是这样生成的一组句子在形式上构成了一种语言，我们可以通过检查一个特定的句子是否是由给定的文法生成的句子集合中的一部分来判断该句子是否属于该语言。从这个意义来看，简单的短语结构语法与相应的形式化理论、广义短语结构语法（GPSG）、LFG、HPSG和构式语法（Construction
