...
\noindent
有些观点认为语迹会带来诸如对于句子有不同解读的问题，事实上，这些问题是不存在的（请看\citealp[\page 146]{Kiss2001a}和\citealp[\S~2.6]{Fanselow2001a}）。以（\mex{1}）的例子来说，可以将mindestens einem Verleger（至少一个出版商）这句话在\_$_i$位置上进行解释，这样就可以得到后面的解读，即fast jedes Gedicht（几乎每一首诗）辖域涵盖了mindestens einem Verleger（至少一个出版商）。但是，这样的解释是不存在的。
