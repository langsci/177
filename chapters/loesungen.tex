...
\item 列表\is{list}可以通过包括开头和其余部分列表的递归结构来描写。其余成分可以是一个非空列表（\type{ne\_list}）或空列表（\type{e\_list}）。列表\relliste{ \type{a}, \type{b}, \type{c} } 可以按照下面这样表示：
...
\z
\item 如果我们通过两个额外特征来扩展（\mex{0}）的数据结构，就有可能不用\emph{append}.。关键词是\emph{difference list}（“差异表”）\is{list!difference}。差异表包括一个列表和指向这个列表末端的指针。
