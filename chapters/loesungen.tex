...
\label{avm-max-lacht}
%\end{figure}%
\item 对于（\mex{1}）中的差别的分析需要捕捉到这样的事实，形容词的格需要与名词的格保持一致。在（\mex{1}a）中，我们使用了\emph{interessant}（有趣的）的属格形式，而（\mex{1}b）包含的形式是与属格的单数不相容的形式。
...
对于名词短语内部的一致关系的改进，请参考\citew[Abschnitt~13.2]{MuellerLehrbuch1}。
