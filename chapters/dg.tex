...
上图描述了中心语与各自的从属词之间的依存关系。这可以更形式化地表示为一条$n$-ary规则\label{page-rule-format-dg}，该规则与第\ref{Kapitel-PSG}章讨论的短语结构规则十分类似（\citealp[\page 305]{Gaifman65a}；\citealp[\page 513]{Hays64a-u}；\citealp[\page 61]{Baumgaertner70a}；\citealp[4.1]{Heringer96a-u}）。比如说，Baumgärtner提出了（\mex{1}）中的一般规则格式：
...
依存\isce[|(]{被动}{passive}语法是一个基于词汇的理论，而且价是其核心概念。基于这个原因，采用基于词汇的方法来分析被动就不足为奇了。也就是说，我们假定有一个被动分词，它具有与主动动词不同的配价需求（\citealp[第12章]{Hudson90a-u}；\citealp[10.3]{Eroms2000a}；\citealp[\page 53--54]{Engel2014a}）。
...
%For a discussion of Groß \& Osborne's account of (\ref{ex-gestern-hat-sich-der-spieler-verletzt}) see page~\pageref{fig-gestern-hat-sich-der-spieler-%verletzt-dg-rising}.
 ...
 \citet{Hudson90a-u}、 \citet[5.3]{Eroms2000a}和 \citet{GO2009a}认为主语依存于助动词，而不是主动词。
...
在\isce[|(]{长距离依存}{long-distance dependency}依存语法中有许多方法来分析非局部依存关系。最简单的方法我们已经在前面的章节中看到了。许多分析只关注依存关系，并且认为位于第二位的动词只是一种可能的语序线性化的方式（\citealp[9.6.2]{Eroms2000a}；\citealp{GO2009a}）。图\vref{fig-diesen-mann-kennt-jeder-dg}展示了针对例（\mex{1}）的分析：
...
\end{figure}%%
这张图与我们之前看到的很多图不同，它不具有可投射性\isce{可投射性}{projectivity}。这意味着有交叉：对于wen（谁）的V\sub{prt}和N之间的联接与联接glaubst（认为）和du（你）以及他们的范畴符号的连线有交叉。根据设想的依存语法的版本，这可以被看作是一个问题，或者不是。让我们来看一下两个选项：如果图\ref{fig-wen-glaubst-du-dass-dg}中给出的类型的非联系性被允许用在Heringer和Eroms的语法中（\citealp[\page 261]{Heringer96a-u}；\citealp[9.6.2]{Eroms2000a}），\footnote{%
...
正如 \citet[\page 77]{Weber97a}指出的，这个范畴不是没有问题的：根据什么说Angst（害怕）是一个实体呢？为什么glauben（认为）是一个具体的过程？也参见 \citet[3.4]{Klein71a-u}关于schlagen（击打）和Schlag（击打）以及类似例子的讨论。即使我们认为Schlag通过转用而从\stem{schlag}的具体过程生成到范畴O，这样的O表示实体的假设也是有问题的。
}
...
\end{itemize}
关于\tes 的转用规则和短语成分规则的关系的进一步讨论参见 \citet[4.9.1--4.9.2]{KO2015a}。Kahane
...
\citet{KP95a}在\hpsgc 的框架下展开了外置的分析。也参见 \citew[13.3]{Mueller99a}。我在下一节将讨论HPSG中这种线性化分析的基本机制。
...
%
 \citet[4.E]{Hudson80a}讨论了（\mex{1}）中所示的无中心语的结构：
