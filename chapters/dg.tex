...
%    the book slowly.}}
\end{figure}%
依存关系的标注使用了 \citet{Engel77}提出来的描写不同依存关系的技术手段：附加语用一条从附加语结点向上的额外的线来表示（也请参阅\citealp{Eroms2000a}）。另一种区分论元/附加语，或者行动元/场景元的方法是作为论元和附加语的地位的明确的区别。所以我们可以使用针对附加语和论元的明确标记，就像在前面标记语法功能一样。德语语法和配价辞典经常使用标签E和A来分别表示Ergänzung和Angabe。
