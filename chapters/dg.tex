...
\end{figure}%%
这张图与我们之前看到的很多图不同，它不具有可投射性\isce{可投射性}{projectivity}。这意味着有交叉：对于wen（谁）的V\sub{prt}和N之间的联接与联接glaubst（认为）和du（你）以及他们的范畴符号的连线有交叉。根据设想的依存语法的版本，这可以被看作是一个问题，或者不是。让我们来看一下两个选项：如果图\ref{fig-wen-glaubst-du-dass-dg}中给出的类型的非联系性被允许用在Heringer和Eroms的语法中（\citealp[\page 261]{Heringer96a-u}；\citealp[第9.6.2节]{Eroms2000a}），\footnote{%
