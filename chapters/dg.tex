...
上图描述了中心语与各自的从属词之间的依存关系。这可以更形式化地表示为一条$n$-ary规则\label{page-rule-format-dg}，该规则与第\ref{Kapitel-PSG}章讨论的短语结构规则十分类似（\citealp[\page 305]{Gaifman65a}；\citealp[\page 513]{Hays64a-u}；\citealp[\page 61]{Baumgaertner70a}；\citealp[第4.1节]{Heringer96a-u}）。比如说，Baumgärtner提出了（\mex{1}）中的一般规则格式：
...
Hudson的词语法也是明晰表示的，正如下面所示，它与HPSG理论也是十分相似的。图\vref{fig-wg-small-children-are-playing-outside}中的表示是图\vref{fig-wg-small-children-are-playing-outside-abbreviated}表示的简缩版的详细描写。
