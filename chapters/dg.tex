...
Stelle) [\ldots]''
  ...
  \citet[\page 81]{Engel70a}针对\vfc 中只有一个成分的说法提出了反例。相关的例子将在第\ref{sec-dg-multiple-frontings}节讨论。
...
%\caption{\label{dg-alleged-murderer}Analysis that would result if one considered all scope-bearing adjuncts
 % to be heads}
\end{figure}%
这个结构可能有非投射性的其他问题。\isc{投射性}\is{投射性} Eroms确实对限定词进行了不同的处理，所以这个非可投射性的类型对他来说不是一个问题。但是，否定的中心语分析会得到德语的所谓一致结构的非可投射性。例（\mex{1}）中的句子有两种解读：在第一种解读中，否定的辖域包括singen（唱歌），而在第二种解读中包括singen darf（唱歌允许）。
