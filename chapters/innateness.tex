...
没有说明词之间是否具有一个具体的常规关系（例如，连接被动分词和完成时分词的词汇规则）。此外，没有说明表达式的意义是如何表示的（它们是按照构式语法所说的整体含义还是从词汇中投射的？）。还有跟理论语言学（请看第\ref{Abschnitt-Phrasal-Lexikalisch}章）有关的问题，并且不能直接从词的统计分布以及由其计算出的结构中推导出来（关于这点的更多内容请看\ref{Abschnitt-U-Dop-phrasal}）。\isc{助动词倒装|)}\is{auxiliary inversion|)}
