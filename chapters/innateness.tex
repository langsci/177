...
通常认为\isc{X-理论@\xbar-理论|(}\is{X-theory@\xbar-theory|(}，所有语言都有对应于\xbarc 模式的句法结构（见\ref{sec-xbar}）（\citealp[\page
...
在乔姆斯基式理论的后期，也有一些看起来对应于语法功能的功能性结点（AgrS\isc{范畴!功能!AgrS}\is{category!functional!AgrS},
AgrO\isc{范畴!功能!AgrO}\is{category!functional!AgrO}, AgrIO\isc{范畴!功能!AgrIO}\is{category!functional!AgrIO}，参见第\pageref{Seite-AgrO}页）。但是， \citet[\S~4.10.1]{Chomsky95a-u}指出，这些功能性范畴只是出于理论内部的原因而提出的，而且应该被排除在普遍语法的范畴之外。参见 \citew{Haider97a}和 \citew[\page 509--510]{Sternefeld2006a-u}对德语的描写，其中无功能性的投射无法在该语言中被驱动。
...
\z
原则C规定了有指的表达式的指称内容。根据原则C，表达式A$_1$不能指称表达式A$_2$，如果A$_2$c"=统制\isc{c-统制}\is{c"=command} A$_1$的话。c"=统制被界定为话语的结构。c"=统制有许多不同的定义，其中一个简单的版本是：如果在短语成分结构中有一条路径从A向上直到下一个结点，然后只能往下到B，那么A c"=统制B。
...
%}
再有，像\mbox{Infl}\isc{范畴!功能!I}\is{category!functional!I}（屈折变化）和Comp\isc{范畴!功能!C}\is{category!functional!C} （标补语）的特殊词类在构建所谓的普遍性的原则时也被提及了（Baltin \citeyear[\page
...
通常\isc{习得!习得速度|(}\is{acquisition!speed|(}认为，儿童学习语言异常快，而这只能是因为他们已经掌握了不需要习得的关于语言的知识（如\citet[\page 144]{Chomsky76c-u}；\citet[\page
...
此外，跟第二语言习得\iscesub{习得}{acquisition}{第二语言习得}{second language}相比，第一语言习得是毫不费力且迅速的这个说法是错误的，正如 \citet[\page 9]{Klein86a-u}的估算所显示的：如果我们假设儿童每天听五个小时的语言会话（保守估计），那么在他们生命中的头五年中，他们有9100小时的语言训练。但是到五岁的时候，他们仍没有习得所有的复杂结构。相比较而言，第二语言学习者，假设有必要的动机，可以在为期六周每天十二个小时（总共500小时）的集训课上学会一门语言的语法。\isc{习得!习得速度|)}\is{acquisition!speed|)}
...
有\isce[|(]{威廉综合症}{Williams Syndrome}一些人智商（IQ\isc{IQ}\is{IQ}）特别低，但是能说出合法的句子。这些人当中有的人有威廉综合症（关于威廉综合症患者能力的讨论，参见 \citew*{BLJLG2000a}）。 \citet{Yamada81a}将这些案例作为证据来证明存在一个独立于其他智力的语法模型。
...
\isc{FoxP2|)}\is{FoxP2|)}\isce[|)]{基因}{gene}
...
在语料库数据和貌似真实的构造出的例子的帮助下， \citet{Pullum96a}证明了这个观点明显是错误的。
  \citet{Pullum96a}在《华尔街日报》中找到了例子，并且 \citet{PS2002a}更为细致地讨论了相关的例子，再加上CHILDES语料库\isc{CHILDES语料库}\is{CHILDES}中的例子，一同说明不只成年人能造出相关类型的句子，而且它们也出现在儿童的输入中。\footnote{%
...
第二，在构造他们的刺激贫乏论时，Legate和Yang认为天赋的语言知识（pro"=脱落参数\isc{参数!pro-脱落|)}\is{parameter!pro"=drop|)}）是存在的。
...
Bod将这个过程用到了成人语言的语料库（英语\ilce{英语}{English}、德语\ilce{德语}{German}和汉语\ilce{现代汉语}{Mandarin Chinese}）上，也用到了CHILDES语料库\isc{CHILDES语料库}\is{CHILDES}的Eve语料上，他这样做是为了考察类比构造\isce{类比}{analogy}是否构成了人类语言习得的模型\isce{习得}{acquisition}。他可以证明，我们上面展示的例子也适用于大规模的自然语言的语料库：尽管在Eve语料中没有跨越复杂NP的助动词移位的例子，有可能通过类比来学会复杂NP内的助动词是不能前置的。
