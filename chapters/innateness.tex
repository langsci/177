...
\mytrans{她大喊大叫。}
\zl
 \citet[\page 352]{AG2008a}指出，将这些从句看作是附加语的分析是不合理的，因为他们只跟非常有限的动词类别有关，即言说类动词和认为类动词。这一属性是论元的属性\isc{论元}\is{argument}，不是附加语的属性\isc{附加语}\is{adjunct}。像地点修饰语这类附加语可以适用于很多动词类。而且，如果（\mex{0}b）中的句子论元被省略，它的含义也变了：而（\mex{0}a）要求交流一些信息，而（\mex{0}b）并不需要。也可以像（\mex{1}）那样用NP来替代句子论元，这些情况一定不希望被处理为状语。
