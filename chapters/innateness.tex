...
\zl
根据\xbarc 模式，auf seinen Sohn必须与stolze相组合，而且只有这样才能得到与它的限定语相组合的\abarc 投射（参见图\vref{Abbildung-AP}中英语形容词性短语的结构）。这样就只可能推导出诸如（\mex{0}b）或（\mex{0}c）的语序。而这两个语序在德语中都是不可能的。如果我们假设德语跟英语完全一致，而且基于某种原因，形容词的补足语必须移到左边的话，这样才有可能拯救\xbarc 模式。如果我们允许这种补救措施，那么当然任何一种语言都可以用\xbarc 模式来进行描写。结果是我们必须针对许多语言提出巨量的移位规则，而这从心理语言学的角度来看是异常复杂和困难的。请看第\ref{Abschnitt-Diskussion-Performanz}章有关与语言运用相适应的语法的讨论。 
