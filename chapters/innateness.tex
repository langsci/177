...
\citet[\page xxv]{BK82a}、 \citet[\page 236--237]{Pinker94a}\isce[|(]{主语}{subject}\isce[|(]{宾语}{object}\isce[|(]{语法功能}{grammatical function}、 \citet[\page 349]{Baker2003b}和其他人认为所有的语言都有主语和宾语。为了说明这个观点到底意味着什么，我们必须探究这些术语。对于大多数的欧洲语言来说，很容易说明主语和宾语是什么（参见\ref{Abschnitt-GF}）；但是，并不是所有的语言都是这样的，或者说在有些语言中用这些术语完全没有意义（\citealp[\S~4]{Croft2001a}; \citealp[\S~4]{EL2009a}）。
...
AgrO\iscesubsub{范畴}{category}{功能}{functional}{AgrO}{AgrO}, AgrIO\iscesubsub{范畴}{category}{功能}{functional}{AgrIO}{AgrIO}，参见第\pageref{Seite-AgrO}页）。但是， \citet[\S~4.10.1]{Chomsky95a-u}指出，这些功能性范畴只是出于理论内部的原因而提出的，而且应该被排除在普遍语法的范畴之外。参见 \citew{Haider97a}和 \citew[\page 509--510]{Sternefeld2006a-u}对德语的描写，其中无功能性的投射无法在该语言中被驱动。
...
正如前面提到的， GB中没有语法功能，但是有对应于语法功能的结构位置。对应于主语的位置是IP的限定语。EPP说明了在SpecIP中必须有一个元素。如果我们假定这个原则具有语言共性，那么每一种语言都必须在这个位置有一个元素。正如我们看到的，这个普遍性假设是有反例的：德语。德语有无人称被动句（\mex{1}a），其中有无主语的动词（\mex{1}b、c）和形容词（\mex{1}d--f）。\dotfootnote{%
	更多有关德语中无主语动词的讨论，参见 \citew[\S~6.2.1, 6.5]{Haider93a}、 \citew{Fanselow2000b}、
...
如果我们允准这样的机制，当然就很容易维持许多普遍性的假设：我们假设一种语言X具有属性Y，然后假设结构性的成分是看不见而且没有意义的。这种分析只能用在理论内部以取得理论的统一性\isce{统一性}{uniformity}（参见\citet[\S~2.1.2]{CJ2005a}）。\dotfootnote{%
...
综上所述，约束理论还有一些尚未解决的问题。英语中原则A--C的\hpsgc 理论变体不能用于德语\citep[\S~20]{Mueller99a}。在LFG理论框架下， \citet{Dalrymple93a}提出了一个约束理论的变体，其中代词性表达的约束属性在词汇中决定。按照这个方法，就可以描述具体语言的代词属性了。\isce[|)]{约束理论}{Binding Theory}
...
在\isce[|(]{邻接}{subjacency}前面章节中讨论的长距离依存受限于某种条件。例如，在英语中，属于名词短语的句子中的任何成分都无法被提取出来。 \citet[\page 70]{Ross67a}把这个限制称为复杂NP限制（Complex NP Constraint\isce{复杂NP限制}{Complex NP Constraint}）。在后面的研究中， \citet[\S~5.1.2]{Ross67a}还构建了右界限制（Right Roof Constraint\isce{右界限制}{Right Roof Constraint}）
...
参见 \citew[\page 211]{Mueller99a}和Müller (\citeyear{Mueller2004d}；\citeyear[\S~3]{Mueller2007c})。相关的荷兰语\ilce{荷兰语}{Dutch}的例子，参见 \citew[\page 52]{Koster78b-u}。
}
...
在\isce[|(]{提取}{extraction}特定条件下，有些成分是不能左向移位的\citep{Ross67a}。这些成分被称为提取的孤岛\iscesub{提取}{extraction}{提取的孤岛}{island}。 \citet[\S~4.1]{Ross67a}构建了复杂NP限制\isce{复杂NP限制}{Complex NP Constraint} （CNPC），该限制表明复杂名词短语是无法提取的。比如说在一个名词短语内部的关系小句的提取就是不可行的： 
...
和其中的参考文献）。由于复杂NP限制这个条件被整合进邻接原则中，相应地邻接原则并不具有普遍适用性，除非有人认为在分析的语言中，NP不是一个约束结点。然而，确实大部分语言不允许从复杂名词短语中进行提取。Hawkins基于处理难度来\isce{语言运用}{performance}分析与之相关的结构（\S~4.1）。他解释了允许这类提取的语言和不允许这类提取的语言之间的区别，这与各自语言中结构的不同处理容量有关，这些结构源于与其他语法属性的提取的互动，如动词位置和其他约定的语法结构（\S~4.2）。
...
\mytrans{她意识到他离开了。}
\zl
从这类句子中提取的不可能性可以通过假定两个约束结点有交叉来解释，这种交叉被认为是不可能的（有关结构的孤岛性质，参见\citet[\S~4]{KK70a}）。这个分析预测了从叙实动词的补足语从句中提取跟在明显的NP论元中提取一样不可行，因为这两个结构是一样的。
...
和 \citet{vanValin98a}一样， \citet[\S~7.2]{Goldberg2006a}认为，空位一定属于可以潜在构成话语的焦点\isce{焦点}{focus}的那部分（参见 \citew{Cook2001a}、 \citew{deKuthy2002a}，有关德语的参见
...
\zl
Goldberg指出，属于背景信息的短语成分是孤岛，即背景构式是孤岛（Backgrounded constructions are islands，简称BCI）。 \citet{AG2008a}对这个语义/""语用分析进行了实验测试，并且将之与纯粹的句法方法进行了比较。他们能够证明信息结构\isce{信息结构}{information structure}属性对于成分的提取力发挥了重要的作用。 \citet[\S~3.H]{Erteschik73a-u}与 \citet[\page 375]{AG2008a}认为，不同语言为了排除提取，在有关多少成分必须属于背景信息方面存在差异。任何情况下，我们不应该认为所有语言都应排除附加语的提取，因为像丹麦语\ilce{丹麦语}{Danish}这些语言可以从关系从句中进行提取\isce{关系小句}{relative clause}。\dotfootnote{%
...
\citet[\S~2.2.4]{EL2009a}讨论了类型学的文献，并且举出了缺少副词和形容词的语言的例子。作者引用了海峡萨利希语（Straits Salish）\ilce{海峡萨利希语}{Straits Salish}作为一种在动词和名词之间没有区别的语言的例子（参见 \citet[\page 481]{EL2009b}）。他们评论说，对于经常使用的排在前四位或前五位的非印欧语系的语言来说，有必要增加额外的词类，如状貌词\isce{状貌词}{ideophone}、方位词\isce{方位词}{positional}、副动词（coverb）\isce{副动词}{coverb}、类别词\isce{类别词}{classifier}等来分析这些语言。\dotfootnote{%
...
有时人们认为所有的自然语言都是递归的，而且在所有语言中一个任意长度的句子是可能的（概述参见\citet[\page 7]{HNG2005a} ，更多内容参见\citew[\S~2]{PS2010a}）。当有人提到递归的时候，通常是指带有自我嵌套的结构，正如我们在（\ref{Beispiel-Satzeinbettung}）的分析中所看到的\citep{Fitch2010a}。但是，有可能有的语言不允许自我嵌套。 \citet{Everett2005a-u}认为毗拉哈语（Pirah{\~a}）\ilce{毗拉哈语}{Pirah{\~a}}就是这样一种语言（但是，参见 \citew*{NPR2009a-u}和
...
即使我们可以找到所有语言共享的结构属性，仍然无法证明天赋语言知识的存在，因为这些相似性可以被追溯到其他因素上。有人认为，所有语言都必须按照这个方式以使得少量的有限资源能够被儿童习得（\citealp[\S~10.7.2]{Hurford2002a}；\citealp[\page 433]{Behrens2009a}）。按照这种说法，在它发展的相关阶段，我们的大脑就是一个限制因素。语言需要适合我们的大脑，而因为我们的大脑是相似的，语言也在某些方面是相似的（参见\citealp[\page 251]{Kluender92a}）。
...
通常\iscesub[|(]{习得}{acquisition}{习得速度}{speed}认为，儿童学习语言异常快，而这只能是因为他们已经掌握了不需要习得的关于语言的知识（如\citet[\page 144]{Chomsky76c-u}；\citet[\page
...
但问题是，我们与其他灵长类动物的区别是我们具有针对语言的特殊认知能力，还是因为我们在认知的领域一般性（domain-general）方面与灵长类动物不同，由此我们才具备了掌握语言的能力？不过， \citet[\S~2]{Fanselow92b}认为人类特有的形式能力（formal competence）不必然是针对语言的。相似地， \citet[\page 7--8]{Chomsky2007a}考虑归并\isce{合并}{Merge}（按照他的观点，这是唯一的构建结构的操作）是否属于语言特有的天赋能力，还是人类特有的一般能力（但是，参见\ref{Abschnitt-Rekursion}，尤其是脚注\ref{fn-Rekursion-Mathematik}）。
...
对于默认值假说带来的问题，参见 \citew[\page 17]{Meisel95a}。 \citet[\page 56, 脚注\,13]{Bickerton97a}不同意克里奥尔语具有默认的参数值的说法。
} 
...
但是，如果这些部位被损坏了，大脑的其他区域可以接管相应的功能。如果损害发生在幼儿早期，即使没有这些大脑部位，语言也可以被学得（相关资源，参见\citet[\S~4.1]{Dabrowska2004a}）。
...
%also not want to talk about a `hearing gene'.
 ...
 \citet*[\page 190]{FHC2005a}也认为FoxP2不是决定语言知识的根源。对这个话题的概述，参见 \citew{Bishop2002a}和 \citew[\S~6.4.2.2]{Dabrowska2004a}，一般意义上的基因问题，参见 \citew{FM2005a}。
...
在指出Pullum和Scholz从《华尔街日报》上得到的数据不必然跟语言习得相关这点上，Legate和Yang是正确的。而且，他们还指出了在数据中没有发现带有复杂主语名词短语的例子，或者至少是可以忽略不计的。但是，他们的论述有三个严重的问题：首先，在虚位主语和pro-脱落语言的属性之间没有关系：加利西亚语\ilce{加利西亚语}{Galician} \citep[\S~2.5]{RU90a-u}是带有虚位代词的pro-脱落语言。意大利语有一个表示存在的虚位成分ci，\footnote{%
...
这样就需要习得主语是在中心语的左边还是右边实现的信息。除了这个基于词汇的分析，另一种方法是，我们可以采用构式语法\isce{构式语法（CxG）}{Construction Grammar (CxG)}（Fillmore \citeyear[\page44]{Fillmore88a}；\citeyear{Fillmore99a}；\citealp[\page 18]{KF99a}）、认知语法\isce{认知语法}{Cognitive Grammar} \citep[\S~9]{Dabrowska2004a}或HPSG理论\indexhpsg \citep{GSag2000a-u}来分析。在这些框架中，只有两种模式\footnote{%
...
参见\citet[\page 324]{Hurford2002a}和 \citet[\S~6.2]{Jackendoff2007a}有关神经网络的特定类型带来的问题，以及 \citet{Pulvermueller2003a,Pulvermueller2010a}提出的不具有这些问题的另一种观点。
...
还需要指出的是：我们已经看到统计信息可以用来推导出复杂语言表达式的结构。现在，问题是这跟乔姆斯基早期反对的统计学方法有什么关系（\citealp[\page 16]{Chomsky57a}）。 \citet[\S~4.2]{Abney96a}详细地讨论了这点。乔姆斯基的早期观点的问题是他讨论的是马尔科夫模型\isce{马尔科夫模型}{Markov model}。这是有限状态自动机的统计版本。有限状态自动机\iscesub{自动机}{automaton}{有限状态自动机}{finite}只能描写3型语言\isce{复杂性类}{complexity class}，这样就不适合分析自然语言了。但是，Chomsky的评论不能适用于普遍的统计学方法。\isce[|)]{统计学}{statistics}\isce[|)]{无监督的面向数据的句法分析（U-DOP）}{Unsupervised Data-Oriented Parsing (U-DOP)}
...
负向证据的另外一种形式是间接的负向证据\iscesubsub{证据}{evidence}{负向证据}{negative}{间接负向证据}{indirect}， \citet[\page 9]{Chomsky81a}认为它们也可以在习得中起作用。 \citet[\S~5.2]{Goldberg95a}举出（\mex{1}a）中的句子作为例子：\colonfootnote{%
