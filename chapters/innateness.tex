...
这些年来，天赋性假说\isce{天赋论}{nativism}（也称作先天论）历经多次修改。尤其是关于先天语言知识组成的假设，即所谓的普遍语法（UG）\isc{普遍语法（UG）}\is{Universal Grammar (UG)}，也不断改变。
...
在讨论基于普遍语法的方法是否是可被检验的问题时， \citet*[\page
  2669]{CKT2010a}声称是不可能从关系从句中进行提取的，而且这些语言的存在质疑了普遍语法这个概念。（“如果儿童习得任何语言时，都可以学会从关系从句中提取语言表达式，那么这就会对普遍语法的基本原则提出严重的质疑。”）由此，他们反驳了Evans和Levinson，以及Tomasello，后面的这些学者认为普遍语法方法不可被检验\isc{普遍语法（UG）!可证伪性}\is{Universal Grammar (UG)!falsifiability}。如果Crain、Khlentzos 和Thornton的观点是正确的，那么（\mex{1}）就会证明普遍语法是假的，这样讨论就结束了。
}
...
%} 
如果我们认为语言可以从所有的可能性（工具包）\isc{普遍语法（UG）!作为工具包}\is{Universal Grammar (UG)!as a toolkit}中进行选择且不会穷尽的话，这种情况对于基于UG的理论来说就不是问题了（\citealp[\page
...
但是，如果我们纵容这个观点，就会有一定的武断性。我们可以针对至少一个语言去设定任意的一套词类系统，然后声称它是普遍语法的一部分，并进一步声明大部分（也许甚至是所有）语言并不会利用所有的词类。 \citet[\page 157]{Villavicencio2002a}就是这样认为的，他在范畴语法的框架下提出了范畴S、NP、N、PP和PRT。这种假设是不可证伪的\isc{普遍语法（UG）!可证伪性}\is{Universal Grammar (UG)!falsifiability}（关于相似情况的讨论以及更为普遍性的讨论参见\citet[\page 436]{EL2009a}、\citet[\page 471]{Tomasello2009a}）。
...
如果我们的语言知识不包括任何有关变换的信息，那么他们关于需要学习什么的观点就是错误的。例如，我们可以假设，正如范畴语法\isc{范畴语法（CG）}\is{Categorial Grammar (CG)}所主张的，助动词构成了一组具有特殊分布属性的词类。一种可能的布局是在疑问句中所观察到的首位，另一种是在主语后\citep[\page 104]{Villavicencio2002a}。
这样就需要习得主语是在中心语的左边还是右边实现的信息。除了这个基于词汇的分析，另一种方法是，我们可以采用构式语法\isc{构式语法（CxG）}\is{Construction Grammar (CxG)}（Fillmore \citeyear[\page44]{Fillmore88a}；\citeyear{Fillmore99a}；\citealp[\page 18]{KF99a}）、认知语法\isce{认知语法}{Cognitive Grammar} \citep[\S~9]{Dabrowska2004a}或HPSG理论\indexhpsg \citep{GSag2000a-u}来分析。在这些框架中，只有两种模式\footnote{%
