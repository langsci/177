...
如果我们允准这样的机制，当然就很容易维持许多普遍性的假设：我们假设一种语言X具有属性Y，然后假设结构性的成分是看不见而且没有意义的。这种分析只能用在理论内部以取得理论的统一性\isce{统一性}{uniformity}（参见\citet[第2.1.2节]{CJ2005a}）。\dotfootnote{%
...
参见 \citew[\page 211]{Mueller99a}和Müller (\citeyear{Mueller2004d}；\citeyear[第3节]{Mueller2007c})。相关的荷兰语\ilce{荷兰语}{Dutch}的例子，参见 \citew[\page 52]{Koster78b-u}。
}
...
\zl
Goldberg指出，属于背景信息的短语成分是孤岛，即背景构式是孤岛（Backgrounded constructions are islands，简称BCI）。 \citet{AG2008a}对这个语义/""语用分析进行了实验测试，并且将之与纯粹的句法方法进行了比较。他们能够证明信息结构\isce{信息结构}{information structure}属性对于成分的提取力发挥了重要的作用。 \citet[第3.H节]{Erteschik73a-u}与 \citet[\page 375]{AG2008a}认为，不同语言为了排除提取，在有关多少成分必须属于背景信息方面存在差异。任何情况下，我们不应该认为所有语言都应排除附加语的提取，因为像丹麦语\ilce{丹麦语}{Danish}这些语言可以从关系从句中进行提取\isce{关系小句}{relative clause}。\dotfootnote{%
...
即使我们可以找到所有语言共享的结构属性，仍然无法证明天赋语言知识的存在，因为这些相似性可以被追溯到其他因素上。有人认为，所有语言都必须按照这个方式以使得少量的有限资源能够被儿童习得（\citealp[第10.7.2节]{Hurford2002a}；\citealp[\page 433]{Behrens2009a}）。按照这种说法，在它发展的相关阶段，我们的大脑就是一个限制因素。语言需要适合我们的大脑，而因为我们的大脑是相似的，语言也在某些方面是相似的（参见\citealp[\page 251]{Kluender92a}）。
...
但是，如果这些部位被损坏了，大脑的其他区域可以接管相应的功能。如果损害发生在幼儿早期，即使没有这些大脑部位，语言也可以被学得（相关资源，参见\citet[第4.1节]{Dabrowska2004a}）。
...
除此之外，也可以观察到，大脑的某个特定部位在阅读时会被激活。可以得出这样的推论，在大脑的某个特定部位进行处理的定位会引发语言知识的天赋机制，如果说这一推论是合理的，那么在阅读时，大脑某些部位的激活就会让我们得出这样的结论，阅读能力是天赋的（\citealp[\page ]{EBJKSPP96a}；\citealp[\page 57]{Bishop2002a}）。但是，这样的假设并不存在（也参见\citet*[\page 196]{FHC2005a}）。
...
因此，对于英语习得来说，先学习一些助动词的范式是Aux NP VP，然后再了解到所有助动词都符合这个模式，这样就足够了。 \citet{LE2001a}也证明了这个观点，他训练了一个神经网络\isce[|(]{神经网络}{neural network}，其中使用了助动词构式中不包括带有关系从句的NP的数据。但是，关系小句在其他结构中会出现。训练语料的复杂度一点一点增加，就像儿童接收语言输入一样\citep{Elman93a}。\dotfootnote{%
