...
每一种理论不仅要描述自然语言，还要解释自然语言。为某一给定语言设定允准其结构的无穷多个语法是可行的（见第\pageref{ua-psg-eins}页的练习\ref{ua-psg-eins}）。这些语法在观察上是充分的（observationally adequate）\isce{观察的充分性}{observational adequacy}。如果一种语法能够符合观察以及母语者语感的话，该语法就实现了描写的充分性（descriptive adequacy）\isce{描写的充分性}{descriptive adequacy}。\footnote{%
...
先天论经常遭到构式语法\indexcxgc、认知语法\isce{认知语法}{Cognitive Grammar}的支持者以及其他理论学派研究人员的反对。其他的解释一般被用来支持这样的事实，即语法范畴、句法结构或者句法结构中语言对象之间的关系具有天赋性。受到批评的另外一点是，由于很多规定被简单化地假设为普遍语法的一部分，语法分析的实际复杂性被模糊掉了。下面是GB/最简方案分析下一些论述的夸张表示。
...
句法\isce[|(]{普遍性}{universal}的普遍性被作为论据来证明语言知识的天赋性（如\citet[\page 33]{Chomsky98a-u}、\citet[\page 237--238]{Pinker94a}）。在文献中，对于什么是普遍性的、什么是语言特定的有不同的看法。支持普遍性的突出代表有：\footnote{%
...
我们可以看到，Pinker将管辖方向之间的关系认定为普遍规则的观点实际上是正确的，更应该说是作为一种倾向性时才是正确的，而不是作为严格的规则。也就是说，有许多语言，其中前置词或后置词的使用与动词的位置之间具有相关性\citep[\page 83]{Dryer92a}。\footnote{%
...
\xbarc 结构被用来限制可能规则的形式。相应假设是这些限制减少了我们可以构建的语法类别，并且，使得语言更容易被习得。但是正如 \citet{KP90a}所展示的，\xbarc 结构的假说无法限制可能语法的数量，如果我们允准空的中心语的话。管约论使用了很多空的中心语\isce{空成分}{empty element}，而且在最简方案\indexmpc 中，这些情况显著地增多了。比如说，（\mex{0}）中的规则可以被重构为：
...
\gll der auf seinen Sohn sehr stolze Mann\\
	 \textsc{art}.\textsc{def} \textsc{prep} 他的 儿子 非常 骄傲 男人\\
...
\gll der sehr auf seinen Sohn stolze Mann\\
	 \textsc{art}.\textsc{def} 非常 \textsc{prep} 他的 儿子 骄傲 男人\\
%	 the very of his son proud man\\
}
\ex[*]{
\gll der auf seinen Sohn stolze sehr Mann\\
	 \textsc{art}.\textsc{def} \textsc{prep} 他的 儿子 骄傲 非常 男人\\
...
\gll [[der Kater] und [die Katze]], die einander lieben\\
     \spacebr{}\spacebr{}\textsc{art}.\textsc{def} 公猫 和 \textsc{art}.\textsc{def} 猫 \textsc{rel} 互相 爱\\
\mytrans{公猫和（母）猫相互爱慕}
...
如果我们希望把所有的信息编码在词汇中，那么我们就需要能够表示共性的极为抽象的组合规则。这种组合性规则的一个例子是范畴语法\isce{范畴语法（CG）}{Categorial Grammar (CG)}的多重应用规则（参见第\ref{Kapitel-CG}章），以及最简方案\indexmpc 中的合并\isce{合并}{Merge}（参见第\ref{Abschnitt-MP}章）。上面这些规则仅仅说明了两个语言学对象被组合在一起。这种组合性当然存在于每一种语言中。但是，对于完全词汇化的语法而言，只有我们允许空中心语并且提出某些特别的假设的话，才有可能来描写语言。这将在\ref{Abschnitt-Phrasale-Konstruktionen}中进行讨论。\isceat[|)]{X-理论}{\xbar-理论}{X-theory}{\xbar-theory} 
...
\mbox{} \citet[\page xxv]{BK82a}、 \citet[\page 236--237]{Pinker94a}\isce[|(]{主语}{subject}\isce[|(]{宾语}{object}\isce[|(]{语法功能}{grammatical function}、 \citet[\page 349]{Baker2003b}和其他人认为所有的语言都有主语和宾语。为了说明这个观点到底意味着什么，我们必须探究这些术语。对于大多数的欧洲语言来说，很容易说明主语和宾语是什么（参见\ref{Abschnitt-GF}）；但是，并不是所有的语言都是这样的，或者说在有些语言中用这些术语完全没有意义（\LATER{\citealp{Durie85a};
...
在Pinker研究的LFG\indexlfgc{}这类理论中，语法功能扮演着重要的角色。事实上，应把句子视为主语、宾语，还是特殊定义的子句型论元（\xcompc）\citep*{DL2000a-u,Berman2003b-u,Berman2007a-u,AMM2005a-u,Forst2006a-u}仍然存在争议。这就意味着当我们讨论语法功能如何分配给论元时具有一定的自由度与灵活性。由此，我们有可能在所有语言中找到一种将语法功能指派给函项论元的形式。
...
与LFG不同，语法功能在GB（参见\citealp{Williams84a,Sternefeld85a}）和范畴语法\indexcgc 中并不重要，在GB中语法功能只能间接地通过指向树的位置来决定。由此，在第\ref{Kapitel-GB}章讨论的方法中，主语是位于IP的限定语位置上的短语。
...
HPSG\indexhpsgc 所持的观点较为中立：它用一个特殊的配价特征来表示主语（在德语语法中，有一个中心语特征还可以对非定式动词的主语进行表征）。但是，\subjfc 的值是从更为普遍的理论性角度推导出来的：在德语中，带有结构格\iscesub{格}{case}{结构格}{structural}的层级最低的旁格\isce{旁格}{obliqueness}成分就是主语（Müller \citeyear[\page 153]{Mueller2002b}; \citeyear[\page 311]{MuellerLehrbuch1}）。
...
在\gbc 理论（扩展的投射原则（Extended Projection Principle，简称EPP）\isce{扩展的投射原则（EPP）}{Extended Projection Principle (EPP)}、\citew[\page 10]{Chomsky82a-u}）和LFG\indexlfg （主语条件\isce{主语条件}{Subject Condition}）中，都有用来确保每个句子都有一个主语的原则。通常认为这些原则具有语言共性。\footnote{%
...
\gll Ihm graut vor der Prüfung.\\
     他.\dat{} 害怕 \textsc{prep} \textsc{art}.\textsc{def} 考试\\
\mytrans{他怕考试。}
...
\gll dass es ihm vor der Prüfung graut\\
	 \textsc{comp} \expl{} 他 \textsc{prep} \textsc{art}.\textsc{def} 考试 害怕\\
\mytrans{他怕考试。}
...
如果我们允准这样的机制，当然就很容易维持许多普遍性的假设：我们假设一种语言X具有属性Y，然后假设结构性的成分是看不见而且没有意义的。这种分析只能用在理论内部以取得理论的统一性\isce{统一性}{uniformity}（参见\citet[\S~2.1.2]{CJ2005a}）。\footnote{%
...
邻接原则被认为是具有语言共性。邻接限制规定移位操作可以跨越一个约束结点\isce{约束结点}{bounding node}，而到底什么是约束结点，则取决于研究的语言（Baltin \citeyear[\page 262]{Baltin81a}；\citeyear{Baltin2006a}；\citealp[\page 57]{Rizzi82b}；\citealp[\page 38--40]{Chomsky86b}）。\LATER{ \citew{PB90a,Newmeyer91a}}\footnote{%
...
\gll {}[\sub{NP} Many proofs [\sub{PP} of            [the theorem t]] t$'$] appeared [that I wanted to think about].\\
     {}          许多  证据    {}       \textsc{prep} \spacebr\textsc{art}.\textsc{def} 定理 {} {} 出现 \spacebr\textsc{rel} 我 想 \textsc{inf} 思考 \textsc{prep}\\
...
\z
这句话可以在这样的场景中使用，其中某人在一个唱片店中看到有些特殊的唱片，并且想起来他以前想读这些唱片上的童话故事。因为他并没有读这些唱片，附加到上位名词是不可能的，所以就附加到Geschichten（故事）上了。通过仔细地挑选名词，就有可能构建出诸如（\mex{1}）的例子，这些例子表示外置允许跨越多个NP结点：\footnote{%
...
\gll Karl hat          mir [eine Fälschung [des Bildes                 [einer Frau \_$_i$]]] gegeben, [die schon lange tot ist]$_i$.\\
     Karl \textsc{aux} 我  \spacebr{}一 伪造 \spacebr{}\textsc{art}.\textsc{def} 照片 \spacebr{}一 女人 {} 给 \spacebr{}\textsc{rel} \textsc{part} 长 死 \textsc{cop}\\
...
\gll Karl hat mir [eine Kopie [einer Fälschung [des Bildes [einer Frau \_$_i$]]]] gegeben, [die schon lange tot ist]$_i$.\\
	 Karl \textsc{aux} 我 \spacebr{}一 复制 \spacebr{}一 伪造 \spacebr{}\textsc{art}.\textsc{def} 照片 \spacebr{}一 女人 {} 给 \spacebr{}\textsc{rel} \textsc{part} 长 死 \textsc{cop}\\
...
\gll Ich habe       [von                    [der                    Vermutung \_$_i$]] gehört, [dass es Zahlen gibt, die die folgenden Bedingungen erfüllen]$_i$.\\
     我 \textsc{aux} \spacebr{}\textsc{prep} \spacebr{}\textsc{art}.\textsc{def} 猜测 {} 听说 \spacebr{}\textsc{comp} \expl{} 数字 给 \textsc{rel} \textsc{art}.\textsc{def} 下面的 要求 满足\\
...
\gll Ich habe [von [einem Beweis [der Vermutung \_$_i$]]] gehört, [dass es Zahlen gibt, die die folgenden Bedingungen erfüllen]$_i$.\\
	我 \textsc{aux} \spacebr{}\textsc{prep} \spacebr{}一 证据 \spacebr{}\textsc{art}.\textsc{def} 猜测 {} 听说 \spacebr{}\textsc{comp} \expl{} 数字 给 \textsc{rel} \textsc{art}.\textsc{def} 下面的 要求 满足\\
...
\gll Ich habe [von [dem Versuch [eines Beweises [der Vermutung \_$_i$]]]] gehört, [dass es Zahlen gibt, die die folgenden Bedingungen erfüllen]$_i$.\\
     我 \textsc{aux} \spacebr{}\textsc{prep} \spacebr{}\textsc{art}.\textsc{def} 尝试 \spacebr{}一 证据 \spacebr{}\textsc{art}.\textsc{def} 猜测 {} 听说 \spacebr{}\textsc{comp} \expl{} 数字 给 \textsc{rel} \textsc{art}.\textsc{def} 下面的 要求 满足\\
...
\gll Der 43jährige will nach eigener Darstellung damit [\sub{NP} den Weg [\sub{PP}~für [\sub{NP} eine
  ...
  \textsc{art}.\textsc{def} 43岁 想要 \textsc{prep} 自己的 描述 那儿.\textsc{prep} {} \textsc{art}.\textsc{def} 道路 \hspaceThis{[\sub{PP}~}\textsc{prep} {} 一 讨论 {} 关于 {} \textsc{art}.\textsc{def} 未来 路线 {} \textsc{art}.\textsc{def} 强
  反对.派 自由.做 \spacebr{}\textsc{rel} 但是 \textsc{prep} 10.4 百分点
  \textsc{art}.\textsc{def} 选票 \textsc{prep} \textsc{art}.\textsc{def} 选举 \textsc{prep}.\textsc{art}.\textsc{def} 十月 远 \textsc{prep} \textsc{art}.\textsc{def} 预期 停留.回 \textsc{aux}\\
...
\gll {}[\ldots] die Erfindung der Guillotine könnte [\sub{NP} die Folge [\sub{NP} eines verzweifelten
    ...
    {}  \textsc{art}.\textsc{def} 发明 \textsc{art}.\textsc{def} 断头台 能够 {} \textsc{art}.\textsc{def} 结果 {} 一 绝望 尝试 \textsc{art}.\textsc{def} 相同.名字 医生 \textsc{cop} \textsc{aux} \spacebr{}他的 病人
...
\mytrans{例如，我们知道Ariva巴士最近已经取得了一些伦敦路线的合同，这些路线不会由低地台无障碍巴士运营了。'}\footnote{%
...
\mytrans{我们那天晚上起草了一个基本需求的清单，必须无条件地满足这些需求，不然我们就不再制作披萨和运送披萨了，而是继续罢工。'}\footnote{%
...
谁 \textsc{aux} 他 刚 读 {} \textsc{art}.\textsc{def} 报告 {} \textsc{rel} \textsc{cop} 关于\\
}
\z
...
\gll She shouted the remark/the question/something I could not understand.\\
     她 大喊 \textsc{art}.\textsc{def} 话/\textsc{art}.\textsc{def} 问题/某事 我 能够 不 理解\\
...
\gll She realized [\sub{NP} the fact [\sub{S} that he left]].\\
     她   意识到   {}         \textsc{art}.\textsc{def} 事实 {} \textsc{rel} 他 离开\\
\mytrans{她意识到他离开了的事实。}
\ex 
...
谁 \textsc{aux} 她 意识到 \textsc{art}.\textsc{def} 事实 \textsc{rel} 他 看见\\
}
\ex[??]{
...
和 \citet{vanValin98a}一样， \citet[\S~7.2]{Goldberg2006a}认为，空位一定属于可以潜在构成话语的焦点\isce{焦点}{focus}的那部分（参见 \citew{Cook2001a}、 \citew{deKuthy2002a}，有关德语的参见
 \citew{Fanselow2003a}）。这就意味着这个部分一定不能被预设\isce{预设}{presupposition}。\footnote{%
不管整句话是不是否定的，预设为真的仍为真。所以说，下面（i.a）和（i.b）都表明有一位法国国王。
  \eal
  \ex 
  \gll The King of France is bald.\\
  \textsc{art}.\textsc{def} 国王 \textsc{prep} 法国 \textsc{cop} 秃的\\
  \mytrans{法国国王是秃子。}
  \ex 
  \gll The King of France is not bald.\\
  \textsc{art}.\textsc{def} 国王 \textsc{prep} 法国 \textsc{cop} 不 秃的\\
  \mytrans{法国国王不是秃子。}
  \zllast
}
...
\gll She didn't         see the          report that      was          about him. $\to$ The          report was         about him.\\
     她 \textsc{aux}.不 看见 \textsc{art}.\textsc{def} 报告 \textsc{rel} \textsc{cop} 关于 他     {}    \textsc{art}.\textsc{def} 报告   \textsc{cop} 关于 他\\
...
\zl
Goldberg指出，属于背景信息的短语成分是孤岛，即背景构式是孤岛（Backgrounded constructions are islands，简称BCI）。 \citet{AG2008a}对这个语义/""语用分析进行了实验测试，并且将之与纯粹的句法方法进行了比较。他们能够证明信息结构\isce{信息结构}{information structure}属性对于成分的提取力发挥了重要的作用。 \citet[\S~3.H]{Erteschik73a-u}与 \citet[\page 375]{AG2008a}认为，不同语言为了排除提取，在有关多少成分必须属于背景信息方面存在差异。任何情况下，我们不应该认为所有语言都应排除附加语的提取，因为像丹麦语\ilce{丹麦语}{Danish}这些语言可以从关系从句中进行提取\isce{关系小句}{relative clause}。\footnote{%
...
\gll Det    hus$_i$  kender jeg en    mand [som har købt \_$_i$].\\
     \textsc{art}.\textsc{def}   房子  认识  我 一 男人 \hspaceThis{[}\textsc{comp} \textsc{aux} 买\\
...
在\ref{Abschnitt-neues-GB}，我们提到了所谓的制图方法\isce{制图}{cartography}，其中有些人提出了超过三十种功能范畴（参见第\pageref{Tabelle-Cinque}页有关Cinque\aimention{Guglielmo
  Cinque}的功能性中心语的表格\ref{Tabelle-Cinque}），并且认为这些范畴与相应固定的句法结构一起构成了普遍语法的一部分。 \citet[\page 55, 57]{CR2010a}甚至提出了超过400种被认为在所有语言的语法中起到了重要作用的功能范畴\iscesub{范畴}{category}{功能}{functional}
。\footnote{%
...
%based on innate language-specific knowledge.
 ...
 \citet[\S~2.2.4]{EL2009a}讨论了类型学的文献，并且举出了缺少副词和形容词的语言的例子。作者引用了海峡萨利希语（Straits Salish）\ilce{海峡萨利希语}{Straits Salish}作为一种在动词和名词之间没有区别的语言的例子（参见 \citet[\page 481]{EL2009b}）。他们评论说，对于经常使用的排在前四位或前五位的非印欧语系的语言来说，有必要增加额外的词类，如状貌词\isce{状貌词}{ideophone}、方位词\isce{方位词}{positional}、副动词（coverb）\isce{副动词}{coverb}、类别词\isce{类别词}{classifier}等来分析这些语言。\footnote{%
...
458]{Haspelmath2009a}和 \citet[\page 453]{Croft2009a}竟然否定了具有跨语言共性的词类的存在。我认为这过于极端，我认为更好的研究策略是试图找到不同语言的共同点。\footnote{%
...
在《科学》杂志的一篇文章中， \citet*{HCF2002a}进一步阐明了只有递归才具有具体领域的普遍性，“提供一个有限成分集合生成无限表达的能力”（参见第\pageref{NP-Regeln-Adj}页关于递归的短语结构规则的例子 (\ref{NP-Regeln-Adj})）。\footnote{%
...
%}
\begin{quotation}
语言的生成并不是简单地创造一个话语的过程；与此同时，它还允许这类表达的无限集合，其中最为重要的是，允许思想赋予表达的条件。语言面对着一个无限且真正没有边界的主观实在，一个人们可能想象的所有事情的缩影。由此，它必须能够利用有限的手段来表达无限的内容，而且这可以通过负责思维和语言的生成能力来确认。\citep[\page 108]{Humboldt88a-u}\footnote{%
...
\noindent
如果只看语言事实，我们可以看到语句的长度有一个上限。这是因为特别长的语句无法处理，说话者需要睡觉或者最终会在某个时刻死去。如果我们设定一个长度为10万个语素的句子，然后假设一个语素库X，我们就可以构造出不到X$^{100,000}$个表达式。如果我们在每个100,000位置上使用每个语素的话，就会得到 X$^{100,000}$这个数。由于不是所有这些表达式都合乎语法，那么实际上是少于X$^{100,000}$个可能的表达式的（参见\citet{Weydt72a}中类似的但是更为具体的说明）。这个数已经够大的了，但是仍是有限的。对于思维来说也是一样：我们不需要无限多的思想（如果“无限”这个概念是按照这个词的数学含义来理解的话），尽管Humboldt和 \citet[\page 137]{Chomsky2008a}的观点是相反的。\footnote{%
...
\citeyearpar{JP80a-u}、对 \citet{Kaplan95a}形式化的LFG\indexlfgc、对 \citet{Rogers97a}重新形式化的GPSG\indexgpsg，以及根据 \citet{King99a-u}、\citet{Pollard99a}和 \citet{Richter2007a}假设的HPSG\indexhpsgc 采用的是模型理论的方法。在基于约束的理论中，我们可以分析像（\ref{Beispiel-Satzeinbettung}）的例子，并且说特定的态度动词选择一个名词性NP和一个that从句，而且这些只能在特定的局部句法配置中出现，句法配置中所含成分之间具有特殊的关系。其中一种关系就是主谓一致。按照这个方式，我们可以表示诸如（\ref{Beispiel-Satzeinbettung}）的表达式，并且不需要说明有多少句子是可以嵌套的。这就意味着基于约束的理论跟结构的数量是有限还是无限这一问题的两个答案都兼容。通过使用按照这个方式构建出的语言能力语法，就有可能开发出语言使用的模型。它能够解释为什么有些字符串，比如说特别长的那些，是不可接受的（参见第\ref{Abschnitt-Diskussion-Performanz}章）。
...
另一个没有递归的语言的例子是瓦尔皮里语（Warlpiri）\ilce{瓦尔皮里语}{Warlpiri}，它通常跟 \citew{Hale76a}一起引用。不过，Hale的规则针对带关系从句的句子的组合是递归的（第85页）。在第98页递归被明确地表示出来。\footnote{%
...
当然，说所有语言都是递归的这个观点有一定的意义：它们遵守这样的规则，一个具体数量的符号可以被组合以构成另一个符号。\footnote{%
...
同样也有人认为，范畴语法\indexcgc 的组合性规则也具有语言的普遍性。我们可以利用这些规则来将函项跟它的论元相组合（\mbox{X/Y $*$ Y = X}）。这些规则跟（\mex{0}）的规则几乎一样抽象。区别是其中一个成分必须是函项。在最简方案\indexmpc 中也有对应的限制，比如说选择性特征（参见\ref{Abschnitt-MG}）和语义角色指派的限制。但是，是否是范畴语法允准了递归结构并不取决于普遍的组合模式，而是词汇项。使用（\mex{1}）中的词汇项，只有可能分析两个句子，并且一定不会构建递归的结构。
...
即使我们可以找到所有语言共享的结构属性，我们仍然无法证明天赋语言知识的存在，因为这些相似性可以被追溯到其他因素上。有人认为，所有语言都必须按照这个方式以使得少量的有限资源能够被儿童习得（\citealp[\S~10.7.2]{Hurford2002a}；\citealp[\page 433]{Behrens2009a}）。按照这种说法，在它发展的相关阶段，我们的大脑就是一个限制因素。语言需要适合我们的大脑，而因为我们的大脑是相似的，语言也在某些方面是相似的（参见\citet[\page 251]{Kluender92a}）。
...
\mytrans{这是一个声明吗？}
\zl
然后作者观察发现，大脑在学习人工语言时不同的区域被激活。这是一个有趣的结果，但是不能说明我们具有天赋的语言知识。它只说明了在我们处理自己的自然语言时活跃的区域在学习其他语言时也是活跃的，而且像调换词序这种玩词游戏也会影响大脑的其他区域。
...
有\isce[|(]{威廉综合症}{Williams Syndrome}一些人智商（IQ\isce{智商}{IQ}）特别低，但是能说出合法的句子。这些人当中有的人有威廉综合症（关于威廉综合症患者能力的讨论，参见 \citew*{BLJLG2000a}）。 \citet{Yamada81a}将这些案例作为证据来证明存在一个独立于其他智力的语法模型。
...
know.\\
我 要 喜欢 \textsc{inf} 告诉 你 另一个 有趣的 故事 但是 我.\textsc{aux} 已经 告诉 你 \textsc{art}.\textsc{def} 唯一 \textsc{pron} 我 知道\\
\mytrans{我想给你讲另一个有趣的故事，但是我已经给你讲了我唯一知道的一个。}
\ex 
\gll The old man from France was more erudite than the young \emph{one}.\\
\textsc{art}.\textsc{def} 老 人 从 法国 \textsc{aux} 更加 博学 比 \textsc{art}.\textsc{def} 年轻 \textsc{pron}\\
...
\textsc{art}.\textsc{def} 学生 \textsc{prep} 化学 \textsc{cop} 更加 全面地 准备 比 \textsc{art}.\textsc{def} \textsc{pron} \textsc{prep} 物理学\\
...
我.\textsc{aux} 更 教 语言学 \textsc{inf} 一 学生 \textsc{prep} 数学 比 \textsc{inf} \textsc{pron} \textsc{prep}  任何 专业 \textsc{prep} \textsc{art}.\textsc{def} 人文学科\\
...
一 支持者 \textsc{prep} Linux 卷 进 一 热烈的 讨论 \textsc{prep} \textsc{pron} \textsc{prep} Windows NT 和 \textsc{art}.\textsc{def} 剩余的 \textsc{prep} \textsc{art}.\textsc{def} 晚上 \textsc{cop} 书呆子 谈话\\
...
问题又出现了：学习者需要听到多少个例子才能算作PSA理论支持者眼中认可的证据。\ilce[|)]{英语}{English}
...
\textsc{art}.\textsc{def} 狗 在 \textsc{art}.\textsc{def} 角落 \textsc{cop} 饿\\
\mytrans{在角落里的狗饿了。}
}
\ex[]{
\gll Is the dog in the corner hungry?\\
\textsc{cop} \textsc{art}.\textsc{def} 狗 在 \textsc{art}.\textsc{def} 角落 饿\\
\mytrans{在角落里的狗饿了吗？}
}
\ex[]{
\gll The dog that is in the corner is hungry.\\
\textsc{art}.\textsc{def} 狗 \textsc{rel} \textsc{cop} 在 \textsc{art}.\textsc{def} 角落 \textsc{cop} 饿\\
\mytrans{在角落里的那只狗饿了。}
}
\ex[]{\label{Hilfsverbinversion-mit-RS}
\gll Is the dog that is in the corner hungry?\\
\textsc{cop} \textsc{art}.\textsc{def} 狗 \textsc{rel} \textsc{cop} 在 \textsc{art}.\textsc{def} 角落 饿\\
\mytrans{在角落里的那只狗饿了吗？}
}
\ex[*]{\label{Hilfsverbinversion-ungrammatisch}
\gll Is the dog that in the corner is hungry?\\
\textsc{cop} \textsc{art}.\textsc{def} 狗 \textsc{rel} 在 \textsc{art}.\textsc{def} 角落 \textsc{cop} 饿\\
}
\zl
...
在语料库数据和貌似真实的构造出的例子的帮助下， \citet{Pullum96a}证明了这个观点明显是错误的。
  \citet{Pullum96a}在《华尔街日报》中找到了例子，并且 \citet{PS2002a}更为细致地讨论了相关的例子，再加上CHILDES语料库\isce{CHILDES语料库}{CHILDES}中的例子，一同说明不只成年人能造出相关类型的句子，而且它们也出现在儿童的输入中。\footnote{%
...
CHILDES\isce{CHILDES语料库}{CHILDES}语料库中的例子反驳了第一助动词需要前置的假说，如（\mex{1}）所示：\footnote{%
...
\textsc{cop} \textsc{art}.\textsc{def} 球 你 \textsc{aux} 谈 \textsc{prep} 在 \textsc{art}.\textsc{def} 盒子 \textsc{prep} \textsc{art}.\textsc{def} 保龄球 瓶\\
...
当 你.\textsc{aux} 睡觉 可以 我 做 \textsc{art}.\textsc{def} 早饭\\
...
哪儿.\textsc{cop} \textsc{art}.\textsc{def} 申请表 Mark 承诺 \textsc{inf} 填 \textsc{adv}\\
...
\gll the          application Mark [\sub{\textsc{aux}} PAST] promised to            fill out          [\sub{\textsc{aux}} is] there\\
     \textsc{art}.\textsc{def} 申请表       Mark {}                  过去  承诺      \textsc{inf} 填   \textsc{adv} {}                  \textsc{cop} 那儿\\
\mytrans{Mark承诺要填的表在那儿}
}
\ex[*]{
\gll Where did the application Mark promised to fill out is?\\
哪儿 \textsc{aux} \textsc{art}.\textsc{def} 申请表 Mark 承诺 \textsc{inf} 填 \textsc{adv} \textsc{cop}\\
}
\zl
...
哪儿.\textsc{cop} \textsc{art}.\textsc{def} 小 蓝色 螃蟹 \textsc{rel} \textsc{cop} \textsc{prep} \textsc{art}.\textsc{def} 房子 以前\\
\mytrans{以前房子里的小蓝螃蟹在哪儿？}
\ex 
\gll Where's the other dolly that was in here?\\
哪儿.\textsc{cop} \textsc{art}.\textsc{def} 另一个 娃娃  \textsc{rel} \textsc{cop} \textsc{prep} 这儿\\
\mytrans{原来在这儿的另一个娃娃在哪儿？}
\ex 
\gll Where's the other doll that goes in there?\\
哪儿.\textsc{cop} \textsc{art}.\textsc{def} 另一个 娃娃 \textsc{rel} 走 在 那儿 \\
...
\ex[]{
\gll Is the dog in the corner hungry?\\
\textsc{cop} \textsc{art}.\textsc{def} 狗 \textsc{prep} \textsc{art}.\textsc{def} 角落 饿\\
\mytrans{在角落里的狗饿吗？}
}
\ex[]{
\gll Is the dog that is in the corner hungry?\\
\textsc{cop} \textsc{art}.\textsc{def} 狗 \textsc{rel} \textsc{cop} \textsc{prep} \textsc{art}.\textsc{def} 角落 饿\\
\mytrans{在角落里的那只狗饿吗？}
}
\zl
...
\ea[*]{
\gll Is the dog that in the corner is hungry?\\
\textsc{cop} \textsc{art}.\textsc{def} 狗 \textsc{rel} \textsc{prep} \textsc{art}.\textsc{def} 角落 \textsc{cop} 饿\\
}
\z
...
\begin{quotation}
当然，构成问句的正确操作是基于结构的：它包括将句子分析为结构短语，并将位于主语NP后的助动词前置，主语NP可以是任意长度的 ：\footnote{%
...
\textsc{cop} \spacebr\textsc{art}.\textsc{def} 女人 \textsc{rel} \textsc{aux} 唱歌 {} 高兴\\
\mytrans{唱歌的女人高兴吗？}
\ex 
\gll Has [the man that is reading a book] e eaten supper?\\
\textsc{aux} \spacebr\textsc{art}.\textsc{def} 人 \textsc{rel} \textsc{aux} 读 一 书 {} 吃 晚饭\\
\mytrans{看书的那个人吃晚饭了吗？}
\end{xlist}
\end{exe}
\end{quotation}
...
因此，对于英语习得来说，先学习一些助动词的范式是Aux NP VP，然后再了解到所有助动词都符合这个模式，这样就足够了。 \citet{LE2001a}也证明了这个观点，他训练了一个神经网络\isce[|(]{神经网络}{neural network}，其中使用了助动词构式中不包括带有关系从句的NP的数据。但是，关系小句在其他结构中会出现。训练语料的复杂度一点一点增加，就像儿童接收语言输入一样\citep{Elman93a}。\footnote{%
...
\textsc{cop} \textsc{art}.\textsc{def} 男孩 \textsc{rel} \textsc{aux} 抽烟 疯狂\\
\mytrans{抽烟的那个男孩疯狂吗？}
\z
...
\textsc{aux} \textsc{art}.\textsc{def} 男孩 \textsc{rel} 抽烟 \textsc{cop} 疯狂\\
}
\z
...
%\gll \hspaceThis{*~} The boy who is running fast can jump high. \\
      {}              \textsc{art}.\textsc{def} 男孩 \textsc{rel} \textsc{aux} 跑 快 能 跳 高\\
\glt \hspaceThis{*~}\quotetrans{跑得快的那个男孩跳得高。}\\
$\to$\\
 {}* \gll Is the boy who running fast can jump high?\\
          \textsc{aux} \textsc{art}.\textsc{def} 男孩 \textsc{rel} 跑 快 能 跳 高\\
\ex 
...
$\to$\\
 {}* \gll Can the boy who run fast can jump high?\\
          能 \textsc{art}.\textsc{def} 男孩 \textsc{rel} 跑 快 能 跳 高\\
\zl
...
\ex 
\gll Watch the dog.\\
小心 \textsc{art}.\textsc{def} 狗\\
\mytrans{小心狗。}
\ex 
\gll The dog barks.\\
\textsc{art}.\textsc{def} 狗 叫\\
\mytrans{狗叫。}
\zl
...
\hspaceThis{[[}\textsc{art}.\textsc{def} 狗 叫\\
\mytrans{狗叫}
\ex \gll {}[the [dog barks]]\\
         \spacebr\textsc{art}.\textsc{def} \spacebr{}狗 叫\\
\mytrans{狗叫}
\zl
一棵树的概率是它所有分析的可能性的总和。
...
什么.\textsc{aux} 这 划痕 做 \textsc{prep} \textsc{art}.\textsc{def} 桌子\\
\mytrans{桌子上怎么有个划痕？}
\ex 
...
\textsc{art}.\textsc{def} 人 \textsc{rel} \textsc{aux} 吃 \textsc{cop} 饿\\
\mytrans{正吃东西的那个人饿了。}
\ex 
\gll Is the boy hungry?\\
\textsc{cop} \textsc{art}.\textsc{def} 男孩 饿\\
\mytrans{男孩饿了吗？}
\zl
...
\textsc{cop} \textsc{art}.\textsc{def} 人 \textsc{rel} \textsc{aux} 吃 饿\\
\mytrans{吃东西的那个人饿了吗？}
}
\ex[*]{
\gll Is the man who eating is hungry?\\
\textsc{aux} \textsc{art}.\textsc{def} 人 \textsc{rel} 吃 \textsc{cop} 饿\\
}
\zl
...
\textsc{art}.\textsc{def} 人 \textsc{rel} \textsc{aux} 吃 嘟囔\\
\mytrans{正吃东西的那个人嘟囔了。}
\ex \gll The man is hungry.\\
\textsc{art}.\textsc{def} 人 \textsc{cop}  饿\\
\mytrans{那个人饿了。}
\ex \gll The man mumbled.\\
\textsc{art}.\textsc{def} 人 嘟囔\\
\mytrans{那个人嘟囔了。}
\ex \gll The boy is eating.\\
\textsc{art}.\textsc{def} 男孩 \textsc{aux} 吃\\
\mytrans{男孩正在吃。}
\zl
...
\textsc{cop}/\textsc{aux} \textsc{art}.\textsc{def} 人 \textsc{rel} \textsc{aux} 吃 \textsc{cop} 饿\\
}
\z
...
尽管如此，这里有必要指出：Bod证明的是，句法结构是可以学习的。那些还没有被归入他的结构中的每个词所包含的词性信息也可以用统计\isce{统计学}{statistics}的方法推导出来\citep{RCF98a,Clark2000a}。\footnote{%
用来区分词类的计算语言学算法是考察整个语料的。但是儿童总是处理其中的一部分。那么，相应的学习过程也一定会包括一个记忆曲线。参见\citew[\page
  67]{Braine87a}。
}
...
负向证据的另外一种形式是间接的负向证据\iscesubsub{证据}{evidence}{负向证据}{negative}{间接负向证据}{indirect}， \citet[\page 9]{Chomsky81a}认为它们也可以在习得中起作用。 \citet[\S~5.2]{Goldberg95a}举出（\mex{1}a）中的句子作为例子：\footnote{%
...
看 \textsc{art}.\textsc{def} 魔术师 使得 \textsc{art}.\textsc{def} 鸟 消失\\
\mytrans{看！魔术师把鸟变没了。}
}
\ex[*]{
\gll The magician disappeared the bird.\\
\textsc{art}.\textsc{def} 魔术师 消失 \textsc{art}.\textsc{def} 鸟\\
}
\zl
...
叫 \textsc{art}.\textsc{def} 警察 马上\\
\mytrans{马上叫警察}
}
\ex[*]{
\gll call immediately the police\\
叫 马上 \textsc{art}.\textsc{def} 警察\\
}
\zl
...
有些使用了神经网络的方法遭到了批评，这是因为它们无法捕捉到人类能力的某些方法，如递归或者话语中相同词的多次使用。
  \citet{Pulvermueller2010a}讨论了一个具有记忆的架构，并且使用它来分析递归结构。在他的概述性文章中，他引用了一些研究来证明许多更为抽象的规则或理论语言学中认为理所当然的模式都可以在神经层面进行证明。但是，Pulvermüller并不认为语言知识是天赋的（第173页）。
