...
根据上面的决策树，如果它们没有屈折变化，则不能归入代词。 \citet[\page 277]{Eisenberg2004a} 指出wo（哪里）是一种“没有屈折变化的关系代词”（uninflected relative pronoun），并且指明这种描述与名词性的用法是不同的，因为名词是有屈折变化的成分。由此，他用“关系副词”\isc{副词!关系副词}（relative adverb\is{adverb!relative}）来指称它们（参见 \citew[\S 856, \S 857]{Duden2005-Authors}）。
...
\mytrans{他们正想起这个男人。}
\zl
在对宾语按照格进行分类的同时，更为普遍的做法是区分“直接宾语”\isc{宾语!直接}（direct object\is{object!direct}）和“间接宾语”\isc{宾语!间接}（indirect object\is{object!indirect}）。顾名思义，直接宾语与间接宾语不同，直接宾语的所指直接受到动词指定的动作的影响。带双宾语的动词，如德语的geben（给），宾格宾语就是直接宾语，而与格宾语就是间接宾语。
...
在下面，我会采用“及物”（transitive）的前一个概念，即那些能在被动式中将宾语变换为主语的动词（\egc 德语中的werden）。对于helfen（帮助）类动词，它们带一个主格和一个与格论元；而schlagen（击打）类动词带有一个主语和一个宾格论元。我会用“二位动词”（two-place verb）或“二价动词”\isc{动词!二价}（bivalent verb\is{verb!bivalent}）动词来指称它们。
