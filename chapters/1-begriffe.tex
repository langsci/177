...
也有学者认为我们有能力构建出无限长的句子（\citealp*[\page 117]{NKN2001a}、\citealp[\page 3]{KS2008a-u}、Dan Everett in \citew{OW2012a}），乔姆斯基也这样认为\citep[\page 341]{Leiss2003a}。但这是不正确的，因为每个句子必然要在某个节点结束。即使是在乔姆斯基传统下发展起来的形式语言也没有无限长的句子。不过，有些形式语法可以描述出一系列无限地包含着有限句子的情况。（\citealp[\page 13]{Chomsky57a}）也可以参看 \citew{PS2010a} 和第\ref{Abschnitt-Rekursion} 节中关于语法的递归性和与语言无限论的观点。
}
...
如果我们并不希望语言仅仅是由一个个形式意义对儿构成的，那么必然需要将话语中小的意义成分构成复杂话语的意义的过程。句法就是用来揭示词汇组合的方式和话语的结构的。比如说，主谓一致（subject-verb agreement）\is{agreement}的知识有助于我们解释下面的德语句子：
...
\subsubsection{替换}
如果可以将一个句子内的词语序列替换（substitution）为另一套不同的词语序列\is{substitution test}，而且句子的可接受程度不变，那么就可以证明这些词语序列是一个构成成分。
...
\subsubsection{代词化}
凡是能够由代词所替代的也构成一个成分\is{pronominalization test}。在例（\mex{1}）中，我们可以用代词er（他）替代“der Mann”（这个男人）：
...
如有两组词语序列可以连在一起\is{coordination!-test|(} ，那么每一组词都是一个构成成分。
...
\gll Zufällig war ich in dem Augenblick zugegen, \emph{wo} der Steppenwolf zum erstenmal unser Haus betrat und bei meiner Tante sich einmietete.\footnotemark\\
	 恰巧地 AUX 我 在 DET 时候 出现 CONJ DEF 荒原狼 PREP.DET 第一次 我们的 房子 进入 和 在 我的 姑姑 \textsc{refl} 租房\\
...
\noindent
在例(\mex{0})的答句中，名词ball（球）被省略了。在例（\mex{-1}d）中则没有这种省略形式。我们也可以认为这里是词类发生了变化。如果一个词的词类发生了变化，但是并没有加上可见的词缀，那么，我们可以把这类现象叫做词类活用\is{conversion}。有些语言学家认为转化是词汇派生\is{derivation} 的一个次级类型。但是，问题是Grüne（绿色）就像形容词一样有屈折变化，并且会随着宾语的所指的性的不同而有所变化。
...
一个构成成分或短语的中心语（head）\is{head|(} 决定了这个组成成分或短语的最重要的属性。同时，中心语也决定了这个短语的构成，即中心语要求在短语中有其他要素的共现。如下例所示（中心语用斜体表示）：
...
\gll dass er dem Mann den Aufsatz gibt\\
	 CONJ 他.\nom{} DET.\dat{} 人.\dat{} DET.\acc{} 论文.\acc{} 给\\
\glt `他给那个人这篇论文'
\z
...
\gll dass er den Mann des Mordes bezichtigte\\
	 CONJ 他 DET.\acc{} 人.\acc{} DET.\gen{} 谋杀.\gen{} 控告\\
\glt `他控告了杀人的人'
\ex 
\gll dass er den Mann den Vers lehrte\\
	 CONJ he DET.\acc{} 人.\acc{} DET.\acc{} 诗.\acc{} 教\\
\glt `他教那个人读诗了'
\zl
这类宾语有时也叫做间接宾语。
...
\gll Er liest den ganzen Tag diesen schwierigen Aufsatz.\\
	 他 读 DET.\acc{} 整.\acc{} 天 这.\acc{} 难的.\acc{} 论文\\
\glt `他花了一整天读这篇难懂的论文。'
\ex 
\gll Er gibt den Armen den ganzen Tag Suppe.\\
	 他 给 DET.\dat{} 穷人.\dat{} DET.\acc{} 整天.\acc{} 天 汤\\
\glt `他花了一整天给穷人汤喝。'
\zl
被动态中状语的格不发生变化：
...
\zl
        ...
        \gll Er lässt den lieben Gott `n frommen Mann sein.\\
	     他 让 DET.\acc{} 亲爱的.\acc{} 上帝 一 虔诚的.\acc{} 人 COP\\
        \glt `他完全是不走心（漠不关心）啊。'
        }
        \ex[*]{
        \gll Er lässt den lieben Gott `n frommer Mann sein.\\
	     他 让 DET.\acc{} 亲爱的.\acc{} 上帝 一 虔诚的.\nom{} 人 COP\\
        }
        \zllast}
...
\gll Er hat [der Frau] das Buch gegeben, [die er kennt].\\
     他 AUX \spacebr{}DET 女人 DEF 书 给 \spacebr{}CONJ 他 认识\\
\glt `他把书交给了他认识的那个女人。'
}
\ex[*]{
...
\gll Über dieses Thema habe ich ihn gebeten, einen Vortrag zu halten.\\
      关于 这 题目  AUX 我   他 请求    ART     演讲    AUX hold\\
\glt `我请他做个有关这个题目的演讲。'
\ex 
\gll das Thema, über das ich ihn gebeten habe, einen Vortrag zu halten\\
     DET 题目  关于  我 他 请求  AUX ART 演讲 AUX 作\\
\glt `这是那个我让他做一个演讲的题目'
\zl
