...
也有学者认为我们有能力构建出无限长的句子（\citealp*[\page 117]{NKN2001a}；\citealp[\page 3]{KS2008a-u}；丹 $\cdot$ 埃弗里特（Dan Everett）在 \citew{OW2012a}这个片子中的25分19秒时这样说道），乔姆斯基也这样认为\citep[\page 341]{Leiss2003a}。但这是不正确的，因为每个句子必然要在某个节点结束。即使是在乔姆斯基传统下发展起来的形式语言也没有无限长的句子。不过，有些形式语法可以描述出一系列无限地包含着定式句子的情况。（\citealp[\page 13]{Chomsky57a}）也可以参看 \citew{PS2010a}和\ref{Abschnitt-Rekursion}中关于语法的递归性\isc{递归}\is{recursion}和语言无限论的观点。
}
...
\mytrans{因为没人想到会下雨。}
\zl
\hspace{+2em}赫勒（Höhle）还讨论了空间位置理论的历史发展。
}
