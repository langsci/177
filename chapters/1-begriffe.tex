...
如果我们并不希望语言仅仅是由一个个形式意义对儿构成的，那么必然需要将话语中小的意义成分构成复杂话语的意义的过程。句法就是用来揭示词汇组合的方式和话语的结构的。比如说，主谓一致\isc{主谓一致}（subject-verb agreement）\is{agreement}的知识有助于我们解释下面的德语句子：
...
为什么要对语言进行形式化\isc{形式化|(} （formalization）\is{formalization|(} 的描述呢？我们先来看两条经典的论述：
...
\zl
%
这类测验叫做替换测试（substitution test）。\isc{替换测试}\is{substitution test}
...
如有一组词语序列\isc{变换测试(}\isc{移位测试|(}\is{permutation test|(}\is{movement test|(}可以移动，而不会影响到其所在句子的合格性，那么这就意味着这个词语序列组成了一个成分。
