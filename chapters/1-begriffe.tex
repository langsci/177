...
如有两组词语序列可以连在一起\isce{并列!并列测试|(}{coordination!-test|(}，那么每一组词都是一个组成成分。
...
\isce{并列!并列测试|)}{coordination!-test|)}
%\ea
...
事实是，我们不能在上例中应用替换和疑问测试的方法的原因是，这些例子中的es都是无指的。我们可以将es替换为另一个虚位成分，比如说das。如果将虚位成分替换为一个有指的表达式，我们可以得到一个不同的语义解释。而语义上的空位概念或者用代词来指代都是没有意义的。\isce{代词!虚位|)}{pronoun!expletive|)}
...
例（\mex{1}）中的并列结构\isce{并列|(}\iscesub{并列}{coordination|(}{coordination}{并列测试}{-test}也被证明是有问题的：
...
有些学派认为补足语包括主语，即补足语的概念等同于论元（参见\citet[\page 342]{Gross2003a}）。有些学者将变位动词的主语看作是补足语（\citealp{Pollard90a-Eng}; \citealp[\page 376]{Eisenberg94b}）。}不是所有的中心语都需要主语（参见\citet[\S~3.2]{MuellerLehrbuch1}）。由此，中心语所带论元的数量也可以相当于中心语所带补足语的数量。\isc{论元|)}\isc{附加语|)}\isce[|)]{价}{argument}\is{adjunct|)}\is{valence|)}
