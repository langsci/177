...
\z
在理论文献中，人们普遍认为“keine Einigung über den Abbau der Agrarsubventionen”构成了一个成分，并且它可以在一定的情况下进行“分裂”\isc{NP-分裂}。
...
词类中最为重要的有动词（verb）、名词（noun）\isc{名词}\is{noun}、形容词（adjective）\isc{形容词}\is{adjective}、介词（preposition）\isc{介词}\is{preposition}和副词（adverb）\isc{副词}\is{adverb}。很多年前，研究德语的学者通常会区分动作类词（action words），描述类词（describing words）和命名类词（naming words）（参看第~\ref{sec-tesniere-pos} 节有关\tes's的范畴系统）。但是这些说法都被证明是有问题的，如下所示：
...
根据上面的决策树，如果他们没有屈折变化，则不能归入代词。\citet[\page 277]{Eisenberg2004a} 指出wo是一种没有屈折变化的关系代词，并且指明这种描述与名词性的用法是不同的，因为名词是有屈折变化的元素。由此，他用关系副词（relative adverb）\isc{关系副词}\is{adverb!relative}来指称他们（参见\citew[\S 856, \S 857]{Duden2005-Authors}）。
...
正如我们所看到到，至今仍然难以区分屈折（inflection）与派生（derivation）\isc{派生}\is{derivation} （产生新的词位）。\citet*[\page263--264]{SWB2003a} 认为，英语\il{英语}\il{English}中的现在分词与过去分词属于派生形式，而在法语\il{法语}\il{French}中则随着性数而发生屈折变化。
...
中心语与其他成分的组合叫做中心语的投射（projection）\isc{投射}\is{projection}。保证短语合法性所需的所有要素的投射叫做最大投射。一个句子就是定式动词的最大投射。
...
那些决定中心语能带什么成分的特征叫做中心语特征（head features）\isc{中心语特征}\is{head feature}。这些特征被认为是由中心语投射（projected）\isc{中心语投射}\is{projection!of features} 出来的。
...
我们用（\mex{0}）来解释逻辑价。不过，逻辑价有时与句法价不同。对于动词rain（下雨）来说，它需要一个虚指代词（expletive pronoun）\isc{虚指代词}\is{pronoun!expletive}作为论元。德语中的自反身动词\isc{自反身代词}\is{verb!inherent reflexives}也是一样的，如sich erholen（复元）。
...
\zl
动词管辖（governs）\isc{管辖}\is{government}论元的格\isc{格}\is{case}属性。
...
论元通常可以划分为主语\isc{主语}\is{subject}和补足语\isc{补足语}\is{complement}。\footnote{有些学派认为补足语包括主语，即补足语的概念等同于论元（参看\citealp[\page 342]{Gross2003a}）。有些学者将变位动词的主语看作是补足语（\citealp{Pollard90a-Eng}; \citealp[\page 376]{Eisenberg94b}）} 不是所有的中心语都需要主语（参看see \citealp[Section~3.2]{MuellerLehrbuch1}）。由此，中心语所带论元的数量可以与中心语所带补足语的数量具有相关性。
...
尽管我认为读者对主语（subject）\isc{主语|(}\is{subject|(} 已经有了清晰的认识，但是给主语下一个跨语言的定义绝不是一件小事。对于德语来说，\citet{Reis82} 提出下面的句法属性作为对主语对界定：
...
下面，我就只用宾语的格属性来指称，而避免使用直接宾语（direct object）\isc{直接宾语}\is{object!direct}和间接宾语（indirect object）\isc{间接宾语}\is{object!indirect}的术语。
...
\addlines
我们可以按照动词所带论元成分的数量与属性来对动词进行分类。一方面，那些可以带宾语且其宾语能变换为被动式的主语多动词叫做及物动词（transitive）\isc{及物动词}\is{verb!transitive}，如love（爱）或beat（击打）这类动词。另一方面，不及物动词不能带宾语，或者其在被动式中不能变成主语，如schlafen（睡觉），helfen（帮忙）或gedenken（纪念）。及物动词还包括双及物动词，如geben（给）和zeigen（展示）。
...
在下面，我会采用及物的前一个概念，即那些能在被动式中将宾语变换为主语的动词。对于helfen（帮助）类动词，它们带一个主格和一个与格论元；而schlagen（击打）类动词带有一个主语和一个宾格论元。我会用二位动词或二价动词（bivalent verb）\isc{二价动词}\is{verb!bivalent}动词来指称它们。
...
\is{verb!-first}\is{verb!-second}\is{verb!-final}\isc{missing verb!-first}\isc{missing
  verb!-second}\isc{missing verb!-final}
