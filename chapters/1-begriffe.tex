...
\gll Sie lachen.\\
     他们 笑\\
\zl
德语也有过去式、命令式、虚拟语气和不定式（分词和带zu与不带zu的不定式）的形态变化。这些形式就构成了动词的屈折词形变化表\iscesub{屈折}{inflection}{词形变化表}{-paradigm}。时态\isce{时态}{tense} （现在式\isce{现在式}{present}、过去式\isce{过去式}{preterite}、将来式\isce{将来式}{future}）、情态\isce{情态}{mood} （陈述语气\isce{陈述语气}{indicative}、虚拟语气\isce{虚拟语气}{subjunctive}、命令语气\isce{命令语气}{imperative}）、人称\isce{人称}{person}（第1，2，3人称）和数\isce{数}{number}（单数\isce{单数}{singular}、复数\isce{复数}{plural}）都在屈折的词形变化表中有所体现。在有些词形变化中这些形式有所重合，如例（\mex{0}c）、例（\mex{0}e）、例 （\mex{0}d）和例 （\mex{0}f）所示。
...
这些形式传统上属于动词的词形变化\iscesub{屈折}{词形变化表}{inflection}{paradigm}。
...
\mytrans{他给那个男人这篇论文}
\z
对于三价动词（带有三个论元的动词）来说，我们可以看到动词要么可以带一个属格宾语\isce{属格}{genitive}（\mex{1}a），要么对于带宾格的直接宾语来说，再带一个宾格\iscesub{格}{case}{宾格}{accusative}宾语（\mex{1}b）：
