...
\gll that Max thinks that Julius knows that Otto claims that Karl suspects that Richard confirms that Friederike is laughing\\
      \textsc{comp} 马克斯 认为 \textsc{comp} 尤利乌斯 知道 \textsc{comp} 奥托 声称 \textsc{comp} 卡尔 怀疑 \textsc{comp} 里夏德 承认 \textsc{comp} 弗里德里克 \textsc{aux} 笑\\
      \mytrans{马克斯认为尤利乌斯知道奥托声称卡尔怀疑里夏德承认弗里德里克在笑。}
\zl
...
\gll that          Max thinks [that                 Julius knows [that Otto claims [that Karl suspects [that Richard confirms [that Friederike is laughing]]]]]\\
     \textsc{comp} 马克斯 认为    \spacebr\textsc{comp} 尤利乌斯 知道  \spacebr\textsc{comp} 奥托 声称 \spacebr\textsc{comp} 卡尔 怀疑 \spacebr\textsc{comp} 里夏德 承认 \spacebr\textsc{comp} 弗里德里克 \textsc{aux} 笑\\
\z
...
也有学者认为我们有能力构建出无限长的句子（\citealp*[\page 117]{NKN2001a}；\citealp[\page 3]{KS2008a-u}；Dan Everett在 \citew{OW2012a}这个片子中的\zhtime{25:19}），乔姆斯基也这样认为\citep[\page 341]{Leiss2003a}。但这是不正确的，因为每个句子必然要在某个节点结束。即使是在乔姆斯基传统下发展起来的形式语言也没有无限长的句子。不过，有些形式语法可以描述出一系列无限地包含着定式句子的情况。（\citealp[\page 13]{Chomsky57a}）也可以参看 \citew{PS2010a}和\ref{Abschnitt-Rekursion}中关于语法的递归性\isc{递归}\is{recursion}和语言无限论的观点。
}
...
\gll Ich festhalte die. \\
    % I \particle.hold them\\\jambox{(Friederike, 2;6)}\\
     我 紧-握 他们\\\jambox{（弗里德里克，2岁6个月大）}
\glt 想说：\quotetrans{我紧紧地握住了。}
\z
弗里德里克（Friederike）说出这句话的时候，正处于习得德语定式动词（finite verb）的占位规则（即定式动词位于第二位）的阶段。但是，她在说这句话时将整个动词放在了第二位，其中就包括可分前缀fest（紧）。正确的用法应该是将这个可分前缀放在小句的末尾。
...
%\noindent
在下一节，我会介绍几种不同的测验方法来判断哪些词与其他词相比是“在一起的”。每当我说到一个“词语序列”\isc{词语序列}（word sequence\is{word sequence}）的时候，通常是指一个任意的线性词语序列，它并不必须具有句法或语义上的联系，比如说例（\mex{0}）中students read during（学生们读在）。而由一组词组成的结构单位通常叫做“短语”\isc{短语}（phrase\is{phrase}）。短语由词构成，如this time（这个时候），或者由词与短语组成，如during this time（在这个时候）。短语内的部分和短语本身叫做“组成成分”\isc{组成成分}（constituents\is{constituent}）。所以说，在图\ref{Abbildung-Schachteln}中，盒子中的所有元素都是句子的组成成分。
...
\gll Peter versucht, [das Buch zu lesen].\\
	 彼得 试图 \spacebr{}\textsc{art}.\textsc{def} 书 \textsc{inf} 读\\
\mytrans{彼得试着读这本书。}
\ex 
\gll Klaus versucht das auch.\\
	 克劳斯 试图 \textsc{art}.\textsc{def} 也\\
\mytrans{克劳斯也试着这么做。}
\zl
...
\gll Keine Einigung erreichten Schröder und Chirac über den          Abbau der~~~~~~~~~~          Agrarsubventionen.\footnotemark\\
     没有一个    一致      达成        施罗德 和  希拉克 关于  \textsc{art}.\textsc{def} 减少  \textsc{art}.\textsc{def} 农业.补贴\\
\footnotetext{《每日新闻》（\emph{tagesschau}）, \zhdate{2002/10/15}，晚八点。}
%tagesschau, 15.10.2002, 20:00.
\mytrans{施罗德和希拉克没能就缩减农业补贴达成一致。 }
\ex 
\gll [Über           den Abbau     der    Agrarsubventionen]     erreichten Schröder und Chirac keine Einigung.\\
     \spacebr{}关于 \textsc{art}.\textsc{def} 减少 \textsc{art}.\textsc{def} 农业.补贴 达成 施罗德 和 希拉克 没有一个 一致\\
\zl
...
\gll Schröder und Chirac erreichten [keine Einigung über den Abbau der~~~~~~~~~~~~~~ Agrarsubventionen].\\
     施罗德 和 希拉克 达成 \spacebr{}没有一个 一致 关于 \textsc{art}.\textsc{def} 减少 \textsc{art}.\textsc{def} 农业.补贴\\
\z
...
\gll {}[Keine Einigung über den Abbau der Agrarsubventionen] erreichten Schröder und Chirac.\\
     \spacebr{}没有一个 一致 关于 \textsc{art}.\textsc{def} 减少 \textsc{art}.\textsc{def} 农业.补贴  达成 施罗德 和 希拉克\\
\z
...
\footnotetext{%
        《日报》（\emph{taz berlin}），\zhdate{1998/7/10}，第22页。
      }
...
\gll Karl hat sich nicht erholt.\\
	 卡尔 \textsc{aux} {\refl} 不 痊愈\\
\mytrans{卡尔还没有恢复。}
}
\ex[*]{
\gll Sich hat Karl nicht erholt.\\
     \refl{} \textsc{aux} 卡尔 不 痊愈\\
}
\zl
...
连词（conjunction）\isc{连词}\is{conjunction}分为从属连词和并列连词。并列连词包括und（和）与oder（或者）。在并列结构中，两组具有同样句法属性的词被组合起来。他们在形式上是彼此有联系的。dass与weil（因为）这两个连词是从属连词，因为他们引导的小句从属于一个更大的句子。
...
\gll Klaus glaubt, dass er lügt.\\
	 克劳斯 认为 \textsc{comp} 他 说谎\\
\mytrans{Klaus认为他在说谎。}
\ex 
\gll Klaus glaubt ihm nicht, weil er lügt.\\
	 克劳斯 相信 他 不 因为 他 说谎\\
\mytrans{克劳斯不相信他，因为他在说谎。}
\zl
...
\mytrans{我想知道你认识谁的姐妹。}
\zl
根据杜登的分类标准，这些词被看作是“关系冠词”（Relativartikelwort）和“疑问冠词”（Interrogativartikelwort）。他们通常被看作是关系代词和疑问代词的一部分（参见 \citew[\page 229]{Eisenberg2004a}）。如果按照艾森贝格（Eisenberg）提出的术语来看，这些词的归类问题是不具有争议的，因为它没有对冠词、代词和名词进行区分，而是将它们都归为名词类。但是对于那些提出需要区分出冠词和代词的学者来说，疑问代词也是一个经常探讨的问题，即它们可以作为冠词使用，也可以替代一个名词短语。
...
\gll Peter schläft im Büro.\\
     彼得 睡觉 在.\textsc{art}.\textsc{def} 办公室\\
\mytrans{彼得在办公室里睡觉。}
\ex 
...
中心语和论元的这种关系叫做“选择”（selection\isc{选择}\is{selection}）和“价”（valence\isc{价|(}\is{valence|(}）。价这个术语是从化学借来的。原子与原子组合成分子具有不同程度的稳定性。电子层的排列方式对这种稳定性起到了重要的作用。如果一个原子与其他原子相组合，这样它的电子层就被占满了，那么这样就可以得到一个稳定的连接。价告诉我们要构成一个元素需要多少个氢原子。在构成水（H$_2$O）的时候，氧原子是二价。我们可以把元素按照价进行分类。按照门捷列夫（Mendeleev）的方法，带有一个特定价的元素被安排在元素周期表的同一个栏中。
...
\gll Ich glaube, dass Klaus erholt.\\
	 我 想 \textsc{comp} 克劳斯 恢复\\
\glt 想说：\quotetrans{我相信克劳斯在恢复之中。}	 
}
\zl
...
\gll the man {\em who\/} {\em Mary\/} {\em loves\/}\\
      \textsc{art}.\textsc{def}  男人 \textsc{rel} 玛丽  爱\\
\mytrans{玛丽爱着的那个男人}       
\zl
...
\gll Klaus behauptet, den Männern zu helfen.\\
	 克劳斯 声称  \textsc{art}.\textsc{def}.\dat{} 男人.\dat{} \textsc{inf} 帮助\\
\mytrans{克劳斯声称要帮助那些男人。 }
}
\ex[]{
\gll Klaus behauptet, dass er den Männern hilft.\\
	 克劳斯 声称 \textsc{comp} 他 \textsc{art}.\textsc{def}.\dat{} 男人.\dat{} 帮助\\
\mytrans{克劳斯声称他在帮助那些男人。}
}
\ex[*]{
...
\gll Dass er Maria geheiratet hat, gefällt mir.\\
	 \textsc{comp} 他 玛丽亚 娶 \textsc{aux} 高兴 我\\
\mytrans{我很高兴他娶了玛丽亚。}
\zl
\eal
\ex
...
这种术语混淆的情况导致了对乔姆斯基的评论的误解，即使它们是由一些知名的语言学家提出来的\citep[\page 59]{CJ2005a}。乔姆斯基指出，英语助动词be带动词的被动态只能用在及物动词上。库里卡弗和杰肯道夫则认为这是不对的，因为还有及物动词不能变换为被动式，如weigh（称重）和cost（花费）。 
...
库里卡弗和杰肯道夫这里说的“及物”（transitive）是指带两个论元的动词。如果我们只把那些动词宾语可
以变换为被动式主语的动词看作是及物动词，那么weigh（称重）和cost（花费）都不能算作是及物动词，这样
库里卡弗和杰肯道夫的观点就站不脚了。\footnote{%
...
\gll (Peter hat erzählt,) dass er das Eis gegessen \emph{hat}.\\
     \hspaceThis{(}彼得 \textsc{aux} 告诉 \textsc{comp} 他 \textsc{art}.\textsc{def} 雪糕 吃 \textsc{aux}\\
\mytrans{彼得说他把雪糕吃完了。}
\ex 
\gll \emph{Hat} Peter das Eis gegessen?\\
	 \textsc{aux} 彼得 \textsc{art}.\textsc{def} 雪糕 吃\\
\mytrans{彼得吃完雪糕了吗？}
\ex 
\gll Peter \emph{hat} das Eis gegessen.\\
	 彼得 \textsc{aux} \textsc{art}.\textsc{def} 雪糕 吃\\
\mytrans{彼得吃完雪糕了。}
\zl
...
\gll Ein Treppenwitz der    Musikgeschichte, daß die Kollegen   von Rammstein vor    fünf Jahren noch im      Vorprogramm   von Sandow spielten.\footnotemark\\
	 一 讽刺 \textsc{art}.\textsc{def} 音乐历史 \textsc{comp} \textsc{art}.\textsc{def} 成员 ……的 德国战车 在……前 五 年 仍然 在……中.\textsc{art}.\textsc{def} 开场 ……的 萨多 表演\\
\footnotetext{%
         低语与沉默，《日报》（taz），\zhdate{1999/07/12}，第14页。 %war das englisch? 07.12.1999, p.\,14
}
\mytrans{音乐历史上具有讽刺意味的一件事是，五年前，德国战车的成员还在为萨多作开场演出。}
\zl
...
\gll Ein Treppenwitz der Musikgeschichte ist, dass die Kollegen von~~~~~~~~~~~ Rammstein vor fünf Jahren noch im Vorprogramm von Sandow spielten.\hspace{-5pt}\\
	 一 讽刺 \textsc{art}.\textsc{def} 音乐历史 \textsc{cop} \textsc{comp} \textsc{art}.\textsc{def} 成员 ……的 德国战车 在……前 五 年 仍然 在……中.\textsc{art}.\textsc{def} 开场 ……的 萨多 表演\\
    %\par}     
\mytrans{五年前，德国战车的成员还在为萨多作开场演出是音乐史上一件具有讽刺意义的事件。}
\zl
...
\gll Lotti, die wo eine tolle Sekretärin ist, hat ein paar merkwürdige~~~~~~ Herren empfangen.\\
     洛蒂  \textsc{rel} 哪儿 一 伟大 秘书 \textsc{cop} \textsc{aux} 一 些 奇怪 男士 欢迎\\
\mytrans{洛蒂作为一名出色的秘书，接待了几位可疑的男士。}
\ex 
...
\gll Der Mann liebt eine Frau, den Peter kennt.\\
     \textsc{art}.\textsc{def} 男人  爱   一 女人 \textsc{rel} 彼得 认识\\
\mytrans{彼得认识的那个男人爱着这个女人。}
\ex 
...
\gll Dass Peter nicht kommt, ärgert Klaus.\\
	 \textsc{comp} 彼得 不 来 惹恼 克劳斯\\
\mytrans{彼得不能来的事实惹恼了克劳斯。}
\ex 
...
\mytrans{因为没人想到会下雨。}
\zl
\hspace{+2em}霍勒（Höhle）还讨论了空间位置理论的历史发展。
}
