...
\citet[\page 926]{Citko2008a}提出了一个分析，其中所有的子结点都可以为父结点贡献力量。结果是像\{~P, \{ D, N \} \}的复杂标记。这是一个高度复杂的数据结构，而且Citko并没有给出任何信息，它所包含的相关信息是如何被接触到的。带有标记\{~P, \{ D, N \} \}的宾语是P、D，还是N呢？我们可以说P有优先性，因为它在嵌套最少的集合中，但是D和N在一个集合中。冲突的特征怎么办呢？介词如何选择DP决定了\{ D, N \}是D，还是N？任何情况下，很清楚的是形式化会包括递归关系，这些递归关系发掘出子集的成分以获得它们的特征。这增加了整体的复杂度，而且在HPSG中不受推崇，因为它对每个语言对象都使用了一种词性。
}
...
\noindent
\hspace{2em}有关这篇文章的更多评论，参见\ref{Abschnitt-Labeling}和\ref{Abschnitt-Spezfikatoren-MP}。
}
