...
MG和HPSG更深一层的区分在于中心语-填充语不是分析长距离依存的唯一模式。正如第\pageref{fn-Kayne-Extraposition}页的脚注\ref{fn-Kayne-Extraposition}所指出的， 右侧（外置\isce{外置}{extraposition}）和前置都有错位的情况。尽管这些问题一定会被分析为长距离依存，它们与其他长距离依存在很多方面都不同（参见\ref{Abschnitt-Fernabhängigkeiten}）。在HPSG框架下有关外置的分析，请参考 \citew{Keller95b}、 \citew{Bouma96}和 \citew{Mueller99a}。
