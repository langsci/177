...
跟上一章介绍的管辖与约束理论一样，最简方案\isc{最简方案（MP）|(}\is{Minimalist Program (MP)|(}由Noam Chomsky在波士顿的麻省理工学院提出。 \citet{Chomsky93b-u,Chomsky95a-u}指出，我们应该认真对待语言进化的问题，而且我们应该能够解释说明语言知识是如何成为我们天赋的一部分的。为了回答这些问题，他认为我们应该重新关注那些语言学分析机制所需的最精简假设的理论发展，并最终得到那些更少地针对个别语言的内在知识的模型。
...
在\ref{sec-passive-gb}\isc{范畴!功能!v@\textit{v}|(}\is{category!functional!v@\textit{v}|(}中，我用\xbarc 结构来表示双及物动词，该动词带上宾格宾语构成一个\vbarc，然后跟与格宾语构成一个\vbarc。这种动词带宾语构成\vbarc 的二元结构以及平铺结构的分析未被许多GB理论和最简方案的实践者所采纳，因为反身代词与否定极项的约束现象并不兼容这些分支结构。
...
\isc{范畴!功能!v@\textit{v}|)}\is{category!functional!v@\textit{v}|)}

\subsection{CP、TP、\vPc 和VP}
%\subsection{CP, TP, \vP, VP}
\label{sec-CP-TP-vP-VP}

\ref{sec-GB-CP-IP-System-English}分析GB理论中的CP/IP系统。在最简方案的发展过程中，屈折短语被分成几个功能性投射\citep{Chomsky89a-u}，其中只有TP\isc{范畴!功能!时}\is{category!functional!Tense}在目前的最简方案的分析中有所涉及。所以，最简方案的TP对应于GB分析中的IP。除了这一变化，CP/IP分析的核心思想被转化为英语的最简方案分析。这一小节将先讨论触发移位的特殊特征（\ref{sec-epp-features}），然后是格指派（\ref{sec-case-mp}）。
...
\end{figure}%
限定词短语（DP）the book是read的宾语，然后核查read的D特征。小\emph{v}选择了主语Anna，因为T有着强势的D特征（由星号“*”\isc{*}\is{*}标记），Anna一定不能在\vPc 内部，但是会移动到TP的限定语位置上。
...
介入关系\isce{介入关系}{intervention}：在结构[X \ldots{} Z \ldots{} Y]中，Z介于 X与Y中间，当且仅当X c-统制\isc{c-统制}\is{c"=command} Y。
...
\end{figure}%
一般认为，这个小\emph{v}的非宾格变量在被动的分析中起到了重要的作用。非宾格动词与被动化的动词相似，因为它们都有一个主语，这些主语在某种程度上也有宾语的特征。小\emph{v}的特别版本由被动式中心语werden选择，这就构成了一个被动短语\isc{范畴!功能!被动}\is{category!functional!Passive}（缩写为PassP）。参见图\ref{fig-passive-schlagen-mp}中针对例（\mex{1}）的分析：
...
在90年代初期，Chomsky对GB理论的基本假设进行了重新思考，并且只保留了那些绝对必要的部分。在最简方案中，Chomsky说明了对GB理论进行修正的核心动因\citep{Chomsky93b-u,Chomsky95a-u}。直到90年代早期，格语法\isce{格语法}{Case Theory}、题元准则\isc{$\theta$-准则@$\theta$-准则}\is{theta-criterion@Theta"=Criterion}、\xbartc、邻接原则\isce{邻接原则}{Subjacency}、约束理论\isce{约束理论}{Binding Theory}、控制理论\isce{控制理论}{Control Theory}等都属于语言的内在机制\citep[\page 804]{Richards2015a}。当然，这就涉及到了非常具体的语言知识是如何进入我们的基因组的问题。最简方案沿着这一思路，并且试图解释更为普遍的认知原则下的语言属性，以及减少具体的内在语言知识的数量。比如说，深层结构和表层结构\isc{D-结构}\is{D"=structure}\isc{S-结构}\is{S"=structure}之间的区别被取消了。移位仍是一种操作，但是只直接用来构建子结构，而不是在一个完整的D"=结构之后完成。语言之间的差别在于这种移位是可见的还是不可见的。
...
移位必须由特征核查所允准这一思想导致了（静态）功能中心语数量的膨胀\isc{范畴!功能|(}\is{category!functional|(}。\footnote{%
...
%}
\citet[\page 147]{Chomsky95a-u}\label{Seite-AgrO}采纳了 \citet{Pollock89a-u}的观点，并认为所有语言都有主宾一致关系\iscesub{主谓一致}{agreement}{宾语}{object}和否定（AgrS\isc{范畴!功能!AgrS}\is{category!functional!AgrS}、AgrO\isc{范畴!功能!AgrO}\is{category!functional!AgrO}、Neg\isc{范畴!功能!Neg}\is{category!functional!Neg}）的功能投射。\footnote{%
...
%}
 \citet[\page 78]{Sternefeld95a}、 \citet[\page 103]{Stechow96a}和 \citet[\page 100--101, 124]{Meinunger2000a}区分了直接宾语和间接宾语（AgrO\isc{范畴!功能!AgrO}\is{category!functional!AgrO}、AgrIO\isc{范畴!功能!AgrIO}\is{category!functional!AgrIO}）的两个一致关系的位置。同样对于AgrS\isc{范畴!功能!AgrS}\is{category!functional!AgrS}、AgrO\isc{范畴!功能!AgrO}\is{category!functional!AgrO}和Neg\isc{范畴!功能!Neg}\is{category!functional!Neg}， \citet{BS97a-u}提出了Share\isc{范畴!功能!共享}\is{category!functional!Share}和Dist\isc{范畴!功能!Dist}\is{category!functional!Dist}来将英语的的辖域现象解释为在LF层的特征驱动移位。对于没有空成分或移位的辖域现象的分析，参见\ref{Abschnitt-leere-Elemente-Semantik}。 \citew[\page 13]{BG2005a}提出了$-$PolP\isc{范畴!功能!$-$Pol}\is{category!functional!$-$Pol}、$+$PolP\isc{范畴!功能!+Pol}\is{category!functional!+Pol}和
\%PolP\isc{范畴!功能!\%Pol}\is{category!functional!\%Pol}范畴来讨论极性问题。
...
\citet[\page 76]{Webelhuth95a}对功能投射进行了概述，并在1995年提出了以下范畴的定义，包括AgrA\isc{范畴!功能!AgrA}\is{category!functional!AgrA}
AgrN\isc{范畴!功能!AgrN}\is{category!functional!AgrN}、AgrV\isc{范畴!功能!AgrV}\is{category!functional!AgrV}、Aux\isc{范畴!功能!Aux}\is{category!functional!Aux}、附着态\isc{范畴!功能!附着态}\is{category!functional!Clitic Voices}、性\isc{范畴!功能!性}\is{category!functional!Gender}、敬语\isc{范畴!功能!敬语}\is{category!functional!Honorific}、$\mu$\isc{范畴!功能!$\mu$}\is{category!functional!$\mu$}、数\isc{范畴!功能!数}\is{category!functional!Number}、人称\isc{范畴!功能!人称}\is{category!functional!Person}、谓词\isc{范畴!功能!谓词}\is{category!functional!Predicate}、时\isc{范畴!功能!时}\is{category!functional!Tense}和Z\isc{范畴!功能!Z}\is{category!functional!Z}。
...
除了AdvP\isc{范畴!功能!副词}\is{category!functional!Adverb}、NegP\isc{范畴!功能!Neg}\is{category!functional!Neg}、AgrP、FinP、TopP和ForceP， \citet*{WHBH2007a-u}提出了外围TopP\isc{范畴!功能!OuterTop}\is{category!functional!OuterTop}。
...
%
\citet[\page 31]{Poletto2000a-u}针对意大利语\ilce{意大利语}{Italian}中附着语的位置，提出了HearerP\isc{范畴!功能!听话者}\is{category!functional!Hearer}和SpeakerP\isc{范畴!功能!说话者}\is{category!functional!Speaker}。
...
\lspbottomrule
\end{tabular}
\isc{范畴!功能!语气}\is{category!functional!Mood}\isc{范畴!功能!T}\is{category!functional!T}\isc{范畴!功能!Mod}\is{category!functional!Mod}\isc{范畴!功能!Asp}\is{category!functional!Asp}\isc{范畴!功能!完成体}\is{category!functional!Perfect}%
...
图\ref{Tabelle-Cinque}显示了小句域内的功能中心语。 \citet[\page 96, 99]{Cinque94a-u} 也说明了带有投射的形容词的顺序：质量\isc{范畴!功能!质量}\is{category!functional!Quality}、大小\isc{范畴!功能!大小}\is{category!functional!Size}、形状\isc{范畴!功能!形状}\is{category!functional!Shape}、颜色\isc{范畴!功能!颜色}\is{category!functional!Color}和国籍\isc{范畴!功能!国籍}\is{category!functional!Nationality}。这些范畴和它们的语序被认为与普遍语法是有关系的（第100页）。
...
\end{figure}%
主语和宾语分别作为vP和VP内部的论元而生成。主语移到了主语短语\isc{范畴!功能!主语}\is{category!functional!Subj}的限定语位置上，而宾语移到了宾语短语\isc{范畴!功能!宾语}\is{category!functional!Obj}的限定语位置上。动词性投射（VP$_k$）移到了助动词前，并移到了包括助动词的短语的限定语位置上。SubjP和ObjP的唯一功能在于给各自移位提供落点。对于宾语在主语前的句子而言，Laenzlinger认为宾语移到了话题短语的限定语位置上。图\ref{Abbildung-Remnant-Movement-Satzstruktur}只包含一个ModP和一个AspP，尽管Laenzlinger认为Cinque提出的所有中心语在所有德语小句的结构中都是存在的。 \citet{Zwart1994a-u}针对荷兰语\ilce{荷兰语}{Dutch}提出了一个类似的分析，宾语和主语从动词位于末位的VP移到了Agr位置上。
...
针对Kayne模型的争论，参见 \citew{Haider2000a}。Haider指出，Kayne类的理论对德语作出了不正确的判断（比如说对于选择的副词和第二谓词的位置，以及动词的复杂构成），由此就无法证明该理论可以解释所有的语言的论断。 \citet[\S~4]{Haider97a}指出，由 \citet{Pollock89a-u}、 \citet{Haegeman95a-u}和其他人提出的空否定\isc{范畴!功能!Neg}\is{category!functional!Neg}中心语的论断是有问题的。参见 \citew{Bobaljik99a}关于Cinque的副词投射的论断的讨论。
...
进而，还需要指出SubjP和ObjP、TraP\isc{范畴!功能!Tra}\is{category!functional!Tra}（及物短语）和IntraP\isc{范畴!功能!Intra}\is{category!functional!Intra} （不及物短语）（\citealp[\page 1745]{Karimi-Doostan2005a}）和TopP\isc{范畴!功能!Top}\is{category!functional!Top} （话题短语）、DistP\isc{范畴!功能!Dist}\is{category!functional!Dist} （数量短语）、AspP\isc{范畴!功能!Asp}\is{category!functional!Asp} （情态短语） （\citealp[\page 22]{EKiss2003a-u}；\citealp[\page 35]{Karimi2005a}）、PathP\isc{范畴!功能!PathP}\is{category!functional!PathP}和PlaceP\isc{范畴!功能!PlaceP}\is{category!functional!PlaceP}\citep[\page 246]{Svenonius2004a-u}这些范畴符号都包含了语法功能\isce{语法功能}{grammatical function}、配价、信息结构和语义的信息。\footnote{%
...
支持 \citet{Cinque99a-u}理论的观点是有问题的，原因是：Cinque认为范畴AdverbP\isc{范畴!功能!副词}\is{category!functional!Adverb}
...
与前述我们讨论的局部性问题相关的是附着形式的位置的特殊功能投射的假设：如果使用SpeakerP\isc{范畴!功能!说话者}\is{category!functional!Speaker} ，那么第一人称单数的附着形式就可以移到正确的限定语位置上，而对于HearerP\isc{范畴!功能!听话者}\is{category!functional!Hearer}来说，第二人称附着形式可以移到正确的位置上\citep[\page
...
最后，对于特征驱动的移位来说有一个概念上的问题，这一问题由Gisbert Fanselow指出：\mbox{} \citet[\page 27]{Frey2004a}\isce[|(]{话题}{topic}\isce[|(]{焦点}{focus}\isce[|(]{信息结构}{information structure}提出了KontrP\isc{范畴!功能!Kontr}\is{category!functional!Kontr}（对比短语），并且 \citet{Frey2004b-u}还提出了TopP\isc{范畴!功能!Top}\is{category!functional!Top}（话题短语）（参见 \citew{Rizzi97a-u}提出的意大利语\ilce{意大利语}{Italian}的TopP和FocP\isc{范畴!功能!Foc}\is{category!functional!Foc}（焦点短语）、 
...
\isc{范畴!功能|)}\is{category!functional|)}\isce[|)]{信息结构}{information structure}\isce[|)]{附加语}{adjunct}
...
\mytrans{我读了你写的东西。}
\zl
例（\mex{0}b）中的what you wrote是所谓的自由关系小句\isc{关系小句!自由关系小句|(}\is{relative clause!free|(}。
...
%}   
但是，一旦在分析中允许批准使用空中心语，就不需要\pref{Label1}到\pref{ex-what-you-wrote}的应用了，因为应用可以导致针对\pref{ex-i-read-what-you-wrote}的两个分析：一个是空名词性中心语，一个是被直接标记为DP的\pref{ex-what-you-wrote}。有人可能会指出，在许多可能的推导式中，最为精简的会胜出，但是转移派生限制\isce{转移派生限制}{transderivational constraint}的假设会得到不想要的结果\citep[\S~5]{Pullum2013a}。\isc{关系小句!自由关系小句|)}\is{relative clause!free|)}
...
在前面两节中，我们讨论了Chomsky有关标记的界定问题以及词汇项的并列问题。在下面一节中，我将探讨Stabler对最简方案的合并的界定，他的界定是与标记相关的，而且其中一个版本没有涉及到上面讨论的问题。我将指出他的形式化方法与HPSG的表示非常一致。\isc{X-理论@\xbar-理论|)}\is{X-theory@\xbar-theory|)}
...
它对一价动词\isc{动词!一价|(}\is{verb!monovalent|(}的处理是不正确的。如果动词与DP组合，按照\pref{Definition-EM}中有关外部合并的定义，就会将DP看作是补足语\footnote{%
