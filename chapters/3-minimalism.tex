...
\ref{sec-GB-CP-IP-System-English}分析GB理论中的CP/IP系统。在最简方案的发展过程中，屈折短语被分成几个功能性投射\citep{Chomsky89a-u}，其中只有TP\isce{范畴!功能!时}{category!functional!Tense}在目前的最简方案的分析中有所涉及。所以，最简方案的TP对应于GB分析中的IP。除了这一变化，CP/IP分析的核心思想被转化为英语的最简方案分析。这一小节将先讨论触发移位的特殊特征（\ref{sec-epp-features}），然后是格指派（\ref{sec-case-mp}）。
...
\end{figure}%
一般认为，这个小\emph{v}的非宾格变量在被动的分析中起到了重要的作用。非宾格动词与被动化的动词相似，因为它们都有一个主语，这些主语在某种程度上也有宾语的特征。小\emph{v}的特别版本由被动式中心语werden选择，这就构成了一个被动短语\isce{范畴!功能!被动}{category!functional!Passive}（缩写为PassP）。参见图\ref{fig-passive-schlagen-mp}中针对例（\mex{1}）的分析：
...
支持 \citet{Cinque99a-u}理论的观点是有问题的，原因是：Cinque认为范畴AdverbP\isce{范畴!功能!副词}{category!functional!Adverb}
...
与前述我们讨论的局部性问题相关的是附着形式的位置的特殊功能投射的假设：如果使用SpeakerP\isce{范畴!功能!说话者}{category!functional!Speaker} ，那么第一人称单数的附着形式就可以移到正确的限定语位置上，而对于HearerP\isce{范畴!功能!听话者}{category!functional!Hearer}来说，第二人称附着形式可以移到正确的位置上\citep[\page
