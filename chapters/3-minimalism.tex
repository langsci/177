...
[TP
     [PassP
       ...
       [\phonliste{werden}\\\textsc{aux}]]
     [{T[past,\st{nom}]}
       [\textit{werden} {[Pass,\st{\textit{u}Infl}:past*]}\\\textsc{aux}\hspaceThis{[Pass,\st{\textit{u}Infl}:past*]}]
       [{T[past]}]]]
\end{forest}
%\end{sideways}
}
...
\gll {}[\emph{Wessen}     Schuhe] "`danach"'  besprenkelt sind, hat keinen Baum gefunden und war nicht zu einem Bogen in der Lage.\footnotemark\\
       \hspaceThis{[}谁的  鞋      \hspaceThis{"`}此后 有斑点的    \textsc{aux}   \textsc{aux} 不     树 找到    和 \textsc{cop} 不   \textsc{prep} 一     弓   \textsc{prep} \textsc{det} 位置\\
...
他 尝试 \textsc{inf} 劝说 但是 \textsc{aux}-NEG 说服 他\\
}
\zl
...
\gll Der          Mann, der         behauptet hat,         dass          Maria gelacht hat,         steht neben        der          Palme, die im letzten Jahr gepflanzt wurde.\\
     \textsc{det} 男人  \textsc{rel} 声称      \textsc{aux} \textsc{conj} Maria 大笑     \textsc{aux} 站   \textsc{prep} \textsc{det} 棕榈树 \textsc{rel} \textsc{prep}.\textsc{det} 上次 年 种植 \textsc{aux}\\
