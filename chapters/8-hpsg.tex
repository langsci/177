...
\noindent
我们可以将\argstlc 视为等同于\gbc 理论的深层结构\iscesee{深层结构}{D-结构}{Deep Structure}{D-stucture}：语义角色按照这个列表来指派。区别在于这里没有经历了转换过程\isce{转换}{transformation}的有序树。这样，所有的语言是从VO还是OV的语序生成而来的问题就无关紧要了。
...
%	 {}\spacebr{}because this man everyone knows\\
\zl
在HPSG和其他无转换的模型中，目标是为了将按照（\mex{0}b）中的顺序排列的论元指派到配价列表中的描写上。配价列表（或者新方法中的\argstc）对应于GB的深层结构（Deep Structure）\iscesee{深层结构}{D-结构}{Deep Structure}{D-stucture}。不同之处在于，中心语本身没有被包括进论元结构中，而这就是D-结构的情况。
