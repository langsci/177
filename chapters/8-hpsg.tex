...
与中心语通过\subcatc 来选择论元类似的是\isc{附加语|(}\is{adjunct|(}，附加语也可以通过使用特征（\textsc{modified}）来选择他们的中心语\isfeat{mod}。修饰名词的形容词与修饰关系小句的介词短语选择一个几乎完整的名词性投射，即一个名词只需要与限定词相组合便可以构成一个完整的NP。（\mex{1}）显示了每个\type{synsem}对象的描写。与\xbarc 理论中（请参阅\ref{sec-xbar}）类似的符号\nbarc 表示这个特征描写的缩写形式。
