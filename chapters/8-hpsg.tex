...
%}
目前，Ann Copestake开发的LKB系统\isce{语言知识构建系统}{Linguistic Knowledge Builder (LKB)} 和Gerald Penn \citep*{MPR2002a-u,Penn2004a-u}开发的TRALE系统拥有最多的用户。DELPH"=IN联盟\isc{DELPH-IN联盟}\is{DELPH-IN}（它的语法片段是基于LKB的）和许多TRALE用户\isc{TRALE系统}\is{TRALE}已经针对许多不同语言开发了许多小型语法和一些大规模的语法片段。下面列出的就是在不同系统中实现的语言：
...
} 和CoGETI\isc{CoGETI网络}\is{CoGETI}网络（基于约束的语法：经验、理论与实现）\footnote{%
...
在介绍完特征向量之后，我们现在可以转向中心语"=论元模式\isc{模式!中心语-论元}\is{schema!head"=argument}的内容了：
...
\end{schema}
模式~\ref{schema-bin-prel}表明了具有类型\type{head"=argument"=phrase}的语言对象必须具有的属性。模式~\ref{schema-bin-prel}中的箭头\isc{\impl}\is{\impl}表示逻辑蕴涵\isce{蕴涵}{implication}，并不是我们在短语结构语法中所知的重写规则的箭头。“$\oplus$”\isc{$\oplus$}\is{$\oplus$} （附加关系\iscesub{关系}{relation}{附加关系}{\emph{append}}）是包括两个列表的关系。（\mex{1}）显示了包括两个元素的列表的可能分叉结构：
...
\z
箭头\isc{\impl}\is{\impl}对应于上面提到的逻辑蕴涵\isce{蕴涵}{implication}。所以说，（\mex{0}）可以这样来解读：
...
中心语"=附加语结构由模式~\ref{ha-schema-prel}\isc{模式!中心语-附加语}\is{schema!head"=adjunct}所允准。\istype{head"=adjunct"=phrase}
...
在\isc{原则!格|(}\is{principle!Case|(}\ref{Abschnitt-struktureller-Kasus}中，结构格与结构之间是不同的。在HPSG的文献中，一般按照 \citet{Haider86}的观点，认为与格是一个词汇格。对于带有词汇格标记的论元来说，它们的格的值直接在论元的描写中表达。带有结构格的论元也在词汇描写中说明了，它们带有结构格，只不过没有给出格的值。为了不生成不合语法的结构，语法必须要确保结构格根据它们出现的语境而得到一个独一无二的值。这点由格原则来处理：\footnote{%
...
\eas
按照 \citew[\page 206--208]{Meurers2000b}的一般动词语迹：\isc{语迹!动词|)}\is{trace!verb|)}\isce{空成分}{empty element}\\
...
这并没有排除这样的事实，我们讨论的这个结构具有不同的属性，只要把它们的处理程度考虑进来的话。参见 \citew{Gibson98a,Hawkins99a}和第\ref{Abschnitt-Diskussion-Performanz}章。
 ...
 \citet[\page
  18]{Haider97c}指出，这里提出的这类分析中，VX语言的分支类型与XV语言的分支类型不同。这影响了c"=统制\isc{c-统制}\is{c"=command}关系，并且对GB/MP下的约束理论产生了影响。但是，分支的方向与HPSG的分析无关，因为约束原则被界定为使用了o"=统制\isc{o-统制}\is{o"=command}
...
\label{hf-schemaa}\isc{模式!填充语-中心语}\is{schema!Filler"=Head}
~\\\samepage
...
\begin{schema}[限定语-中心语模式]\isc{模式!限定语-中心语}\is{Schema!Specifier"=Head"=}
...
HPSG理论不同于\gbtc 及其后续的变体，因为它并没有假定转换关系。在上世纪80年代，有一些GB的表示变体被提出来，这些变体中没有D"=结构\isc{D-结构}\is{D"=structure}，S"=结构\isc{S-结构}\is{S"=structure}也不是从D"=结构通过对移位成分的原始位置进行标记而创造出来的。相反，有人直接假定带有语迹的S"=结构，连同S"=结构到逻辑形式\isc{逻辑形式（LF）}\is{Logical Form
