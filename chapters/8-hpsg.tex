...
\noindent
（\mex{0}）中的词汇规则允准了选择VP（图\ref{verb-movement-syn}中的\iboxt{1}）的动词。这个VP的\dslvc 对应于词汇规则输入动词的\locvc。\dslvc 的部分也是图\ref{verb-movement-syn} \iboxb{2}中表示的配价信息。因为\dslc 是一个中心语特征，VP的\dslvc 与动词语迹是相同的，而且由于动词语迹的\locvc 与\dslvc 是相同的，动词kennen的\subcatc 信息也可以在语迹中获得。语迹与其论元的组合跟普通动词相比是完全一样的。
