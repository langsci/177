...
} 和CoGETI\isce{CoGETI网络}{CoGETI}网络（基于约束的语法：经验、理论与实现）\footnote{%
...
%}. 
上面列出的大部分语法片段由DELPH-IN的成员开发，其中有一些是基于语法矩阵（Grammar Matrix）的\isce{语法矩阵}{Grammar Matrix}。语法矩阵是为LKB开发的一个平台，它为语法编写者提供了一个类型学驱动的初始语法，该语法对应于所开发语言的属性\citep*{BFO2002a-u}。核心语法工程（the CoreGram project）\isce{核心语法}{CoreGram}\footnote{\url{https://hpsg.hu-berlin.de/Projects/CoreGram.html}。 \mytodayc。
}
...
\gll dass Peter das Buch schläft\\
	 \textsc{comp} Peter \textsc{art}.\textsc{def} 书 睡觉\\
...
\gll dass Peter über den Mann erwartet\\
	 \textsc{comp} Peter 关于 \textsc{art}.\textsc{def} 人 等待\\
...
\gll {}[der Mann] und [die Frau]\\
	 {}\spacebr{}\textsc{art}.\textsc{def} 男人 和 \spacebr{}\textsc{art}.\textsc{def} 女人\\
...
\end{schema}
模式~\ref{schema-bin-prel}表明了具有类型\type{head-argument-phrase}的语言对象必须具有的属性。模式~\ref{schema-bin-prel}中的箭头\isce{\impl}{\impl}表示逻辑蕴涵\isce{蕴涵}{implication}，并不是我们在短语结构语法中所知的重写规则的箭头。“$\oplus$”\isce{$\oplus$}{$\oplus$} （附加关系\iscesub{关系}{relation}{附加关系}{\emph{append}}）是包括两个列表的关系。（\mex{1}）显示了包括两个元素的列表的可能分叉结构：
...
支配模式并没有说明任何跟子结点顺序有关的问题\indexgpsgc。正如在GPSG中，语序规则在HPSG中被另外处理。语序规则可以参考子结点的属性，或它们在模式中的功能（中心语\isce{中心语}{head}、论元\isce{论元}{argument}、附加语\isce{附加语}{adjunct} ……），或者两者都可参考。
...
%% } 
如果我们假定所有的中心语具有特征\initial\isfeat{initial} ，那么位于它们所带论元之前的中心语的\initialvc 为 “$+$”，而位于他们所带论元之后的中心语的值为“--”。（\mex{1}）中的线性顺序规则确保了诸如（\mex{2}b、d）的不合乎语法的顺序被规则排除了。\footnote{%
...
\ex[]{
\gll {}[in [den Schrank]]\\
     \spacebr{}\textsc{prep} \spacebr{}\textsc{art}.\textsc{def} 壁橱\\
%     \spacebr{}in \spacebr{}the cupboard\\
}
\ex[*]{
\gll {}[[den Schrank] in]\\
     \hspaceThis{[[}\textsc{art}.\textsc{def} 壁橱 \textsc{prep}\\
...
\gll {}[Dem Mann helfen] will er nicht.\\
 {}\spacebr{}\textsc{art}.\textsc{def} 男人 帮助 想 他 不\\
\mytrans{他不想帮助这个男人。}
...
{}\spacebr{}\textsc{art}.\textsc{def} 男人 帮助 \textsc{aux} 他 不\\
\mytrans{他没帮助这个男人。}
...
[V{[\type{fin}, \subcat \eliste]}, name=fin1
	...
	[V{[\type{fin}, \subcat \sliste{ \ibox{1} }]}, name=fin2
		[\ibox{2} NP{[\textit{dat}]}
			[dem Mann;\textsc{art}.\textsc{def} 男人,roof]]
		[V{[\type{fin}, \subcat \sliste{ \ibox{1}, \ibox{2} }]}, name=fin3
			[\ibox{3} NP{[\textit{acc}]}
				[das Buch;\textsc{art}.\textsc{def} 书,roof]]
...
GPSG\indexgpsgc 的中心语特征规约\isce{中心语特征规约（HFC）}{Head Feature Convention (HFC)}确保了子结点的中心语特征与那些中心语子结点上的特征是一致的。HPSG语法有一个类似的规则。但与GPSG不同的是，特征结构中的一组特征里明确地包含了中心语特征。他们被列于路径\textsc{synsem$|$loc$|$cat$|$head}下。（\mex{1}）说明了词汇项gibt（给）的信息：
...
GB、LFG和TAG这些理论跟HPSG与CxG这些理论的一个重要差别在于语言对象的语义内容是按照特征结构来模拟的，这跟所有其他的属性是一样的。正如前面提到的，语义信息在路径\textsc{synsem|""loc|""cont}下。（\mex{1}）给出了Buch（书）的\contvc 的例子。这一表示是基于最小递归语义\indexmrs （MRS）的：\footnote{%
...
\gll Die Frau$_i$ kauft ein Buch$_j$. Sie$_i$ liest es$_j$.\\
	  \textsc{art}.\textsc{def} 女人 买 一 书 她 读 它\\
...
\gll ein dem König treues Mädchen\\
	一 \textsc{art}.\textsc{def}.\dat{} 国王 忠诚的 女孩\\
\mytrans{对国王忠诚的女孩儿}
...
不同于修饰语选择中心语的假设，我们可以把中心语所有可能的附加语的描写都纳入到中心语中。这个观点由 \citet[\page 161]{ps}提出。 \citet[\S~1.9]{ps2}对早期的分析进行了修订，因为不能说明修饰语的语义。\footnote{%
...
\gll die Wurst in der Speisekammer\\
     \textsc{art}.\textsc{def} 香肠 \textsc{prep} \textsc{art}.\textsc{def} 食品箱\\
%     the sausage in the pantry\\
}
\ex[*]{
\gll die Wurst in\\
	 \textsc{art}.\textsc{def} 香肠 \textsc{prep}\\
%	 the sausage in\\
}
\zl
...
}一条词汇规则\isce[|(]{词汇规则}{lexical rule}将词根作为输入，并允准了分词形式，并且最凸显的论元（所谓的指定论元\iscesub{论元}{argument}{指定论元}{designated}）受到了抑制。\footnote{%
...
\gll Der Mann hat den Weltmeister geschlagen.\\
	   \textsc{art}.\textsc{def} 男人 \textsc{aux} \textsc{art}.\textsc{def} 世界.冠军 打\\
\mytrans{这个男人打了世界冠军。}
...
\gll Der Mann wird den Weltmeister geschlagen.\\
	\textsc{art}.\textsc{def} 男人 \passiveprs{} \textsc{art}.\textsc{def} 世界.冠军 打\\
%	 the man is the world.champion beaten\\
}
\ex[]{
\gll Der Weltmeister wird geschlagen.\\
	 \textsc{art}.\textsc{def} 世界.冠军 \passiveprs{} 打\\
\mytrans{世界冠军被人打了。}
...
在\iscesub[|(]{原则}{principle}{格}{Case}\ref{Abschnitt-struktureller-Kasus}中，结构格与结构之间是不同的。在HPSG的文献中，一般按照 \citet{Haider86}的观点，认为与格是一个词汇格。对于带有词汇格标记的论元来说，它们的格的值直接在论元的描写中表达。带有结构格的论元也在词汇描写中说明了，它们带有结构格，只不过没有给出格的值。为了不生成不合语法的结构，语法必须要确保结构格根据它们出现的语境而得到一个独一无二的值。这点由格原则来处理：\footnote{%
...
我们在这儿无法得到完美的分析。不过，需要指出的是，与分词相同的词汇项被用于（\mex{1}）。
...
\gll Gibt [der Mann der Frau das Buch \_$_{gibt}$]?\\
	 给 \spacebr{}\textsc{art}.\textsc{def} 男人 \textsc{art}.\textsc{def} 女人 \textsc{art}.\textsc{def} 书\\
...
\gll Kennt [der Mann der Frau das Buch \_$_{gibt}$]?\\
	 认识 \spacebr{}\textsc{art}.\textsc{def} 男人 \textsc{art}.\textsc{def} 女人 \textsc{art}.\textsc{def} 书\\
...
特征\dslc 是由 \citet*{Jacobson87}在范畴语法\indexcg 的框架下提出用来描写英语\ilce{英语}{English}倒装\isce{助动词倒装}{auxiliary inversion}的中心语移动的。 \citet{Borsley89}采用了这一观点，并将其译为HPSG的术语，这样就可以看到，在CP/IP系统的HPSG变体中，中心语移位是如何用\textsc{dsl}模拟的。\textsc{dsl}特征在HPSG的中心语移位过程中的引入由这样的事实驱动，与\ref{Abschnitt-Fernabhängigkeiten-HPSG}讨论的长距离依存\isce{长距离依存}{long-distance dependency}不同的是，这类移位是局部的。
...
对于动词位于首位的具体的词汇项由下面的词汇规则\iscesub[|(]{词汇规则}{lexical rule}{动词首位位置}{verb-initial position}允准：\footnote{%
...
具有固定语序的语言\iscesub{成分序列}{constituent order}{固定}{fixed}\iscesub{成分序列}{consituent order}{自由}{free}（如英语）与德语这类语言不同，因为它们从一个方向开始释放论元（更多有关英语\ilce{英语}{English}的主语的内容，参见\ref{Abschnitt-Spr}），而自由语序的语言可以按照任意顺序将动词与论元相组合。在固定语序的语言中，要么是\ibox{1}，要么是\ibox{3}，总是空列表。由于德语结构没有受到与\ibox{1}或\ibox{3}相关的限制，也就是说，\ibox{1}和\ibox{3}要么是空列表，要么包括某些成分，这一直觉是指自由语序的语言比固定语序的语言具有更少的限制。我们可以将这个分析与\ref{Abschnitt-Kaynesche-Modelle}中的Kayne式\aimention{Richard S. Kayne}的分析相比较，那儿的分析认为所有语言都是从基础语序 [specifier [head complement]]推导而来的（参见图\vref{Abbildung-Remnant-Movement-Satzstruktur}关于德语作为SVO语言的分析\citep{Laenzlinger2004a}）。在这些分析中，像英语这种语言是最基本的情况，而自由语序的语言需要在理论上付出相当多的努力来得到正确的语序。与之相对的是，这里提出的分析需要更多理论上的限制，如果这个语言在它的成分排列上具有更多的限制的话。被允准结构的复杂度在HPSG理论的方法下并没有语言与语言之间的区别。语言只是在它们所属的分支类型上是不同的。\footnote{%
...
长距离依存\isce[|(]{长距离依存}{long-distance dependency}分析应用了由GPSG中发展而来的技术：关于缺失成分的信息被传递到树上（或者特征结构中）。\footnote{%
...
\gll Vielleicht hat der Schauspieler seinen Text vergessen.\\
     也许    \textsc{aux} \textsc{art}.\textsc{def} 男演员 他的 台词 忘记\\
...
\gll Einem Schauspieler ist der Text entfallen.\\
     一.\dat{} 男演员 \textsc{aux} \textsc{art}.\textsc{def}.\nom{} 台词 忘记\\
\mytrans{一位男演员忘记台词了。}
%     a.\dat{} actor is the.\nom{} text forgotten\\
%\mytrans{An actor forgot the text.}
\ex
\gll Einem Kind wurde das Fahrrad gestohlen.\\
     一.\dat{} 孩子 \passivepst{} \textsc{art}.\textsc{def}.\nom{} 自行车 偷\\
...
\gll Hat vielleicht der Schauspieler seinen Text vergessen?\\
     \textsc{aux} 也许    \textsc{art}.\textsc{def} 男演员 他的 台词 忘记\\
...
\gll Ist einem Schauspieler der Text entfallen?\\
      \textsc{aux}  一.\dat{} 男演员     \textsc{art}.\textsc{def}.\nom{} 台词 忘记\\
\mytrans{一个男演员忘记台词了吗？}
 %    is  a.\dat{} actor     the.\nom{} text forgotten\\
%\mytrans{Did an actor forget the text?}
\ex
\gll Wurde einem Kind das Fahrrad gestohlen?\\
     \passivepst{} 一.\dat{} 孩子 \textsc{art}.\textsc{def}.\nom{} 自行车 偷\\
...
%\mytrans{Everyone knows this man.}
\z
在介绍完基本思想后，我们现在来看技术上的细节：与我们在\ref{Abschnitt-Verbstellung-HPSG}中讨论的动词移位不同的是，成分移位是非局部的，这就是为什么两个移位类型按照不同的特征（\textsc{slash}\isfeat{slash} vs.\ \textsc{dsl}\isfeat{dsl}）来模拟的原因。\textsc{dsl}是一个中心语特征，而且跟其他所有中心语特征一样，投射到投射层的最高点（更多有关中心语特征原则的内容，参见第\pageref{prinzip-hfp}页）。另一方面，\slaschc 是一个属于\textsc{synsem|nonloc}下表示的\textsc{nonloc}特征的特征。\nonlocc 特征的值是带有特征\textsc{inherited}（或者简写为\textsc{inher}）和\textsc{to-bind}的结构。\textsc{inher}的值是一个结构，它包含长距离依存所涉及的元素的信息。（\mex{1}）给出了 \citet[\page 163]{ps2}提出的结构：\footnote{%
...
\label{hf-schemaa}\isce{模式}{schema}{填充语-中心语}{Filler-Head}
~\\\samepage
...
\begin{schema}[限定语-中心语模式]\isce{模式}{schema}{限定语-中心语}{Specifier-Head}
...
\gll die          Zerstörung der          Stadt durch         die Soldaten\\
     \textsc{art}.\textsc{def} 毁灭        \textsc{art}.\textsc{def} 城市  \textsc{prep} \textsc{art}.\textsc{def} 士兵\\
...
%				[Zerstörung;destruction]]
			[\ibox{3} NP{[\type{gen}]}
				[der Stadt; \textsc{art}.\textsc{def} 城市,roof]]]
%				[der Stadt; of the city,roof]]]
		[\ibox{2} PP{[\type{durch}]}
			[durch die Soldaten; \textsc{prep} \textsc{art}.\textsc{def} 士兵,roof]]]]
...
%the objects in terms of possible extractions is captured.
 ...
 \citew[\S~9]{ps2}提出的进一步的扩展是引入一个额外的列表，它在较新的研究中叫做\argst\isfeat{arg-st}。\argstc 表示论元结构。\argstlc 对应于我们在本章遇到的\subcatlc。它包括中心语的论元，它们按照旁格等级来确定顺序。这个列表中的成分连接到中心语的语义内容中的论元角色（参见\ref{Abschnitt-HPSG-Semantik}）。约束理论应用于\argstlc。这一层次的表达可能对大部分语言来说都是一样的：每一种语言都有语义谓词和语义论元。大多数语言利用句法范畴在选择中发挥作用，所以既有句法选择，也有语义选择。\footnote{%
...
\noindent
我们可以将\argstlc 视为等同于\gbc 理论的深层结构\isce{深层结构}{Deep Structure}：语义角色按照这个列表来指派。区别在于这里没有经历了转换过程\isce{转换}{transformation}的有序树。这样，所有的语言是从VO还是OV的语序生成而来的问题就无关紧要了。
...
在HPSG中，特征描写被用来为语言对象的所有属性建立模型：根、词、词汇规则和支配模式都用相同的形式工具来描写。与GPSG\indexgpsgc 和LFG\indexlfg 不同的是，这里没有独立的短语结构规则。由此，尽管HPSG代表中心语驱动的短语结构语法，这里并没有短语结构语法。在HPSG的实现中，短语结构的支撑通常用来提高处理的效率。但是，这并不属于理论的一部分，而且在语言学上也不是必需的。
...
\gll der Mann, [\sub{RS} [\sub{PP} an den] [\sub{S/PP} wir gedacht haben]]\\
     \textsc{art}.\textsc{def} 男人   {}        {}        \textsc{prep} \textsc{rel}  {}          我们 想 \textsc{aux}\\
\mytrans{我们想起的那个男人}
...
在最近的会议论文集中，也可以看到对于短语模式进行区分的倾向。提出的观点从对空元素的删除到激进地采取短语式的分析都有。\footnote{%
...
HPSG利用了类型特征描写来描述语言对象。概括可以通过带有多重承继的体系来表示。承继关系在构式语法\indexcxg 中也起到了重要的作用。在诸如GPSG\indexgpsgc、范畴语法\indexcg  和TAG\indextag 中，它并不是理论解释的一部分。在实现中，宏语\isce{宏语}{macro}（macros）通常用来表示共现的特征值偶对\citep*{DKK2004a}。按照假定的构架，这类宏语不适合短语的描写，因为，在诸如GPSG\indexgpsgc 和LFG\indexlfg 的理论中，短语结构规则的表达是不同于其他特征值偶对的（但是，参见 \citew*{ADT2008a,ADT2013a}用于c-结构标记的宏和承继关系）。进而，类型和宏之间还有更多不同之处，这些差异具有更为正式的性质：在类型系统中，类型在一定条件下可以从具体特征和具体值中推导出具体结构。对于宏来说，并不是这样，因为它们只是简称。不过，由这种差异引起的语法分析的后果是微不足道的。
...
在\gbtc 中，主语\isce{主语}{subject}和宾语\isce{宾语}{object}这两个术语没有起到直接的作用：我们可以用这些术语来进行描写，但是主语和宾语并没有根据特征或相似的机制来标记。无论如何，我们也是可以做出区分的，因为主语和宾语通常都在树中不同的位置上实现（主语位于IP的限定语位置上，而宾语作为动词的补足语）。在HPSG理论中，主语和宾语也不是理论的原始对象。因为配价列表是有序的，这就意味着可以将\argstc 成分与语法功能联系起来：如果有一个主语，这发生在配价列表的第一个位置上，然后宾语紧随其后。\footnote{%
...
%	 {}\spacebr{}because this man everyone knows\\
\zl
在HPSG和其他无转换的模型中，目标是为了将按照（\mex{0}b）中的顺序排列的论元指派到配价列表中的描写上。配价列表（或者新方法中的\argstc）对应于GB的深层结构（Deep Structure）\isce{深层结构}{Deep Structure}。不同之处在于，中心语本身没有被包括进论元结构中，而这就是D-结构的情况。
...
%}  
在这个分析中， HPSG中的论元结构对应于LFG中的f-结构。LFG\indexlfg 的扩展的中心语域\iscesub{中心语域}{head domain}{扩展}{extended}也可以在HPSG中来模拟，其中多重中心语可以共享相同的f-结构。为此，我们可以使用函数组合\isce{函数组合}{function composition}，因为它在范畴语法\indexcg 那章中被提出来了（参见第\ref{Kategorialgrammatik-Komposition}章）。限于篇幅，这点被译成HPSG理论的确切方式就不在这里解释了。读者可以参考 \citet{HN94a}的原文，以及 \citew[\S~15]{MuellerLehrbuch1}的解释。
...
\gll Dem Mann wurde ein Buch geschenkt.\\
	 \textsc{art}.\textsc{def}.\dat{} 男人 \passivepst{} 一.\nom{} 书 给\\
\mytrans{这个男人被给了一本书。}
%	 the.\dat{} man was a.\nom{} book given\\
