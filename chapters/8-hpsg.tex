...
目前，在语法开发方面有两大团体：DELPH-IN联盟（应用HPSG的深层语言处理）\bracketfootnote{%
...
%\vspace{-\baselineskip}
%\end{figure}%
我们可以看到该特征描写包括词的语音、句法范畴和语义。为了简便，\textsc{phonology}(\phonc )的值大部分按照正字法来表示。在完整的理论中，\phonvc 是一个包括节律栅\isce{节律栅}{metrical grid}和轻重音\isce{轻重音}{accent}信息的复杂结构。参见 \citew{BK94b}、 \citew{Orgun96a}、 \citew{Hoehle99a-u}、 \citew{Walther99a-u}、 \citew[\S~6]{Crysmann2002a}和 \citew{Bildhauer2008a}在HPSG框架下关于语音学\isce{语音学}{phonology}的分析。针对（\mex{0}）中表示的详细信息将在下面的章节中给予解释。
...
如果我们假定所有的中心语具有特征\initial\isfeat{initial} ，那么位于它们所带论元之前的中心语的\initialvc 为 “$+$”，而位于他们所带论元之后的中心语的值为“--”。（\mex{1}）中的线性顺序规则确保了诸如（\mex{2}b、d）的不合乎语法的顺序被规则排除了。\dotfootnote{%
  名词短语会给（\mex{1}）带来问题：截至目前，限定词被看作是论元，并且被囊括进中心语名词的\subcatlc 中。限定词在名词的左边出现，而名词的所有其他论元在右边出现。这个问题可以通过重新界定线性化规则\citep[\page 164--165]{Mueller99a}来解决，或者通过为限定词引入一个特殊的配价属性来解决\citep[\S~9.4]{ps2}。有关使用这一特征的方法，参见\ref{Abschnitt-Spr}。
}
...
GB、LFG和TAG这些理论跟HPSG与CxG这些理论的一个重要差别在于语言对象的语义内容是按照特征结构来模拟的，这跟所有其他的属性是一样的。正如前面提到的，语义信息在路径\textsc{synsem|""loc|""cont}下。（\mex{1}）给出了Buch（书）的\contvc 的例子。这一表示是基于最小递归语义\indexmrs （MRS）的：\colonfootnote{%
...
%     they see themselves\\
\zl
共指的哪个部分是可能的还是必须的这个问题由约束理论决定\isce{约束理论}{Binding Theory}。 \citet{PS92a,ps2}指出，HPSG中的约束理论在实现约束关系时并没有像GB理论中关于树的构型问题那样引起许多的问题。但是，HPSG理论中的约束理论还有一些待解决的问题\citep[\S~20.4]{Mueller99a}。
...
不同于修饰语选择中心语的假设，我们可以把中心语所有可能的附加语的描写都纳入到中心语中。这个观点由 \citet[\page 161]{ps}提出。 \citet[\S~1.9]{ps2}对早期的分析进行了修订，因为不能说明修饰语的语义。\dotfootnote{%
...
\end{figure}%
% with a new paragraph this looks strange. So I run the two sentences together.
...
在\iscesub[|(]{原则}{principle}{格}{Case}\ref{Abschnitt-struktureller-Kasus}中，结构格与结构之间是不同的。在HPSG的文献中，一般按照 \citet{Haider86}的观点，认为与格是一个词汇格。对于带有词汇格标记的论元来说，它们的格的值直接在论元的描写中表达。带有结构格的论元也在词汇描写中说明了，它们带有结构格，只不过没有给出格的值。为了不生成不合语法的结构，语法必须要确保结构格根据它们出现的语境而得到一个独一无二的值。这点由格原则来处理：\colonfootnote{%
这里的格原则被简化了。所谓的“升”格\isce{提升}{raising}需要特殊的处理。更多细节可以参考 \citew{Meurers99b}、 \citew{Prze99b}和 \citew[\S~14,
    \S~17]{MuellerLehrbuch1}。这些著作中给出的格原则与 \citet*{YMJ87}提出的理论非常相似，由此该原则也可以解释他们的著作中所讨论的语言的格系统，尤其是爱尔兰语复杂的格系统。\isce{爱尔兰语的}{Icelandic}
}
...
我们在这儿无法得到完美的分析。但是应当注意的是，与分词相同的词汇项可用于（\mex{1}）。
...
\zl
助动词决定了哪些论元被实现（\citealp{Haider86}、\citealp[\S~17]{MuellerLehrbuch1}）。（\ref{pass-lr-mlr-str}）中的词汇规则允准了可以用在被动和完成式中的形式。这样，\vformvc 属于\type{ppp}，它表示完成式被动分词（participle perfect passive）。
...
对于动词位于首位的具体的词汇项由下面的词汇规则\iscesub[|(]{词汇规则}{lexical rule}{动词首位位置}{verb-initial position}允准：\colonfootnote{%
...
这并没有排除这样的事实，我们讨论的这个结构具有不同的属性，只要把它们的处理程度考虑进来的话。参见 \citew{Gibson98a,Hawkins99a}和第\ref{Abschnitt-Diskussion-Performanz}章。
 ...
 \citet[\page
  18]{Haider97c}指出，这里提出的这类分析中，VX语言的分支类型与XV语言的分支类型不同。这影响了c-统制\isce{c-统制}{c-command}关系，并且对GB/MP下的约束理论产生了影响。但是，分支的方向与HPSG的分析无关，因为约束原则被界定为使用了o-统制\isce{o-统制}{o-command}
    \citep[\S~6]{ps2}，而o-统制对应于旁格层级\isce{旁格}{obliqueness}，即\subcatlc 中元素的语序，而不是这些元素与中心语组合的语序。
}
...
在本章中，\subcatc 被认为是唯一的配价特征。这对应于 \citew[\S~1--8]{ps2}的理论主张。对于组成成分的组合，还需要至少一个额外的配价特征和一个对应的模式。这个额外的特征叫做限定语（\textsc{specifier}，简写为\textsc{spr}），它被用来表示英语\citep[\S~9]{ps2}和德语\citep[\S~9.3]{MuellerLehrbuch1}语法中限定词与名词的组合。一般认为，名词选择它的限定词。对于名词Zerstörung（毁坏）来说，我们有如下的\catvc：
...
%
%
在英语语法中，\sprc 特征也用来表示动词对主语的选择\citep*[\S~4.3]{SWB2003a}。在（\mex{1}）这样的句子中，动词首先与它的所有补足语组合（较新的研究中的\subcatc 和\compsc 中的成分），然后在第二步应用模式\ref{schema-spr-h}将主语组合进来。
...
\citew[\S~9]{ps2}提出的进一步的扩展是引入一个额外的列表，它在较新的研究中叫做\argst\isfeat{arg-st}。\argstc 表示论元结构。\argstlc 对应于我们在本章遇到的\subcatlc。它包括中心语的论元，它们按照旁格等级来确定顺序。这个列表中的成分连接到中心语的语义内容中的论元角色（参见\ref{Abschnitt-HPSG-Semantik}）。约束理论应用于\argstlc。这一层次的表达可能对大部分语言来说都是一样的：每一种语言都有语义谓词和语义论元。大多数语言利用句法范畴在选择中发挥作用，所以既有句法选择，也有语义选择。\dotfootnote{%
...
\addlines[2]
在HPSG的第一个十年中（\citealp*{ps,ps2,NNP94a-ed-not-crossreferenced}），HPSG与范畴语法是非常相似的，即使这里已经提到了一些区别，这是因为它是一个强势的基于词汇的理论。短语的句法构造与语义内容都是由中心语决定的（所以叫做中心语驱动的）。一旦遇到无法直接进行中心语驱动的分析，比如所讨论的短语中没有中心语，那么通常的做法就是假定空中心语\isce{空成分}{empty element}。一个例子就是 \citet[\S~5]{ps2}中关系小句的分析。由于空中心语可以被指派给任意句法配价和任意的语义（关于这一点的讨论，参见第\ref{Abschnitt-Diskussion-leere-Elemente}章），我们并没有好的理由来解释人们为什么要假定空的中心语，比如说这个空位置可以在其他语境中得到实现。但是，为服务于理论假设而提出的空成分并不是这样（即空成分并不能在其他语境中出现）。基于此， \citet{Sag97a}提出了不用任何空成分的关系小句的分析。正如（\mex{-1}）和（\mex{0}）草拟的分析一样，关系小句直接由分句组合而成，以构成关系小句。对于英语中可观察到的不同类型的关系小句，Sag提出了不同的支配规则。他的分析偏离了强势的词汇主义：在 \citew{ps2}中，只有六条支配模式，而在 \citew{GSag2000a-u}中有23条。
...
\furtherreading{
这里，理论各部分的表示跟其他理论一样都是相对来说比较简短的。对于HPSG理论更为全面的介绍，包括特征构架的动机，参见 \citew{MuellerLehrbuch1}。\nocite{Mueller99a,Mueller2002b}特别是，这里仅简略说明了被动的分析。更为全面的分析包括非宾格动词、形容词分词、情态不及物动词、不同的被动变体以及长被动\iscesub{被动}{passive}{长被动}{long}，这些可以参考 \citew[\S~3]{Mueller2002b}和 \citew[\S~17]{MuellerLehrbuch1}。
