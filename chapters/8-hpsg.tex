...
HPSG从其他理论中借鉴了许多不同的思路，而且近期的分析受到了其他理论框架理论发展的影响。针对配价信息和函项组合\isce{函项组合}{function composition}处理的函数-变元结构借鉴自范畴语法\indexcgc。函项组合在对德语和汉语这类语言的复杂动词结构的分析中起到了重要的作用。直接支配\iscesub{支配}{dominance}{直接支配}{immediate}/线性次序\isce{线性次序}{linear precedence}\isce{ID/LP语法}{ID/LP grammar}模式（ID/LP模式，参见第\ref{sec-IDLP-intro}节）和长距离依存（参见\ref{sec-nld-gpsg}）的Slash机制来自GPSG\indexgpsgc。这里针对德语动词位置的分析受到管辖与约束理论\indexgbc 框架下开发的语法的启发（参见\ref{sec-verb-position-gb}）。
...
我们应该注意到，该分析适用于没有成分移动的被动。这里没有涉及GB分析中的问题\indexgbc。论元的重新排序（参见\ref{Abschnitt-HPSG-lokale-Umstellung}）是独立于被动化的。与\gpsgc、\cgc 或Bresnan的LFG分析\indexlfgc 不同的是，宾格宾语在词汇映射理论\isce{词汇映射理论（LMT）}{Lexical Mapping Theory (LMT)}（参见第\pageref{page-LMT}页）引入之前完全没有被提及。被动可以直接分析为主语的抑制。任何别的成分都与语法的其他原则具有互动关系。\isce[|(]{被动}{passive}
