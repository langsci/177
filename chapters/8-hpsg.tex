...
HPSG理论不同于\gbtc 及其后续的变体，因为它并没有假定转换关系。在上世纪80年代，有一些GB的表示变体被提出来，这些变体中没有D"=结构\isc{D-结构}\is{D"=structure}，S"=结构\isc{S-结构}\is{S"=structure}也不是从D"=结构通过同时对移位成分的原始位置进行标记而创造出来的。相反，有人直接假定带有语迹的S"=结构，而且连S"=结构到逻辑形式\isc{逻辑形式}\is{Logical Form
