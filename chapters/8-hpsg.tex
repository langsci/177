...
Packard2015a-u% ACE http://moin.delph-in.net/AceTop
 ...
 \citet{UBCCDDEEMMO-96a}和 \citet{Bolc:Czuba:ea:96a-u}比较了已有的和1990年代初开发的系统。 \citet{MelnikHandWritten}比较了LKB和 TRALE。也请参阅 \citew[5.1]{MuellerCoreGram}。
}
%.\footnote{%
...
如果我们假定所有的中心语具有特征\initial\isfeat{initial} ，那么位于它们所带论元之前的中心语的\initialvc 为 “$+$”，而位于他们所带论元之后的中心语的值为“--”。（\mex{1}）中的线性顺序规则确保了诸如（\mex{2}b，d）的不合乎语法的顺序被规则排除了。\dotfootnote{%
  名词短语会给（\mex{1}）带来问题：截至目前，限定词被看作是论元，并且被囊括进中心语名词的\subcatlc 中。限定词在名词的左边出现，而名词的所有其他论元在右边出现。这个问题可以通过重新界定线性化规则\citep[\page 164--165]{Mueller99a}来解决，或者通过为限定词引入一个特殊的配价属性来解决\citep[9.4]{ps2}。有关使用这一特征的方法，参见\ref{Abschnitt-Spr}。
}
...
%     they see themselves\\
\zl
共指的哪个部分是可能的还是必须的这个问题由约束理论决定\isce{约束理论}{Binding Theory}。 \citet{PS92a,ps2}指出，HPSG中的约束理论在实现约束关系时并没有像GB理论中关于树的构型问题那样引起许多的问题。但是，HPSG理论中的约束理论还有一些待解决的问题\citep[20.4]{Mueller99a}。
...
不同于修饰语选择中心语的假设，我们可以把中心语所有可能的附加语的描写都纳入到中心语中。这个观点由 \citet[\page 161]{ps}提出。 \citet[1.9]{ps2}对早期的分析进行了修订，因为不能说明修饰语的语义。\dotfootnote{%
...
这里展示的分析应用了任意顺序的论元的组合，这与GB/MP框架下的 \citet{Fanselow2001a}的分析，以及 \citet[3.1]{Hoffmann95a-u}和 \citet{SB2006a-u}的范畴语法的分析是很相似的。Gunji\nocite{Gunji86a}早在1986年就针对日语\ilce{日语}{Japanese}提出了类似的HPSG分析。
...
在本章中，\subcatc 被认为是唯一的配价特征。这对应于 \citew[第1--8章]{ps2}的理论主张。对于组成成分的组合，还需要至少一个额外的配价特征和一个对应的模式。这个额外的特征叫做限定语（\textsc{specifier}，简写为\textsc{spr}），它被用来表示英语\citep[第9章]{ps2}和德语\citep[9.3]{MuellerLehrbuch1}语法中限定词与名词的组合。一般认为，名词选择它的限定词。对于名词Zerstörung（毁坏）来说，我们有如下的\catvc：
...
%
%
在英语语法中，\sprc 特征也用来表示动词对主语的选择\citep*[4.3]{SWB2003a}。在（\mex{1}）这样的句子中，动词首先与它的所有补足语组合（较新的研究中的\subcatc 和\compsc 中的成分），然后在第二步应用模式\ref{schema-spr-h}将主语组合进来。
