...
%}
目前，Ann Copestake开发的LKB系统\isce{语言知识构建系统}{Linguistic Knowledge Builder (LKB)} 和Gerald Penn \citep*{MPR2002a-u,Penn2004a-u}开发的TRALE系统拥有最多的用户。DELPH-IN联盟\isce{DELPH-IN联盟}{DELPH-IN}（它的语法片段是基于LKB的）和许多TRALE用户\isce{TRALE系统}{TRALE}已经针对许多不同语言开发了许多小型语法和一些大规模的语法片段。下面列出的就是在不同系统中实现的语言：
...
HPSG从其他理论中借鉴了许多不同的思路，而且近期的分析受到了其他理论框架理论发展的影响。针对配价信息和功能组合\isce{功能组合}{function composition}处理的函子参数结构借鉴自范畴语法\indexcg。函子构成在对德语和汉语这类语言的复杂动词结构的分析中起到了重要的作用。直接支配\iscesub{支配}{dominance}{直接支配}{immediate}/线性次序\isce{线性次序}{linear precedence}\isce{ID/LP语法}{ID/LP grammar}模式（ID/LP模式，参见第\ref{sec-IDLP-intro}节）和长距离依存（参见\ref{sec-nld-gpsg}）的Slash机制来自GPSG\indexgpsg。这里针对德语动词位置的分析受到管辖与约束理论\indexgb 框架下开发的语法的启发（参见\ref{sec-verb-position-gb}）。
...
\begin{samepage}
\iscesub{原则}{principle}{非局部}{nonlocal feature}
...
也参见 \citew{Mueller2008a}关于德语和英语\ilce{英语}{English}中描写性谓词\isce{描写性谓词}{depictive predicate}的分析，他们分别指向中心语的实现的和未实现的论元列表。这一分析也可以在\ref{sec-locality}中得到解释。
}  
