...
%%
    在转入下一节有关论元结构的极端不充分赋值方法之前，我想评论一下 \citet*{AGT2014a}最近的一篇文章。该文章讨论了用短语结构的方法引入同源宾语和受益者\isce[|(]{受益者}{benefactive}。后者的例子见（\mex{1}a）。
...
\zl
按照作者的观点，名词短语the children（儿童们）不是sing（唱）的论元，而是由非强制性允准受益者的c-结构规则贡献的。
...
\z
每当该规则被激活时，\textsc{Benefactive}模板会增加一个受益者角色并且相应语义也会与插入结构中的动词兼容。作者展示了如何利用映射来解释例（\mex{-1}b）中的被动\isce[|(]{被动}{passive}句，但是没有提供允准这些例句的c-结构。为了分析这些例句，需要一个分析被动VP的c-结构规则并且这个规则可以允许一个受益者。所以情况会是：
...
\z
在 \citet[\page 220]{Mueller2004b}中，我曾经指出过在复杂形式谓词中的嵌套只是受到语言运用\isce{语言运用}{performance}因素的制约（也可以参见\ref{Abschnitt-Kompetenz-Performanz-TAG}）。在德语中，包含四个以上动词的复杂动词\isce{复杂动词}{verbal complex}很少见。 \citet[\page 58--59]{Evers75a}指出，在荷兰语\ilce{荷兰语}{Dutch}中情况有所不同，因为荷兰语的复杂动词的分支结构不同于德语：在荷兰语中，包含五个以上动词的复杂动词是存在的。Evers将这种差异归结于德语复杂动词的处理负荷更大（也可以参见\citet[\S~3.7]{Gibson98a}）。Haugereid不得不假设荷兰语比德语有更多的规则。按照这个方式，他就会放弃语言运用与语言能力之间的差异并且将语言运用限制直接纳入到语法中。如果他要保持两者的差异，就需要假设无限的图式或者具有功能不确定性\isce{功能不确定性}{functional uncertainty}的规则，因为嵌套深度只受到语言运用因素的约束\citep{HN94a}。因为不管怎样，对于宾语提升来说需要这种提升分析（正如上面所讨论的），我们需要对这些问题予以重视。
...
\z
\ea
向后组合\iscesub{组合}{composition}{向后}{backward} (< B)\\
